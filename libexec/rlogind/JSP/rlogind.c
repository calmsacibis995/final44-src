/*-
 * Copyright (c) 1983, 1988, 1989, 1993, 1995
...
"%Z% Copyright (c) 1983, 1988, 1989, 1993, 1995\n\
	The Regents of the University of California.  All rights reserved.\n";
#endif /* not lint */
...
{
	int authenticated = 0, master, pid, on = 1;
	struct hostent *hp;
	...
	int f, p;
{
	pcc = 0, fcc = 0;
	int cc, nfd, n;
	char cntl, *fbp, *pbp, pibuf[1024+1], fibuf[1024];
...
setup_term(fd)
	int fd;
{
	char *cp = strchr(term+ENVSIZE, '/'), *speed;
	struct termios tt;
