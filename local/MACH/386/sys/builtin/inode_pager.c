 ...
 * HISTORY
 * $Log:	inode_pager.c,v $
 * Revision 2.34.3.1  91/07/14  23:53:46  rvb
 * 	USER CONTRIBUTED: allow root to be "perferred"
 * 	[91/06/27            mg32]
 * 
...
#define INDIRECT_PAGEMAP(vs)	(PAGEMAP_SIZE((vs)->size) > PAGEMAP_THRESHOLD)
	...
	boolean_t	can_cache;
{
	...
	if (ip->i_vm_info->pager == MEMORY_OBJECT_NULL) {
...
#if	MACH_XP
		is = inode_pager_create(ip, TRUE, FALSE);
		...
		inode_pager_t is = inode_pager_lookup(ip->i_vm_info->pager);
	...
	port_reference(ip->i_vm_info->pager);
		...
		if (can_cache)
			pager_cache(vm_object_lookup(ip->i_vm_info->pager), TRUE);
	}
#endif	MACH_XP
	...
	vm_offset_t	addr;
{
	...
	if (is->swapfile) {
		if ((actual_offset = pf_bmap(is, offset, B_READ)) == 
							    (vm_offset_t)-1) 
			return;
	} else {
...
#if	MACH_VFS
	(void) VOP_PAGE_WRITE(ITOV(ip), addr, PAGE_SIZE,
			   actual_offset << is->paging_shift,
			   is->identity, TRUE);
#else	MACH_VFS
	if (inode_has_page(is->pager, actual_offset << is->paging_shift)) {
 	    	memory_object_data_write(is->pager,
					 is->pager_request,
					 actual_offset,
					 addr,
					 PAGE_SIZE);
...
inode_pager_timeout()
{
	...
	ip = istruct_get(is);
	LOCK_INODE(ip);
	ret = is_page_read(is, ip, entry->offset, inode_pager_input_buffer);
	switch(ret) {
		case 0:
	...
	vm_page_t	m;
{
	...
	if (!is->paging) {
		 ...
		 */
		if (f_offset + size > ip->i_vm_info->inode_size)
			size = ip->i_vm_info->inode_size - f_offset;
	}
#endif	MACH_NBC
	...
	boolean_t	never;
{
	...
	unix_master();
	for (i = 0; i < NMOUNT; i++)
		if ((mount[i].m_dev == device) && (mount[i].m_bufp != NULL)) {
	...
	vm_size_t	length;
{
	...
	printf("memory_object_lock_completed: called\n");
	return(KERN_FAILURE);
}
...
void		inode_pager()
{
	...
	(void) vm_set_default_memory_manager(inode_pager_self, &t);
	...
	memory_object_t	pager;
{
	...
	ip->i_flag &= ~ITEXT;
	ip->i_vm_info->pager = MEMORY_OBJECT_NULL; /* so irele will free */
	RELEASE_INODE(ip);
	...
	register struct inode	*ip;
{
	...
	mfs_uncache(ip);
	pager_cache(vm_object_lookup(ip->i_vm_info->pager), FALSE);
	...
	register struct inode	*ip;
{
	...
	if ((o = vm_object_lookup(ip->i_vm_info->pager)) != VM_OBJECT_NULL)
		inode_uncache(ip);
