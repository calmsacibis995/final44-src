 ...
 * HISTORY
 * $Log:	pcb.c,v $
 * Revision 1.8.1.7  90/12/07  22:53:11  rvb
 * 	Add t_bitmap to allow v8086 tasks do vga and other groovy stuff
 * 	directly.
 * 	[90/11/27            rvb]
 * 
 * Revision 1.8.1.6  90/04/15  11:45:38  rvb
 * 	Must set iopl bitmapbase = sizeof (struct tss386), so it user
 * 	may not do pio.
 * 	[90/04/10            rvb]
 * 
 * Revision 1.8.1.5  90/03/28  08:30:50  rvb
 * 	v86 changes [grm]
 * 
 * Revision 1.8.1.4  90/03/07  13:26:53  rvb
 * 	Make U_ADDRESS on by default.
 * 	[90/03/01            rvb]
 * 
 * Revision 1.8.1.3  90/02/28  15:48:15  rvb
 * 	Revision 1.14  89/11/02  14:15:24  kupfer
 * 		Debugger support: add flavor for FP coprocessor state, and add
 * 		pcb_synch.
 * 
 * Revision 1.8.1.2  90/02/01  13:36:25  rvb
 *	Temporary vga hack
 *	[90/01/31            rvb]
 *   
 * Revision 1.8.1.1  89/11/21  12:45:27  rvb
 ...
 *	[88/01/25            rvb]
 *    
 * Revision 1.8  89/09/25  12:25:09  rvb
 * 	seg_desc -> fakedesc
 * 	[89/09/23            rvb]
 * 
 * Revision 1.7  89/09/20  17:26:39  rvb
 * 	Add a lot of stuff to set up the LDT.  Amazing anything worked
 * 	at all.  BUT some thought should be given to whether we want an
 * 	LDT and how it should be used.
 * 			Revision 1.11  89/09/14  13:09:08  kupfer
 * 			Initialize the LDT in kernel_pmap.  This makes a set of General
 * 			Protection faults in the kernel go away.
 * 			Revision 1.10  89/07/10  15:50:37  kupfer
 * 			Fold back in Carl's %esi & %edi fix (1.7).
 * 	[89/09/20            rvb]
 * 
 * Revision 1.9  89/07/07  17:55:22  kupfer
 * X79 merge, 2nd attempt.
 * 
 * Revision 1.5  89/04/05  12:59:02  rvb
 * 	Strangely no one set cr3 before the longjmp to "main" and it did
 * 	not complaign till now.
 * 	[89/03/30            rvb]
 * 
 * Revision 1.4  89/02/26  12:26:55  gm0w
 * 	Changes for cleanup.
 * 
 * 11-Jul-88  John Dugas (xxx) at Prime Computer Inc.
 * 11-Jul-88  Scott Nacey (xxx) at Prime Computer Inc.
 *	Adapted vax/pcb.c to the 80386.
 */
...
/*	pcb->t_esp0 = ksp;*/
	...
	pcb->t_es = KDSSEL;
	pcb->t_ss = KDSSEL;
	pcb->t_bitmapbase = (u_short) 0xFFFF;
	/*
	...
	unsigned int		count;
{
	...
	case i386_ISA_PORT_MAP_STATE:
		return(set_isa_port_map_state(thread, tstate, count));
		break;
	default:
 ...
 */
	...
	if (!(state->efl & EFL_VM)) {
		...
		if (!(saved_state->efl & EFL_VM)) {
			...
			/* eeeeck */
			thread->pcb->pcb_tss.t_esp0 += sizeof(struct i386_v86_info);
			thread->pcb->pcb_tss.t_bitmapbase = (u_short) 0xFFFF;
		}
	}
...
kern_return_t
set_isa_port_map_state(thread, tstate, count)
	thread_t		thread;
	thread_state_t		tstate;
	unsigned int		count;
{
	register struct tss386		*tss = &thread->pcb->pcb_tss;
	register u_char 		*pm = (u_char *) &tss->t_bitmap[0];

	if (count < i386_ISA_PORT_MAP_STATE_COUNT)
		return(KERN_INVALID_ARGUMENT);

	if (tss->t_bitmapbase != (u_short) &(((struct tss386 *) 0)->t_bitmap))
		tss->t_bitmapbase = (u_short) &(((struct tss386 *) 0)->t_bitmap);

	bcopy(((struct i386_isa_port_map_state *)tstate)->pm,
	      pm,
	      i386_ISA_PORT_MAP_STATE_COUNT * sizeof (int));
	...
	unsigned int		*count;		/* IN/OUT */
{
	...
	case i386_FLOAT_STATE:
		...
		break;
	case i386_ISA_PORT_MAP_STATE:
		return(get_isa_port_map_state(thread, tstate, count));
		break;
	default:
		return(KERN_INVALID_ARGUMENT);
	}
...
kern_return_t
get_isa_port_map_state(thread, tstate, count)
	register thread_t	thread;
	thread_state_t		tstate;		/* pointer to OUT array */
	unsigned int		*count;		/* IN/OUT */
{
	...
	if (tss->t_bitmapbase != (u_short) &(((struct tss386 *) 0)->t_bitmap)) {
		register int i = (((MAXTSSIOADDR+1)>>3)>>2);
		register u_long *pml = (u_long *)pm;

		tss->t_bitmapbase = (u_short) &(((struct tss386 *) 0)->t_bitmap);
		while (i-- > 0) *pml++ = 0xffffffff;
	}

	bcopy(pm,
	      ((struct i386_isa_port_map_state *)tstate)->pm,
	      i386_ISA_PORT_MAP_STATE_COUNT * sizeof (int));
