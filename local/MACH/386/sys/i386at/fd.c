 ...
 * HISTORY
 * $Log:	fd.c,v $
 * Revision 2.1.1.4  91/07/14  23:55:44  rvb
 * 	USER CONTRIBUTED comment as how better to initialize
 * 	the DMA registers.
 * 	[91/06/27            mg32]
 * 
 * Revision 2.1.1.3  91/03/28  08:45:32  rvb
 * 	Major improvements from mg32.
 * 	[91/03/25            rvb]
 * 
...
struct	isa_driver fddriver = {fdprobe, fdslave, fdattach, "fd", 0, 0, 0};
...
{
	int			spot = STSREG((int) ctlr->ctlr_addr);
	struct ctrl_info	*cip = &ctrl_info[ctlr->ctlr_ctlr];
	...
	if ((inb(spot) & DATAOK) == DATAOK) {
		...
		printf("fdc%d: port = %x, spl = %d, pic = %d.\n",
		       ctlr->ctlr_ctlr, ctlr->ctlr_addr, ctlr->ctlr_spl,
		       ctlr->ctlr_pic);
		return(1);
	} else
		return(0);
...
struct isa_dev		*dev;
{
	struct unit_info	*uip = &unit_info[dev->dev_unit];
	struct ctrl_info	*cip = &ctrl_info[dev->dev_ctlr];

	uip->dev = dev;
	dev->dev_addr = dev->dev_mi->ctlr_addr;
	uip->addr = dev->dev_addr;
	uip->b_cmd = &cip->b_cmd;
	uip->b_seekaddr = 0;
	uip->av_forw = 0;
	uip->wakeme = 0;
...
struct isa_dev *dev;
{
	printf("fd%d:  port = %x, spl = %d, pic = %d. (controller %d, slave %d)\n",
		dev->dev_unit, dev->dev_addr, dev->dev_spl, dev->dev_pic,
...
int	otyp;			/* not used */
{
	...
	int			slave = uip->dev->dev_slave;
	struct ctrl_info	*cip = &ctrl_info[uip->dev->dev_ctlr];
	...
	if (unit < MAXUNIT){
	  /* Since all functions that use this are called from open, we only
		...
		if(!error && !(uip->d_drtab.dr_type & OKTYPE)) {
			if ((MEDIATYPE(dev)<0) || (MEDIATYPE(dev)>3))
				goto endopen;
			...
			++cip->b_rwerr;
			wbp->b_dev = dev; wbp->b_proc = 0; wbp->b_error = 0;
			wbp->b_resid = 0; wbp->b_flags = (B_READ|B_VERIFY);
			wbp->b_bcount = 512;
...
long cmdarg;
{
	...
	cmdp->c_rbmtr &= ~(1<<(RBSHIFT+uip->dev->dev_slave));
...
struct unit_info *uip;
{
	...
	outd = ((uip->b_cmd->c_rbmtr&MTRMASK)<<MTR_ON)|uip->dev->dev_slave;
...
struct unit_info *uip;
{
	...
	while((rty_flg--)&&rtn){
		...
		cmdp->c_intr |= WUPFLAG;
		x = SPL();
		outb(DATAREG(uip->addr), uip->dev->dev_slave);
		rtn = ERROR;
		while(rtn) {
			uip->wakeme = 1;
			sleep(uip, PZERO);
...
register int	cylno;
{
	...
	if(rtn = fdc_sts(FD_OSTS, uip))			/* status check */
		return(rtn);
	outb(DATAREG(uip->addr), uip->dev->dev_slave);	/* drive number */
	if(rtn = fdc_sts(FD_OSTS, uip))			/* status check */
		return(rtn);
	x = SPL();
	/* Device to wake up specified in open */
...
#ifdef	MACH_KERNEL
		timeout(fdintr,uip->dev->dev_ctlr,cnt0);
#else	MACH_KERNEL
		cmdp->c_timeid = timeout(fdintr,uip->dev->dev_ctlr,cnt0);
#endif	MACH_KERNEL
	}
	return(rtn);
}
