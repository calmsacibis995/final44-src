	...
	io_buf_t	block;
{
	 ...
	 */
	if( vm_map_pageable(kern_device_io_map, block, block+block_size, FALSE) != KERN_SUCCESS)
	...
	int		* blockCnt;
{
	 ...
	 */
	if ( (err = vm_allocate(kern_device_io_map, &addr, block_size, TRUE)) != KERN_SUCCESS) {
		printf("block_read: cannot allocate a buffer!");
		return( err );
	}
	
	if( (err = vm_map_pageable(kern_device_io_map, addr, addr+block_size, FALSE)) != KERN_SUCCESS) {
	...
	*blockCnt = local_buf.b_bcount - local_buf.b_resid;
	 ...
	 */
	(void) vm_move(kern_device_io_map, addr, ipc_soft_map,
		       *blockCnt, TRUE, (vm_offset_t *) block);
	...
	daddr_t		ra_block_size;
{
	...
	if (vm_allocate(kern_device_io_map, &addr, block_size, TRUE) != KERN_SUCCESS)
		panic("block_read_ahead: cannot allocate a buffer!");
	if( vm_map_pageable(kern_device_io_map, addr, addr+block_size, FALSE) != KERN_SUCCESS)
...
#define LOCAL_PORT		device_enabled_set
