 ...
 *		LOCAL_PORT		A port set to receive on
 *
...
void		SERVER_LOOP()
{
	...
	if (port_set_add(my_self, LOCAL_PORT, my_notify) != KERN_SUCCESS) {
	...
	for (;;) {
		in_msg->msg_local_port = LOCAL_PORT;
		...
		if (in_msg->msg_local_port == my_notify) {
