 ...
 * HISTORY
 * $Log:	zalloc.c,v $
 * Revision 2.11.3.1  90/03/07  13:28:47  rvb
 * 		Print more info when a zone is exhausted. [kupfer]
 * 
	...
	register zone_t	zone;
{
	...
	while (addr == 0) {
		...
		if ((zone->cur_size + zone->elem_size) > zone->max_size) {
			if (zone->exhaustible)
				break;

			if (!zone_ignore_overflow) {
				printf("zone \"%s\" empty; %d elements in use.\n",
				       zone->zone_name, zone->count);
				panic("zalloc");
			}
		}
		unlock_zone(zone);
