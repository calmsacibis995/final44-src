 ...
 * HISTORY
 * $Log:	route.c,v $
 * Revision 2.5  89/07/14  15:29:21  rvb
 * 	Ultrix defines struct rtentry with one more pointer than bsd.
 * 	So fix routing IOCTLS to work with both.
 * 	[89/07/14            rvb]
 * 
 * Revision 2.4  89/03/09  20:39:53  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/25  18:54:45  gm0w
 * 	Updated copyright.
 * 	[89/02/12            gm0w]
 * 
 * Revision 2.2  89/01/18  00:52:59  jsb
 * 	NFS support: changes to rtredirect.
 * 	[89/01/17  10:39:22  jsb]
 * 
 */
/*
...
#ifdef	PMAX
			     &&
	    cmd != SIOCADDRT+0x40000 && cmd != SIOCDELRT+0x40000
#endif	PMAX
)
		return (EINVAL);
	if (!suser())
