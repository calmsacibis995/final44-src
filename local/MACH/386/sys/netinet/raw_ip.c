 ...
 * HISTORY
 * $Log:	raw_ip.c,v $
 * Revision 2.3.7.1  91/07/14  23:54:53  rvb
 * 	USER CONTRIBUTED: for route calculation
 * 	[91/06/27            mg32]
 * 
 * Revision 2.3  89/03/09  20:45:53  rpd
 * 	More cleanup.
 * 
 * Revision 2.2  89/02/25  19:00:55  gm0w
 * 	Updated copyright.
 * 	[89/02/12            gm0w]
 * 
...
#endif
	/*
	 * if the protocol is IPPROTO_RAW, the user handed us a complete
	 * IP packet.  Otherwise, allocate an mbuf for a header and fill it
	 * in as needed.
	 */
	if (proto != IPPROTO_RAW) {
	  /* Calculate data length and get an mbug for IP header */
	  int len = 0;
	  struct mbuf *m0;
