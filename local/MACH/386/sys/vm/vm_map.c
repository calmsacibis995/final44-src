...
#define RETURN(value)	{ result = value; goto BailOut; }
	...
	if (anywhere) {
		...
		*address = start;
		...
		if ((entry->next != &map->header) && (entry->next->start < end))
			RETURN(KERN_NO_SPACE);
	}
	...
	if ((map->first_free == entry) && (entry->end >= new_entry->start))
		map->first_free = new_entry;
	...
	register vm_map_entry_t	src_entry, dst_entry;
{
	...
	if (dst_entry->object.vm_object != VM_OBJECT_NULL &&
	    !dst_entry->object.vm_object->temporary)
	...
	if (src_entry->wired_count == 0) {
		 ...
		 *	the entry won't make copy-on-write pages writable in
		 *	vm_map_protect.  [It can't hurt to do this even if
		 *	it's not necessary.  It would probably be more clear
		 *	to just have vm_map_protect never use pmap_protect with
		 *	write access; faulting would fix things up. XXX ]
		 */
	...
	while (src_start < src_end) {
		...
		if (!src_entry->is_a_map && !dst_entry->is_a_map) {
			...
			if (src_entry->is_a_map) {
				...
				vm_map_lock_set_recursive(src_map);
			}
			...
			if (dst_entry->is_a_map) {
				...
				vm_map_lock_set_recursive(dst_map);
			}
			...
			if (dst_map == new_dst_map)
				vm_map_lock_clear_recursive(dst_map);
			if (src_map == new_src_map)
				vm_map_lock_clear_recursive(src_map);
		}
