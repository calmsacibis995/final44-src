 ...
 * Revision 2.22  92/02/04  19:29:24  mja
 * 	Fixed call to vm_map_copyout() in table() to provide 
 * 	discard arg.
 * 	[91/11/14            berman]
 * 
 * Revision 2.21  91/01/10  14:44:58  mja
 * 	Fix null pointer dereference bug in table_ttyloc();
 * 	[90/11/20  11:25:45  mja]
 * 
 * Revision 2.20  90/10/18  18:26:17  mrt
 * 	Fixed bug in table_fsparam loop
 * 	[90/10/18            mja]
 * 
		...
		case TBL_ARGUMENTS:
			...
			if(psdebug)
				printf("got %d at %x; copy...", copy_size,
					copy_start);
#endif
			if (vm_map_copyout(kernel_pageable_map, &copy_start, copy_result, TRUE)
			    != KERN_SUCCESS) {
#ifdef	PSDEBUG
				if(psdebug)
					printf("failed\n");
...
boolean_t table_ttyloc(index, data, size)
	...
	unsigned	*size;
{
	int majdev, mindev, *nttyp;
	extern int ntty;
	extern int *nttysw[];

	majdev = major(index);
	mindev = minor(index);
	if (majdev >= ntty || (nttyp=nttysw[majdev]) == 0 || mindev >= *nttyp)
		return(FALSE);
