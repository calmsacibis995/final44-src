 ...
 * HISTORY
 * $Log:	kern_clock.c,v $
 * Revision 2.21  91/04/03  12:54:19  mbj
 * 	Omron changes[sic]
 * 
...
#define tick	myticks
	...
	if (needsoft) {
...
#if	defined(sun3) || defined(sun4) || defined(luna)
		{
			int softclock();
#ifdef luna
	                softcall(softclock, USERMODE(ps) != 0, cpu_number());
#else  luna
	                softcall(softclock, USERMODE(ps) != 0);
#endif luna
		}
#else	defined(sun3) || defined(sun4) || defined(luna)
			setsoftclock();
#endif	defined(sun3) || defined(sun4) || defined(luna)
#endif	i386
#endif	defined(mac2)
	}
}
...
#if	!defined(sun3) && !defined(sun4) && !defined(mips) && !defined(luna)
/*ARGSUSED*/
softclock(pc, ps)
	caddr_t pc;
	int ps;
#endif	!defined(sun3) && !defined(sun4) && !defined(mips) && !defined(luna)
{
#ifdef	mips
	acksoftclock();
#endif	mips
