 ...
 * HISTORY
 * $Log:	kern_exit.c,v $
 * Revision 2.22  92/02/06  00:23:24  mja
 * 	Added hooks for file reference tracing.
 * 	[92/01/03            lily]
 * 
 * Revision 2.21  91/04/03  12:55:31  mbj
 * 	Omron changes[sic]
 * 
 * Revision 2.20  90/09/07  19:15:56  mrt
...
#if DFSTRACE > 0
			if (traceon)
				dfs_log(DFS_CLOSE, (short) i, (short) DFS_EXIT);
#endif DFSTRACE
			closef(f);
			u.u_ofile[i] = NULL;
		}
...
#if	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386) || defined(mac2) || defined(luna)
...
#endif	i386
#ifdef	luna88k
  	rup = (struct rusage_dev *)u.u_ar0[R4];
  	u.u_error = wait3(options=u.u_ar0[R3], &ru, -1);
#endif	luna88k
#endif	multimax
