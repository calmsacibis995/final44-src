 ...
 * HISTORY
 * $Log:	kern_fork.c,v $
 * Revision 2.13  92/02/06  00:23:31  mja
 * 	Added hooks for file reference tracing.
 * 	[92/01/03            lily]
 * 
 * Revision 2.12  91/04/03  12:55:51  mbj
 * 	Omron changes[sic]
 * 
	...
	int isvfork;
{
	...
	/*
	 * When a child process is forked while the parent is in single
	 * step mode, there will be one or two step breakpoints set in
	 * the child when it starts up.  It is probably not correct to
 	 * try to support this so we put the actual instructions back
	 * now.
	 */

        if (u.u_procp->p_flag & STRC) 	/* if process is traced */
         {

		if (u.u_next1.addr != DEST_NULL) /* do we have trap set here? */
	 	 {
			store_instruction
		      	 (
				u.u_procp,	/* pointer to proc structure */
				u.u_next1.addr,	/* address to store inst */
				u.u_next1.data	/* the instruction to restore */
		      	 );
			u.u_next1.addr = DEST_NULL; /* clear the entry */
	 	 }

		if (u.u_next2.addr != DEST_NULL) /* do we have trap set here? */
	 	 {
			store_instruction
		      	 (
				u.u_procp,	/* pointer to proc structure */
				u.u_next2.addr,	/* address to store inst */
				u.u_next2.data	/* the instruction to restore */
		      	 );
			u.u_next2.addr = DEST_NULL; /* clear the entry */
	 	 }
	 }
...
out:
#if DFSTRACE > 0
	if (traceon) dfs_log(DFS_FORK, p2);
#endif DFSTRACE
	u.u_r.r_val2 = 0;
