 ...
 * HISTORY
 * $Log:	kern_mman.c,v $
 * Revision 2.20  92/02/04  19:29:52  mja
 * 	Changed semantics of smmap() to be consistent with BSD; only
 * 	allow a start address that is page aligned.  Previously we munged
 * 	the user start address silently to be page aligned.
 * 	Fixed typo (closed comment) in smmap().
 * 	[91/12/15            berman]
 * 
 * Revision 2.19  91/05/07  09:47:38  rvb
 * 	From omron:  getpagesize has been a mistake we fixed it
 * 	a bit.  I am afraid to change it to be correct for the vax.
 * 	[91/05/07            rvb]
 * 
...
smmap()
{
	...
	/*
	 *	We bend a little - round the size to the nearest 
	 *	page boundary.  Don't accept start addr that isn't aligned.
	 */
