 ...
 * HISTORY
 * $Log:	sys_generic.c,v $
 * Revision 2.11  92/02/11  17:46:22  mja
 * 	     For luna, correct certain treatments of errno to be in line
 * 	     with documentation.
 * 	[92/01/31            jfriedl]
 * 
 * Revision 2.10  92/02/06  00:23:47  mja
...
#if DFSTRACE > 0
	if (traceon) 
		dfs_log((rw == UIO_READ)?DFS_READ:DFS_WRITE, fp, 
			(short) ((struct a *)u.u_ap)->fdes);
#endif DFSTRACE
...
#define	STK_PARAMS	128
	...
	if (size > IOCPARM_MAX) {
#if defined(luna)
		u.u_error = EINVAL;	/* ioctl shuldn't return EFAULT */
#else
		u.u_error = EFAULT;
#endif
		return;
