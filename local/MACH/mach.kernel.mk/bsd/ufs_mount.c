 ...
 * HISTORY
 * $Log:	ufs_mount.c,v $
 * Revision 2.15  92/02/08  00:03:19  mja
 * 	Correct fs_clean typo under SMART_FSCK/PMAX.
 * 
 * Revision 2.14  91/01/23  13:27:43  berman
 * 	Turned on SMART_FSCK code for clean file systems.
 * 	Mount dirty file systems read-only and use uprintf().
 * 	[91/01/11            berman]
 * 
...
found:
	...
	if (fs->fs_clean != FS_CLEAN && fs->fs_clean != 1 && !is_root)
	    uprintf("ufs_mount: file system not clean, mounting read only.\n");
	is_root = 0;
	ronly = 1;
#endif	defined(SMART_FSCK) || defined(PMAX)
