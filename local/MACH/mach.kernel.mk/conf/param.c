 ...
 * HISTORY
 * $Log:	param.c,v $
 * Revision 2.13  91/08/30  09:38:03  berman
 * 	Change forumation of NPROC and ncallout to be influence by the
 * 	number of processors. For single processors, the numbers work out
 * 	the same as the old definitions. For multiprocesors which can
 * 	easily have larger call out lists and more aggressive users 
 * 	creating more processes, these changes are highly advisible.
 * 	[91/07/30  12:33:31  ern]
 * 
...
/* the default is 64 for num of user processes but people tend to max
 * out multiprocessors intentionally SO factor that in to NPROC
 */
#define NPROC (16 + (4 * NCPUS) + (8 * MAXUSERS))
int	nproc = NPROC;
#define NINODE (((NPROC + 16 + MAXUSERS) + 32)*2)
int	ninode = NINODE;
int	nchsize = NINODE * 11 / 10;
int	nfile = 16 * (NPROC + 16 + MAXUSERS) / 10 + 32;
int	ncallout = (16*NCPUS) + NPROC;
