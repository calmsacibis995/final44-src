 ...
 * HISTORY
 * $Log:	main.c,v $
 * Revision 2.4  89/03/09  20:10:39  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/25  17:59:43  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  89/01/23  22:18:52  af
 * 	Changes for I386: define bcmp
 * 	[89/01/09            rvb]
 * 
...
#else	defined(CMU) && defined(ns32000)
		if (*cp == '\0')
#endif	defined(CMU) && defined(ns32000)
			return (argc);
		/*
		 * copy argument
		 */
	...
	int *size;
{
	...
	while (*cp1 && *cp1 != '\n' && *cp1 != COMMENTCHAR)
		hash += (int)*cp1++;
	*size = cp1 - cp;
#else	defined(CMU) && defined(balance)
	while (*cp1 && *cp1 != '\n')
		hash += (int)*cp1++;
	*size = cp1 - cp + 1;
#endif	defined(CMU) && defined(balance)
	hash &= HSHSIZ - 1;
	return (hash);
}
