 ...
 * Revision 2.5  91/05/24  15:40:33  mbj
 * 	Support emulation of negative system calls.
 * 	Made the emulation dispatch vector zone exhaustible.  Now
 * 	task_set_emulation() returns KERN_RESOURCE_SHORTAGE instead of
 * 	panicing on zone overflow.
 * 	[91/05/24            mbj]
 * 
...
{
	extern int nsysent;
	vm_size_t dispatch_size;
	
	eml_max_emulate_count = nsysent - EML_MIN_SYSCALL;
	dispatch_size = sizeof(struct eml_dispatch) +
			(eml_max_emulate_count-1) * sizeof(eml_routine_t);

	eml_zone = zinit(
		dispatch_size,
		(vm_size_t) (eml_dispatch_max_num * dispatch_size),
		PAGE_SIZE, FALSE, "emulation routines" );
	zchange(eml_zone, FALSE, FALSE, TRUE);	/* make it exhaustible */
}
	...
	int		routine_number;
{
	...
	if ( routine_number >= eml_max_emulate_count + EML_MIN_SYSCALL
 ...
 */
	...
	task->eml_dispatch->disp_vector[routine_number - EML_MIN_SYSCALL] =
		routine_entry_pt;
