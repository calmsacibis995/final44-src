 ...
 * HISTORY
 * $Log:	task.c,v $
 * Revision 2.16  90/10/18  12:54:30  mrt
 * 	17-Oct-90 David Black (dlb) at Open Software Foundation
 * 	     Add thread_block() to loop that forcibly terminates threads in
 * 	     task_terminate() to fix livelock.  Also hold reference to thread
 * 	     when calling thread_force_terminate().
 * 	[90/10/18  12:38:34  mrt]
 * 
 * Revision 2.15  90/09/10  21:10:56  mrt
	...
	unsigned int		*task_info_count;	/* IN/OUT */
{
	...
	switch (flavor) {
		...
		*task_info_count = TASK_BASIC_INFO_COUNT;
		...
		events_info->faults = map->vm_stat.faults;
		events_info->zero_fills = map->vm_stat.zero_fill_count;
		events_info->reactivations = map->vm_stat.reactivations;
		events_info->pageins = map->vm_stat.pageins;
		events_info->cow_faults = map->vm_stat.cow_faults;
		{
			register int		msgin, msgout;
			...
			thread = (thread_t) queue_first(list);
			while (!queue_end(list, (queue_entry_t) thread)) {
				msgout += thread->messages_sent;
				msgin += thread->messages_received;
				thread = (thread_t) queue_next(&thread->thread_list);
			}
			task_unlock(task);
			events_info->messages_sent = msgout;
			events_info->messages_received = msgin;
		}
