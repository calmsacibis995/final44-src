 ...
 * Revision 2.15  89/10/04  12:42:01  rpd
 * 	Fixed sync_vp declaration; static got dropped somewhere.
 * 
 * Revision 2.14  89/10/03  19:26:53  rpd
 * 	Merged with my version from NeXT.  Summary:
 * 	Added nfs_nlinks.  Fixed EINVAL check in rwvp.
 * 	Removed Rick's NBC hacks in nfs_getattr and sync_vp.
 * 	Fixed truncation in nfs_create and nfs_setattr.
 * 	Removed dlb's hack to avoid panic in do_bio.
 * 	New version of nfs_page_read which correctly handles
 * 	reads past EOF (returning EINVAL).
 * 	[89/10/02  16:01:30  rpd]
 * 
 * Revision 2.13  89/08/09  09:13:41  jsb
 * 	Workaround to avoid panic in do_bio when a dirty rnode is
 * 	truncated by an open operation.
 * 	[89/08/02            dlb]
 * 
 * Revision 2.12  89/08/02  08:05:02  jsb
 * 	Eliminated MACH conditionals. Replace kallocs with zallocs.
 * 	[89/07/31  16:27:24  jsb]
 * 
 * Revision 2.11  89/06/30  22:33:43  rpd
 * 	Fixed the ISVDEV macro.
 * 	[89/06/30  22:15:49  rpd]
 * 
 * Revision 2.10  89/06/12  14:52:27  jsb
 * 	Added new nfs_page_write routine. Check for deadlock before calling
 * 	mfs_fsync in sync_vp.
 * 	[89/06/12  11:40:06  jsb]
 * 
 * Revision 2.9  89/06/03  15:37:05  jsb
 * 	To save stack space, dynamically allocate nfsattrstat structures
 * 	instead of declaring them as local variables.
 * 	[89/06/02  14:57:33  jsb]
 * 
 * 	Have nfs_page_read print and return the correct error code.
 * 	[89/05/26  23:03:09  jsb]
 * 
 * Revision 2.8  89/05/11  15:40:16  gm0w
 * 	Minor fixes for NBC code from rfr.
 * 	[89/05/11            gm0w]
 * 
 * Revision 2.7  89/05/11  14:42:01  gm0w
 * 	Added vn_read1dir vnode op entry.
 * 	[89/05/11            gm0w]
 * 
 * Revision 2.6  89/04/22  15:26:08  gm0w
 * 	Changed to use new inode macros for manipulation of inodes.
 * 	Removed FIFO code.  Picked up new NBC code from rfr.
 * 	[89/04/14            gm0w]
 * 
 * Revision 2.5  89/03/09  20:54:40  rpd
 * 	More cleanup.
 * 
 * Revision 2.4  89/02/25  19:09:52  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.3  89/01/23  15:27:00  rvb
 * 	Forward declarations for GCC.
 * 	[89/01/22            rvb]
 * 
 * Revision 2.2  89/01/18  00:57:00  jsb
 * 	If EREMOTE not defined, define it as ENFSREMOTE;
 * 	deleted wierd #ifdef vax code...
 * 	[89/01/17  14:09:37  jsb]
 * 
 *
...
/* @(#)nfs_vnodeops.c	1.5 87/09/11 3.2/4.3NFSSRC */
	...
	struct ucred *cred;
{
	...
	if (rw == UIO_WRITE && (vp->v_mode&VFMT) == VREG &&
...
#endif
	ZALLOC(nfs_attrstat_zone, ns, struct nfsattrstat *);
	do {
	...
	enum staleflush fflag;
{
	 ...
	 */
	delta = (rp->r_nfsattrtime.tv_sec - na->na_mtime.tv_sec) >> 4;
	if ((vp->v_mode&VFMT) == VDIR) {
	...
	struct ucred *cred;
{
	...
	rp = vtor(vp);
	s = splclock();
	i = timercmp(&time, &rp->r_nfsattrtime, <);
	splx(s);
	if (i) {
		/*
...
#if	MACH_NBC
		if ((vap->va_size < vp->v_vm_info->inode_size) &&
			  (vp->v_vm_info->dirty || (rp->r_flags & RDIRTY)))
			vap->va_size = vp->v_vm_info->inode_size;
#endif	MACH_NBC
	...
	struct ucred *cred;
{
	...
	if ((vap->va_nlink != -1) || (vap->va_blocksize != -1) ||
		...
		if (vap->va_size != -1) {
			...
			vp->v_vm_info->inode_size = vap->va_size;
#endif	MACH_NBC
	...
	struct ucred *cred;
{
	...
	if (cred->cr_uid != va.va_uid) {
		...
		if (cred->cr_gid == va.va_gid)
			goto found;
		gp = (gid_t *) cred->cr_groups;
		for (; gp < (gid_t *) &cred->cr_groups[NGROUPS] && *gp != NOGROUP; gp++)
			if (va.va_gid == *gp)
				goto found;
		mode >>= 3;
	}
	...
	struct ucred *cred;
{
	...
	if (rp->r_unldvp != NULL) {
		...
		VN_RELE(rp->r_unldvp);
		rp->r_unldvp = NULL;
		ZFREE(nfs_name_zone, rp->r_unlname);
		rp->r_unlname = NULL;
		crfree(rp->r_unlcred);
		rp->r_unlcred = NULL;
	}
	...
	struct ucred *cred;
{
	...
	*vpp = (struct vnode *) dnlc_lookup(dvp, nm, cred);
	if (*vpp) {
		...
		ZALLOC(nfs_diropres_zone, dr, struct nfsdiropres *);
		...
		ZFREE(nfs_diropres_zone, dr);
	}
	/*
	 * If vnode is a device create special vnode
	 */
	if (!error && ISVDEV(((*vpp)->v_mode)&VFMT)) {
		struct vnode *newvp;
	...
	struct ucred *cred;
{
	...
	*vpp = (struct vnode *)0;
	 ...
	 */
	if ((va->va_mode&IFMT) == VCHR || (va->va_mode&IFMT) == VBLK) {
	...
	if (!error) {
		...
		if (!error) {
			*vpp = makenfsnode(
			...
			if (va->va_size == 0) {
				/* this isn't size but rather type == VREG */
#if	MACH_NBC
				mfs_trunc(*vpp, 0);
#endif	MACH_NBC
				sync_vp(*vpp);
	...
	struct ucred *cred;
{
	...
	bzero(rd.rd_entries, (u_int)count);
	...
	register struct buf *bp;
{
	...
	if (bp->b_flags & B_INVAL)
		panic("nfs_strategy: invalid buffer");
...
int	rnode_read_aheads = 0;
int	rnode_read_individuals = 0;

nfs_page_read(vp, buffer, size, offset, cred)
	struct vnode *vp;
	caddr_t buffer;
	int size;
	vm_offset_t offset;
	struct ucred *cred;
{
	...
	/*
	 *	Note that if we weren't given any
	 *	credentials, we'll fall back on using
	 *	whatever the rnode already contains.
	 */
	...
	do {
		...
		struct vnode *mvp; /* "mapped" vnode with the data */
		daddr_t mbn;	/* "mapped" block where our data lies */
		...
		VOP_BMAP(vp, lbn, &mvp, &mbn);

		/*
		 *	The existing nfs_bmap won't return a negative
		 *	block number to indicate a hole.  However,
		 *	I don't know if the spec allows for it.
		 */
		...
		if ((long)mbn < 0) {
			 ...
			 *	read-ahead of the next logical block.
			 */

			if ((rp->r_lastr + 1) == lbn) {
				daddr_t mrabn;	/* "mapped" read-ahead block */

				rnode_read_aheads++;
				VOP_BMAP(vp, lbn+1, NULL, &mrabn);
				bp = breada(mvp, mbn, bsize, mrabn, bsize);
			} else {
				rnode_read_individuals++;
				bp = bread(mvp, mbn, bsize);
			}
			rp->r_lastr = lbn;
...
nfs_page_write(vp, buffer, size, offset, cred, init)
	register struct vnode	*vp;
	caddr_t		buffer;
	int		size;
	vm_offset_t	offset;
	...
	boolean_t	init;
{
	...
	if (init)
		panic("nfs_page_write: called from data_initialize");
	...
	uiop.uio_resid = size;
	error = nfs_rdwr(vp, &uiop, UIO_WRITE, 0, cred);
...
zone_t nfs_attrstat_zone;
zone_t nfs_diropres_zone;
zone_t nfs_mntinfo_zone;
zone_t nfs_name_zone;
zone_t nfs_path_zone;
zone_t nfs_rnode_zone;

nfs_zone_init()
{
	nfs_attrstat_zone	= zinit(sizeof(struct nfsattrstat), 1024*1024,
					PAGE_SIZE, FALSE, "nfs attrstat");

	nfs_diropres_zone	= zinit(sizeof(struct nfsdiropres), 1024*1024,
					PAGE_SIZE, FALSE, "nfs diropres");

	nfs_mntinfo_zone	= zinit(sizeof(struct mntinfo), 1024*1024,
					PAGE_SIZE, FALSE, "nfs mntinfo");

	nfs_name_zone		= zinit(NFS_MAXNAMLEN, 1024*1024,
					PAGE_SIZE, FALSE, "nfs name");

	nfs_path_zone		= zinit(NFS_MAXPATHLEN, 1024*1024,
					PAGE_SIZE, FALSE, "nfs path");

	nfs_rnode_zone		= zinit(sizeof(struct rnode), 1024*1024,
					PAGE_SIZE, FALSE, "nfs rnode");
}
