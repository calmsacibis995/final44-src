 ...
 * HISTORY
 * $Log:	nfs_xdr.c,v $
 * Revision 2.8  89/08/02  08:05:22  jsb
 * 	Eliminated MACH conditionals.
 * 	[89/07/31  16:28:49  jsb]
 * 
 * Revision 2.7  89/06/03  15:37:17  jsb
 * 	Fixes to xdr_rrok: release resources on ALL error paths; corrected
 * 	bogus comments; let server retry send with xdr_bytes if
 * 	xdrmbuf_putbytes appears to lose.
 * 	[89/06/02  14:01:59  jsb]
 * 
 * 	Remove gateway support from xdr_putrddirres; under MACH_AFS,
 * 	replace it with a stub that calls the afs/nfs_gateway.c version.
 * 	[89/05/26  23:06:33  jsb]
 * 
 * Revision 2.6  89/05/01  18:02:02  rpd
 * 	Changed xdr_{get,put}rddirres for afs gateway support.
 * 	[89/05/01  13:50:33  jsb]
 * 
 * Revision 2.5  89/04/22  15:26:21  gm0w
 * 	Changed to use new inode macros for manipulation of inodes.
 * 	[89/04/14            gm0w]
 * 
 * Revision 2.4  89/03/09  20:54:52  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/25  19:10:02  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  89/01/18  00:57:23  jsb
 * 	Include file references.
 * 	[89/01/17  14:13:22  jsb]
 * 
 */
/* @(#)nfs_xdr.c	1.5 87/09/11 3.2/4.3NFSSRC */
	...
	if (xdr_fattr(xdrs, &rrok->rrok_attr)) {
		if (xdrs->x_op == XDR_ENCODE && rrok->rrok_bp) {
			...
			if (xdrmbuf_putbuf(xdrs, rrok->rrok_data,
				...
				/* before giving up, try the 'client' way */
				int rv = xdr_bytes(xdrs, &rrok->rrok_data,
				    (u_int *)&rrok->rrok_count, NFS_MAXDATA);
				rip->done = 1;
				rrokfree(rip);
				return rv;
			}
		} else { /* client side, or server that didn't use bread */
...
{
#if	MACH_AFS
	return (bool_t) afs_xdr_putrddirres(xdrs, rd);
#else	MACH_AFS
	struct direct *dp;
	char *name;
	int size;
