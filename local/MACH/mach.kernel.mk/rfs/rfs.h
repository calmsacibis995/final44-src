 ...
 * HISTORY
 * $Log:	rfs.h,v $
 * Revision 2.13  91/11/05  18:27:41  mja
 * 	Add field to rfsWho header to record the error code from the
 * 	bad ticket file fetch (so that it can also be returned on
 * 	subsequent calls from other clients which may not have caused
 * 	the initial bad fetch);  add additional flags to record expired
 * 	tickets and non-existent ticket files.
 * 	[91/11/05  00:24:40  mja]
 * 
 * Revision 2.12  91/08/05  14:32:30  berman
 * 	Changed definitions of MAXTKTFILELEN and struct rfsWho to always
 * 	fit inside an MBUF (MLEN).  This prevents allocating clusters by
 * 	mistake.
 * 	[91/08/05            berman]
 * 
 * Revision 2.11  91/08/01  15:25:26  berman
 * 	Added a constant for the max length of a ticket file name.
 * 	[91/07/31  23:36:10  chs]
 * 
 * 	Added support for kerberos authentication (RFS protocol is now
 * 	version 3).  Removed support for IDENTIFY, which didn't work anyway.
 * 	Removed GETS and SETS ioctl defines, which no longer exist anyway.
 * 	Added 2 new ioctls, NEWTKT and CACHESOCK.
 * 	Added a define for the path to the RFS control file.
 * 	Added include of <sys/param.h> so everyone who includes this
 * 	file doesn't have to.
 * 	[91/07/22  11:45:25  chs]
 * 
...
#define RFSVERSION	3		/* protocol version #3 */
...
#define rmsg_C_auth	rmsg_header.rmsg_h_byte[4]
#define rmsg_C_cid	rmsg_body.rmsg_b_cid

/* for old-style auth compatibility - flush these eventually */
...
enum rfsRW
{
    ...
    RFSRW_TKTFILE,			/* path to ticket file */
};
...
#define RFSIOCNEWTKT	_IO (R, 2)
#define RFSIOCCACHESOCK	_IO (R, 3)
...
#define RFSCONTROL_PATH "/../.CONTROL"
...
#define RFSWHO_BADTKTFILE	0x1
#define RFSWHO_EXPTKTFILE	0x2
#define RFSWHO_NOTKTFILE	0x4

/* A header for struct rfsWho used to make MAXTKTFILELEN fit inside an MBUF */
struct rfsWhoHead {
    u_short	rwh_refc;		/* reference count */
    u_short	rwh_ltktfile;		/* length of path to ticket file */
    u_short	rwh_flags;		/* info about the ticketfile */
    u_short	rwh_badcode;		/* error code from bad ticket fetch */
};

#define MAXTKTFILELEN		(MLEN - sizeof(struct rfsWhoHead))

struct rfsWho
{
    struct rfsWhoHead	h;		/* header structure */
    char rw_tktfile[MAXTKTFILELEN];	/* the path itself */
};

/* Macros to make the imbedded header structure appear part of struct rfsWho */
#define rw_refc		h.rwh_refc
#define rw_ltktfile	h.rwh_ltktfile
#define rw_flags	h.rwh_flags
#define rw_badcode	h.rwh_badcode
