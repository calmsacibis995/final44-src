 ...
 * Revision 2.19  91/01/24  16:07:21  berman
 * 	Pass pointer to struct nameidata in rfs_namescall().
 * 	[91/01/24            berman]
 * 
 * Revision 2.18  91/01/24  11:34:42  berman
 * 	Changed name of nameidata struct to nd to avoid name clash in
 * 	non-vfs configuration.
 * 	[91/01/24            berman]
 * 
 * Revision 2.17  91/01/23  13:30:36  berman
 * 	Added a struct nameidata to rfs_namescall() to preserve the
 * 	first name during the second lookup.
 * 	[91/01/10            berman]
 * 
    ...
    struct nameidata *nip;
{
    ...
    if (error = rfsSend(rcbp, &rmsg, sizeof(rmsg.rmsg_header)))
	goto fail;
    if (error = rfsSendString(rcbp, nd.ni_dirp, rmsg.rmsg_P_len1))
	goto fail;
