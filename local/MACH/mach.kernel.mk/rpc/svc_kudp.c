 ...
 * HISTORY
 * $Log:	svc_kudp.c,v $
 * Revision 2.6  89/08/02  08:07:01  jsb
 * 	Eliminated MACH conditionals. Replaced kallocs with zallocs.
 * 	[89/07/31  16:59:47  jsb]
 * 
 * Revision 2.5  89/03/09  20:59:53  rpd
 * 	More cleanup.
 * 
 * Revision 2.4  89/02/25  19:15:46  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.3  89/01/23  15:27:44  rvb
 * 	Forward declarations for GCC.
 * 	[89/01/22            rvb]
 * 
 * Revision 2.2  89/01/18  01:04:59  jsb
 * 	Include mach.h; use udpmsgbuf_{alloc,free} (see clnt_kudp.c).
 * 	[89/01/17  15:38:45  jsb]
 * 
 *
...
zone_t rpc_dupreq_zone;
zone_t rpc_udpdata_zone;
...
#endif
	ZALLOC(rpc_svcxprt_zone, xprt, SVCXPRT *);
	rpc_buffer(xprt) = (caddr_t)udpmsgbuf_alloc();
	ZALLOC(rpc_udpdata_zone, ud, struct udp_data *);
	...
	register SVCXPRT   *xprt;
{
	...
	ZFREE(rpc_udpdata_zone, ud);
	udpmsgbuf_free(rpc_buffer(xprt));
	ZFREE(rpc_svcxprt_zone, xprt);
}
	...
	register struct svc_req *req;
{
	...
	if (ndupreqs < MAXDUPREQS) {
		ZALLOC(rpc_dupreq_zone, dr, struct dupreq *);
		if (drmru) {
...
svc_kudp_zone_init()
{
	rpc_dupreq_zone		= zinit(sizeof(struct dupreq),
					1024*1024, 0, FALSE, "rpc dupreq");

	rpc_udpdata_zone	= zinit(sizeof(struct udp_data),
					1024*1024, 0, FALSE, "rpc udpdata");
}
