...
/*
 * HISTORY
 * $Log:	fifo_vnodeops.c,v $
 * Revision 2.7  89/10/03  19:27:17  rpd
 * 	Added fifo_badop to vnodeops, for new VOP_NLINKS.
 * 	[89/10/02  16:03:09  rpd]
 * 
 * Revision 2.6  89/08/02  08:09:27  jsb
 * 	Eliminated MACH conditionals. Added freefid (and other missing ops).
 * 	Use zalloc in place of kalloc.
 * 	[89/07/31  16:39:33  jsb]
 * 
 * Revision 2.5  89/04/22  15:30:14  gm0w
 * 	Added FIFOS conditional around module.
 * 	[89/04/14            gm0w]
 * 
 * Revision 2.4  89/03/09  21:01:12  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/25  19:16:52  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  89/01/18  01:08:10  jsb
 * 	Include file references.
 * 	[89/01/17  15:05:22  jsb]
 *
...
/* @(#)fifo_vnodeops.c	1.3 87/06/30 3.2/4.3NFSSRC */
...
#endif	SUN_LOCK
	spec_fid,
	fifo_badop,	/* page_read */
	fifo_badop,	/* page_init */
	fifo_badop,	/* read1dir */
	spec_freefid,
	fifo_badop,	/* nlinks */
};
	...
	struct ucred *cred;
{
	...
	case FREAD:
		if (fp->fn_size != 0)		/* anything to read? */
			return (1);
		if (fp->fn_rsel &&
		    fp->fn_rsel->thread->wait_event == (int)&selwait)
			fp->fn_flag |= FIFO_RCOLL;
		else
	...
	case FWRITE:
		...
		if ((fp->fn_size < PIPE_BUF) && (fp->fn_rcnt > 0))
			return (1);
		if (fp->fn_wsel &&
		    fp->fn_wsel->thread->wait_event == (int)&selwait)
			fp->fn_flag |= FIFO_WCOLL;
		else
	...
	struct ucred *cred;
{
	...
	sunsave(VTOS(vp));
	zfree (vm_info_zone, vp->vm_info);
	ZFREE(specfs_fnode_zone, VTOF(vp));
	return (0);
}
	...
	struct vnode *vp;
{
	...
	ZALLOC(specfs_fnode_zone, fp, struct fifonode *);
	...
	register struct fifonode *fp;
{
	...
	/* the call to kalloc() might sleep, so leave fifonode locked */

	fifo_alloc += FIFO_BUFFER_SIZE;
	bp = (struct fifo_bufhdr *)kalloc((u_int)FIFO_BUFFER_SIZE);
	fp->fn_nbuf++;
	...
	struct fifonode *fp;
{
	...
	kfree((caddr_t)bp, (u_int)FIFO_BUFFER_SIZE);
...
{
	specfs_fifonode_zone	= zinit(sizeof(struct fifonode), 1024*1024,
					0, FALSE, "specfs fifonode");
}
