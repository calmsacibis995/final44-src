 ...
 * HISTORY
 * $Log:	mkmakefile.c,v $
 * Revision 2.27  92/02/15  15:49:50  mja
 * 	Switch PCOPTS to standard COPTS_P;  also use standard S_RULE_*
 * 	structure for MIPS assembly.
 * 	[92/02/15  15:09:43  mja]
 * 
 * Revision 2.26  92/02/11  17:59:30  mja
 * 	     Additional support for the luna makefile. Support for
 * 	the luna miniroot.
 * 	[92/01/31            jfriedl]
 * 
 * Revision 2.25  91/09/24  20:08:23  mja
 * 	Insert ${MIG_ALL_FILES} macro in the vmunix dependency list in
 * 	place of the old dependencies on sometimes missing targets to
 * 	eliminate unneccesary re-links.
 * 	[91/09/22  03:40:39  mja]
 * 
 * Revision 2.24  91/04/03  13:14:14  mbj
 * 	Omron changes[sic]
 * 
 * Revision 2.23  91/04/02  12:17:55  mbj
 * 	Cast all sprintf's (void).
 * 	Allow for profiling I386.
 * 	[90/01/22            rvb]
 * 
 * Revision 2.22  90/12/17  17:16:13  berman
 * 	Added the SUN4C machine type field to the users struct.
 * 	[90/05/23            berman]
 * 
...
static	struct users {
	...
	{ 8,  2, 32 },			/* MACHINE_MAC2 */
	{ 8, 2, 32 },			/* MACHINE_SUN4C */
	{ 16, 8, 1024},                 /* MACHINE_LUNA88K */
};
...
#ifdef luna88k
		/*
		 * skip MACH_* symbols and mig* symbols in 
		 *      Makefile.template.
		 */
		if (kernobj && strncmp(line, "MACH_", 5) == 0)
			while (fgets(line, BUFSIZ, ifp) != 0)
				if (*line == '\n')
					break;
		if (kernobj && strncmp(line, "$(MACH_", 7) == 0)
			while (fgets(line, BUFSIZ, ifp) != 0)
				if (*line == '\n')
					break;
		if (kernobj && strncmp(line, "mig", 3) == 0)
			while (fgets(line, BUFSIZ, ifp) != 0)
				if (*line == '\n')
					break;
#endif luna88k
...
#ifdef luna88k
		if(strncmp("locore", sp, 6) != 0 &&  strcmp("luna_fp.o", sp) != 0 && 
		   strcmp("eh.o", sp) != 0 && strcmp("debug_msg.o", sp) != 0 && 
		   strcmp("vsnprintf.o", sp) != 0)
#endif
		fprintf(fp, "%s ", sp);
		lpos += len + 1;
		*cp = och;
	...
	FILE *ofp;
{
	...
	while (fgets(line, BUFSIZ, ifp) != 0) {
#ifdef luna88k
	    if (kernobj && strncmp(line, "luna_fp.o", 9) == 0) {
		fprintf(ofp, "luna_fp.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/luna_fp.o luna_fp.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "locore.o", 8) == 0) {
		fprintf(ofp, "locore.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/locore.o locore.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "locore_asm_routines.o", 21) == 0) {
		fprintf(ofp, "locore_asm_routines.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/locore_asm_routines.o locore_asm_routines.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "eh.o", 3) == 0) {
		fprintf(ofp, "eh.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/eh.o eh.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "locore_c_routines.o", 19) == 0) {
		fprintf(ofp, "locore_c_routines.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/locore_c_routines.o locore_c_routines.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "debug_msg.o", 11) == 0) {
		fprintf(ofp, "debug_msg.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/debug_msg.o debug_msg.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	    if (kernobj && strncmp(line, "vsnprintf.o", 11) == 0) {
		fprintf(ofp, "vsnprintf.o:\n");
		fprintf(ofp, "\t-ln -s ../OBJ88/vsnprintf.o vsnprintf.o\n");
		while (fgets(line, BUFSIZ, ifp) != 0)
		    if (*line == '\n')
			break;
	    }
	...
	*cp = '\0';
	...
	if (och == 's') {
		...
		case MACHINE_MIPS:
			...
			case NORMAL:
			case DRIVER:
				fprintf(f, "\t${S_RULE_1A}%s %s%s%.*s${S_RULE_1B}%s\n",
				    (ftp->f_extra?ftp->f_extra:""), extras, 
			...
			case PROFILING:
				if (!profiling)
					continue;
				fprintf(f, "\t${S_RULE_1A_P}%s %s%s%.*s${S_RULE_1B_P}%s\n",
				    (ftp->f_extra?ftp->f_extra:""), extras, 
						source_dir, tp-np, np, nl);
				fprintf(f, "\t${S_RULE_2_P}%s\n", nl);
				fprintf(f, "\t${S_RULE_3_P}\n\n");
				break;
...
#ifdef	multimax
	fprintf(f, "%s .ORDER: ${MIG_ALL_FILES} %s.sys ${SYSDEPS}\n",
		fl->f_needs, fl->f_needs);
#else	multimax
	fprintf(f, "%s: ${MIG_ALL_FILES} %s.sys ${SYSDEPS}\n",
		fl->f_needs, fl->f_needs);
#endif	multimax
