 ...
 * HISTORY
 * $Log:	heap_kmem.c,v $
 * Revision 2.2  89/07/11  13:55:14  jjc
 * 	Moved from sun3/ to sun/ since it is used by both the Sun 3 and 4.
 * 	[89/07/06  17:22:05  jjc]
 * 
 * Revision 2.5  89/03/09  21:33:17  rpd
 * 	More cleanup.
 * 
 * Revision 2.4  89/02/25  19:34:25  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.3  88/10/18  00:30:21  mwyoung
 * 	Forward declarations of static's to make gcc happy:
 * 		static check_need_to_free();
 * 	[88/10/14            rvb]
 * 
 * 17-May-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Changed sun_kmem_alloc() to zero the memory that it returns.
 *
 * 16-Feb-88  David Golub (dbg) at Carnegie-Mellon University
 *	Converted to new include file names and conditionals.
 *
 * 26-Jan-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Grabbed from SunOS 3.4 and modified for Mach.
 *
 */
...
#ifdef	DEBUG
printf("sun_kmem_init returning\n");
prtree(kmem_info.free_root, "sun_kmem_init");
#endif
}
 ...
 *
 * function result:
 *	sun_kmem_alloc returns a pointer to the allocated block; a null
	...
	register u_int	nbytes;
{
	...
	int s;
	int numbytes = nbytes;		/* save no. of bytes to allocate */
	...
	if (nbytes < SMALLEST_BLK) {
		printf("illegal sun_kmem_alloc call for %d bytes\n", nbytes);
		panic("sun_kmem_alloc");
	}
	check_need_to_free();
	...
	if (mweight(kmem_info.free_root) <= nbytes) {
		 ...
		 */
		if (!morecore(nbytes)) {
			printf("sun_kmem_alloc failed, nbytes %d\n", nbytes);
			panic("sun_kmem_alloc");
		}
	}
		...
		if ((int) a->block->data & ALIGNMASK &&
			...
			*/
			...
			if (a->size > nbytes) {
				...
				*/
				sun_kmem_free((caddr_t)nextblk(a->block, nbytes),
				    (u_int)a->size - nbytes);
			}
			freehdr(a);
			nbytes -= ALIGNMORE(a->block->data);
...
#ifdef	DEBUG
printf("sun_kmem_alloc returning %x\n", retblock);
prtree(kmem_info.free_root, "sun_kmem_alloc");
#endif
	/*
	rmlog(0, retblock, nbytes, caller());
	*/
	if (retblock)
		bzero((caddr_t)retblock, numbytes);
	(void) splx(s);
	return ((caddr_t)retblock);
...
#ifdef	DEBUG
printf("sun_kmem_free. ptr %x nbytes %d\n", ptr, nbytes);
prtree(kmem_info.free_root, "sun_kmem_free");
#endif
	if (nbytes == 0) {
		return;
	}
	 ...
	 */
	if (ptr < (caddr_t)vm_map_min(mb_map) || ptr > (caddr_t)vm_map_max(mb_map)) {
		printf("sun_kmem_free: illegal pointer %x\n",ptr);
		panic("sun_kmem_free");
		return;
	}
	s = splnet();
	...
	while (neighbor != NIL) {
		...
		if (ptr < neigh_block) {
			if (ptr + nbytes == neigh_block) {
				 ...
				 * is bad news.
				 */
				 printf("sun_kmem_free: free block overlap %x+%d over %x\n", ptr, nbytes, neigh_block);
				 panic("sun_kmem_free: free block overlap");
			} else {
				/*
				 * Search to the left
			 	*/
			...
			if (neigh_block + neigh_size == ptr) {
				 ...
				 */
				panic("sun_kmem_free block already free");
			} else {
				/*
				 * search to the right
				 */
			 ...
			 * as "ptr == neigh_block"
			 */
			panic("sun_kmem_free: block already free as neighbor");
			return;
		} /*else*/
		neighbor = *np;
	} /*while*/
	...
	register u_int nbytes;	/* Size of node to be released */
{
	...
	if (nbytes >= sizeof (struct need_to_free)) {
		if ((int)ptr & ALIGNMASK) {
			i = ALIGNMORE(ptr);
			sun_kmem_free_intr(ptr, i);
			sun_kmem_free_intr(ptr + i, nbytes - i);
			(void) splx(s);
			return;
		}
	...
	u_int nbytes;
{
	...
	sun_kmem_free((caddr_t) p, nbytes);
	return (true);
	...
	u_int npages;
{
	...
	s = splimp();
	va = (caddr_t)kmem_mb_alloc(kmem_info.map, npages * CLBYTES);
	if (va == 0)
		panic("getpages: no more memory");
#ifdef	DEBUG
