 ...
 * HISTORY
 * $Log:	autoconf.c,v $
 * Revision 2.10  90/07/03  16:47:51  mrt
 * 	Merged mt Xinu 2.6MSD Changes.
 * 	[90/05/17  18:14:48  mrt]
 * 
 * Revision 2.9  89/07/11  13:55:51  jjc
 * 	Moved localetheraddr() and ether_print() to 
 * 	sunif/if_subr.c.
 * 	[88/07/12            jjc]
 * 
 * Revision 2.8  89/04/05  18:09:40  rvb
 * 	Added cases for CPU_SUN3_{110,60} to machine_conf setup of
 * 	the machine_slot structure.
 * 	[89/04/04            mrt]
 * 
 * Revision 2.7  89/03/09  21:31:54  rpd
 * 	More cleanup.
 * 
 * Revision 2.6  89/02/25  19:33:04  gm0w
 * 	Added new copyright, fixed includes, and removed else leg
 * 	of CMU and MACH conditionals as part of kernel cleanup.
 * 	[89/02/09            jjc]
 * 
 * Revision 2.5  88/12/19  02:49:08  mwyoung
 * 	Add declaration of cpu.  Use new include file format.
 * 	[88/12/13            mwyoung]
 * 
 * 11-Jun-88  Michael Young (mwyoung) at Carnegie-Mellon University
 *	When there are no ZS devices, there's also no zsgetspeed routine.
 *
 *  1-Mar-88  Robert Baron (rvb) at Carnegie-Mellon University
 *	Changes for new pmap module:
 *		io_addr_map -> pmap_map_page
 *
 * 04-Oct-87  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Added changes for Sun 3/60 and 3/110.
 *
 *  4-Aug-87  David Golub (dbg) and Jonathan J. Chew (jjc)
 *		at Carnegie-Mellon University
 *	Don't cache memory passed to io_addr_map().
 *
 * 28-Feb-87  Avadis Tevanian (avie) at Carnegie-Mellon University
 *	Fixed up the machine_slot information generation.
 *
 * 07-Feb-87  Mike Accetta (mja) at Carnegie-Mellon University
 *	Added include of "generic.h".
 *	[ V5.1(F2) ]
 *
 * 23-Oct-86  David Golub (dbg) at Carnegie-Mellon University
 *	Eliminated references to swap devices (setup) for MACH.
 *
 * 23-Aug-86  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Made more modifications for Mach:
 *	1) Added dummy routine, "slave_config", to be filled in when
 *	   this machine has more than one cpu.
 *	2) Added "strncmp" routine which lived in "subr_xxx.c"
 *	   in Sun Unix.
 *	3) Added "machine_conf" routine to set up machine slot
 *	   table and machine information record.
 *
 *  5-Aug-86  David Golub (dbg) at Carnegie-Mellon University
 ...
 */
#ifndef	lint
static	char sccsid[] = "@(#)autoconf.c 1.7 87/02/16";
#endif
...
/*
 * Flag that cold-start is in progress.  Used by drivers' initialization
 * routines.
 */
	...
	register struct vec *vp;
{
	...
	if (reg == NULL) {
		...
		if (extent == 0)
			extent = 1;
		a = kmem_alloc_pageable(kernel_map, round_page(ctob(extent)));
		pmap_map_page(a, pageval, round_page(ctob(extent)));
		reg = (u_short *)((u_long)a | offset);
	}
	i = (*mdr->mdr_probe)(reg, unit);
	if (i == 0) {
		if (a)
		{
			kmem_free(kernel_map, a, round_page(ctob(extent)));
		}
		return (0);
	}
...
/*
 *	We swap to normal file systems for MACH.
 */
swapconf()
{
...
consconfig()
{
	...
	for (md = mbdinit; mdr = md->md_driver; md++) {
		...
		if (fb_type==FBTYPE_SUN4COLOR && strncmp(cp, "cgfour", 5)==0)
			bwmajor = CGFOURMAJOR, bwunit = md->md_unit;
	}
	/*
...
machine_conf()
{
	...
	switch (cpu) {
	case CPU_SUN3_50:
		machine_slot[0].cpu_subtype = CPU_SUBTYPE_SUN3_50;
		break;
	case CPU_SUN3_160:
		machine_slot[0].cpu_subtype = CPU_SUBTYPE_SUN3_160;
		break;
	case CPU_SUN3_260:
		machine_slot[0].cpu_subtype = CPU_SUBTYPE_SUN3_260;
		break;
	case CPU_SUN3_110:
		machine_slot[0].cpu_subtype = CPU_SUBTYPE_SUN3_110;
		break;
	case CPU_SUN3_60:
		machine_slot[0].cpu_subtype = CPU_SUBTYPE_SUN3_60;
		break;
	}
...
/*
 * This may need to be moved into subr_xxx.c someday, but it is currently
