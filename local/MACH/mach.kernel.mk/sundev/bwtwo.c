 ...
 * HISTORY
 * $Log:	bwtwo.c,v $
 * Revision 2.8  90/02/19  15:13:39  jjc
 * 	Picked up SunOS 3.5 driver and modified for Mach.
 * 	[90/02/12            jjc]
 * 
 */
#ifndef lint
static char sccsid[] = "@(#)bwtwo.c 1.1 87/12/21 SMI";
#endif
...
#ifndef sun2
#ifdef	MACH
#include <machine/eeprom.h>	/* sun3 is for brain-damaged 3.x config */
#else	MACH
#include <sun3/eeprom.h>	/* sun3 is for brain-damaged 3.x config */
#endif	MACH
	...
	register struct mb_device *md;
{
...
#if BW2_COPY_MEM_AVAIL
	{
...
#ifdef	MACH
			kmem_free(kernel_map, md->md_addr,
				  round_page(softc->size));
#else	MACH
		...
		/*
	 	 ...
	 	 */
#ifdef	MACH
	copy_to_phys(fbvirtaddr, ptob((u_int)(btop(OBFBADDR) | PGT_OBMEM)),
		     softc->size);
#else	MACH
...
bwtwointr()
{
	...
	for (softc = bw2_softc; softc < ENDOF(bw2_softc); softc++) {
		if (!softc->image)
			continue;
#ifdef sun2
		if (softc->cr->vc_int) {
