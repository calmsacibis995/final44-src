 ...
 * HISTORY
 * $Log:	cgfour.c,v $
 * Revision 2.8  89/07/11  17:37:49  jjc
 * 	Changed to include machine dependent header files from machine/ not sun3/
 * 	because the Sun 4 uses this device driver too.
 * 	[89/07/06  16:46:40  jjc]
 * 
 * Revision 2.7  89/03/10  01:30:23  rpd
 * 	More cleanup.
 * 
 * Revision 2.6  89/03/07  23:51:10  rpd
 * 	Fixed cgfourprobe() to map in color map and frame buffer
 * 	using pmap_map_page() because some lazy bum punted the
 * 	whole thing and changed the probe to always fail!
 * 	[89/02/01            jjc]
 * 
 * Revision 2.5  89/02/25  19:49:56  gm0w
 * 	Added new copyright and fixed includes as part of kernel
 * 	cleanup.
 * 	[89/02/10            jjc]
 * 
 * Revision 2.4  88/09/27  17:31:40  rvb
 * 	Fixed Log
 * 
 * Revision 2.3  88/09/27  16:08:02  rvb
 * 	23-Sep-88  Robert Baron (rvb) at Carnegie-Mellon University
 * 	In cgfour_update_cmap: change a > b += c to a > (b += c)  Sun
 * 	pcc is too lenient.
 * 	[88/09/27  15:55:20  rvb]
 *
 * 23-Sep-88  Robert Baron (rvb) at Carnegie-Mellon University
 *	In cgfour_update_cmap: change a > b += c to a > (b += c)  Sun
 *	pcc is too lenient.
 *
 * 05-Mar-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Converted to Mach.
 *
 */
	...
	int unit;
{
	...
	/* probe the colormap and determine the cg4 type */
	{
		...
		while (1) {
#ifdef	MACH
			pmap_map_page(cmap, ptab->cmap_addr, sizeof *cmap);
#else	MACH
...
#ifdef	MACH
			/*
			 * Don't bother mapping out color map
			 * Leave it up to doprobe() to mapout the color map
			 * since it allocated it and mapped it in originally
			 */
#else	MACH
...
#undef	SIZE_TAB_ENT
	...
	/* map frame buffer sections */
	{
...
#ifdef	MACH
		if ((fbaddr = (caddr_t)kmem_alloc(kernel_map,
				ctob(pages1 + pages1 + pages8))) == 0){
		...
		for (fb = 0; fb < CG4_NFBS; fb++) {
...
#ifdef	MACH
 			pmap_map_page(fbaddr, ptab->fb_addr[fb], pages * NBPG);
#else	MACH
			 ...
			 * Leave it up to doprobe() to mapout the color map
			 * since it allocated it and mapped it in originally
			(void) mapout(kernel_pmap, softc->cmap,
				(int) btoc(sizeof *softc->cmap));
			 */

			(void) kmem_free(kernel_map, fbaddr,
				ctob(pages1 + pages1 + pages8));
#else	MACH
			(void) mapout(
