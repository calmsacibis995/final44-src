 ...
 * HISTORY
 * $Log:	cgtwo.c,v $
 * Revision 2.7  92/02/04  21:16:05  mja
 * 	Replaced entire contents with SunOS 4.1.1 version and converted
 * 	to MACH.
 * 	[91/10/28            berman]
 * 
 *
 */
#ifndef lint
static char sccsid[] = "@(#)cgtwo.c 1.40 89/04/22 SMI";
#endif
...
#ifdef	MACH
#define fbgetpage(addr) (getkpgmap(addr) & PG_PFNUM)
#define fbmapin(virt, phys, size)       (pmap_map_page((virt), (phys), (size)))
	...
	int unit;
{
	...
	/*
	 * Allocate new virtual memory for the part of device we will access.
	 * This is necessary, because simply remapping the virtual
	 * address to another physical address will result in doprobe()
	 * freeing the wrong address later when there is no CG2 there.
	 */
	fb = (struct cg2fb *)kmem_alloc_pageable(kernel_map,
						 round_page(CG2_MAPPED_SIZE));

	pmap_map_page(fb, page+btop(CG2_MAPPED_OFFSET), CG2_MAPPED_SIZE);
#else	MACH
	/* 
...
#ifdef	MACH
	{
	 	kmem_free(kernel_map, (vm_offset_t)fb, CG2_MAPPED_SIZE);
		return 0;
	}
#else	MACH
		return 0;
...
#if	MACH
			/*
			 * We have to re-map using VME_D32 for 
			 * the type bits.
			 */
			pmap_map_page(fb, page ^ (PGT_VME_D16 ^ PGT_VME_D32),
				      CG2_MAPPED_SIZE);
#else	MACH
			fbmapin((caddr_t) fb, 
...
int unit;
{
	...
	s = splx(pritospl(md->md_intpri));
        ...
        timeout(cgtwotimeout, (caddr_t) unit, hz);
