...
#ifdef	MACH
#define GP1SIZE		(GP1_SIZE)	/* size of entire GP board */
#define VME_GP1SIZE	(GP1_SIZE)	/* size of entire GP board */
#else	MACH
	...
	int	unit;
{
	...
	short *gpptr, *gpucodeptr;
         ...
         */
#ifdef	MACH
        if ((a = (caddr_t)kmem_alloc_pageable(kernel_map, (long)(btoc(GP1SIZE)))) == 0)
                panic("out of kernelmap for gp1");
        gponeextra[unit].gpx_addr = a;
        gponeextra[unit].gpx_shmem = a + GP1_SHMEM_OFFSET;
	gpptr = (short *) a;
#else	MACH
	...
	gpptr = (short *) kmxtob(a);
         ...
         */
#ifdef	MACH
        mapin(kernel_pmap, a, page, (int)btoc(GP1SIZE), PG_V|PG_KW,
		FALSE);
#else	MACH
...
#ifdef	MACH
		mapout(kernel_pmap, a, (int)btoc(GP1SIZE));
		kmem_free(kernel_map, (vm_offset_t)a, (long)(btoc(GP1SIZE)));
#else	MACH
