 ...
 * HISTORY
 * $Log:	kbd.c,v $
 * Revision 2.10  90/12/17  17:20:44  berman
 * 	Added changes from SunOS 4.0.3c for Sparcstation keyboard.
 * 	[90/09/10            berman]
 * 
 * Revision 2.9  90/02/19  15:13:51  jjc
 * 	Picked up SunOS 3.5 driver and modified for Mach.
 * 	[90/02/13            jjc]
 * 
 */
#ifndef lint
static	char sccsid[] = "@(#)kbd.c 1.1 87/12/21 Copyr 1986 Sun Micro";
#endif
...
#define kmem_alloc(size)	sun_kmem_alloc(size)
#define kmem_free(ptr, size)	sun_kmem_free(ptr, size)
#endif	MACH

/* TODO: Fix KIOCTYPE to not lie about kbd type */
/* TODO: Add new kbd info ioctl for KB_SUN4 */
	...
	int flag;
{
	...
	case KIOCTYPE:
		*(int *)data = (kbdd->kbdd_state.k_idstate == KID_OK)?
		...
		/*
		 * XXX: The big lie; Using this ioctl report 4 as 3
		 * (for backwards compatibility).  The plan is to in-
		 * vent a new ioctl that gives extensive keycap info &
		 * will report the correct hw type.
		 */
		if (*(int *)data == KB_SUN4)
			*(int *)data = KB_SUN3;
		break;
...
#if	MACH
		switch (key) {
		    case 129:	/* some vt-220s have this id */
			key = KB_VT220;
		    case KB_SUN3:
		    case KB_SUN4:
		    case KB_VT220:
			...
			if (key == 0x03 || key == 0x04) {    /* Sun-3 or 4 keyboard */
			    kbdid(tp, (key == 0x03)? KB_SUN3: KB_SUN4);
#endif	MACH
			/*
			 * We just did a reset command to a keyboard
...
/* Have our own kernel debugger */
				...
				return;
#if	MACH_KDB
#define KDB_ABORT	54	/* 'q' on SUN3 keyboard */
			} else if (key == KDB_ABORT) {
				DELAY(100000);
				kdb_kintr();	/* kick kernel debugger */
				k->k_state = NORMAL;
				kbduse(tp, (u_char)IDLEKEY, k); /* fake */
				return;
#endif	MACH_KDB
