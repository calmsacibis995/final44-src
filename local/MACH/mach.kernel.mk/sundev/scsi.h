 ...
 * HISTORY
 * $Log:	scsi.h,v $
 * Revision 2.8  90/07/03  16:48:54  mrt
 * 	Merged mt Xinu 2.6MSD Changes.
 * 	[90/05/17  18:15:40  mrt]
 * 
 * Revision 2.7  89/03/09  21:44:59  rpd
 * 	More cleanup.
 * 
 * Revision 2.6  89/02/25  19:54:11  gm0w
 * 	Added new copyright as part of kernel cleanup.
 * 	[89/02/12            jjc]
 * 
 * Revision 2.5  89/01/27  10:21:55  rvb
 * 	Made VOLATILE (again ... sigh!)
 * 	[88/03/01            rvb]
 * 
 * Revision 2.4  89/01/23  15:28:47  rvb
 * 		Fixed a stupid declaration error that gcc detected.
 * 	[88/03/01            rvb]
 * 
 * Revision 2.3  88/12/07  17:20:37  mikeg
 * 	Entered new version in kernel sources from SunOS 3.5
 * 	to support new SCSI drivers.
 * 	[88/12/01  14:11:42  mikeg]
 * 
 */
...
/* @(#)scsi.h	1.1 87/12/21	Copyr 1987 Sun Micro */
...
#define SC_GROUP_0_SIZE	6		/*  for standalone boot */
#define scc0_count	sg.g0.count0	/* for standalone boot */
...
struct	st_scsi_scb {		/* scsi status completion block */
	/* byte 0 */
	u_char	ext_st1	: 1;	/* extended status (next byte valid) */
	u_char	vu_06	: 1;	/* vendor unique */
	u_char	vu_05	: 1;	/* vendor unique */
	u_char	status	: 4;	/* intermediate status sent */
	u_char	vu_00	: 1;	/* vendor unique */
	/* byte 1 */
	u_char	ext_st2	: 1;	/* extended status (next byte valid) */
	u_char	reserved: 6;	/* reserved */
...
struct	scsi_ext_sense {	/* scsi extended sense for error class 7 */
	...
	u_char			/* reserved */
#ifdef	__GNU__
		optional_13;	/* Doesn't pcc need a variable here */
#endif	__GNU__
};
...
struct scsi_unit {
	...
	/* special cmd stuff: */
	struct	dk_cmd un_ioctl_cmd;	/* command passed in by ioctl */
	int	un_sbuf_flags;		/* flags for special commands */
	caddr_t un_sbuf_buffer;		/* special buffer to hold data */
	int	un_sbuf_mbcookie;	/* mbcookie for DVMA allocation */
	/* current transfer: */
	...
	u_short	un_count;		/* num sectors to xfer (for cdb) */
	u_short	un_last_count;		/* last num sectors xfered */
	...
	caddr_t	un_scratch_addr;	/* scratch buffer address for MT02 */
};
...
/*#define SC_UNF_RESERVED	0x0040  /* reserved for future use */
#define SC_UNF_NO_DISCON	0x0080  /* disable disconnects */
#define SC_UNF_WORD_XFER	0x0100  /* Unaligned word transfer */

#define vol	volatile
struct scsi_ctlr {
vol	int	c_reg;			/* controller registers in I/O space */
	struct	scsi_cdb c_cdb;		/* command description block */
vol	struct	scsi_scb c_scb;		/* status completion block */
vol	struct	scsi_sense *c_sense;	/* sense info on errors */
	struct 	scsi_unit *c_un;	/* scsi unit using the bus */
	struct	scsi_ctlr_subr *c_ss;	/* scsi device subroutines */
vol	struct	udc_table c_udct;	/* scsi dma info for Sun3/50 */
...
struct scsi_disk {
#if	MACH
	/*
	 * Mach uses Berkeley labels, declared below
	 */
#else
	struct	dk_map dk_map[NLPART];	/* logical partitions */
	struct	dk_geom dk_g;		/* disk geometry */
#endif	/* MACH */
	...
	u_short	un_status;		/* status from last sense */
...
#if	MACH
	/* disklabel-related stuff */
	u_char	un_wlabel;		/* write-enable for label sector */
	struct	disklabel un_label;	/* disk label, includes partition map */
	u_long	un_openpart;		/* partitions open */
	u_long	un_bopenpart;		/* block partitions open */
	u_long	un_copenpart;		/* character partitions open */
#else	/* MACH */
	u_char;
#endif	/* MACH */
};
...
struct scsi_tape {
	...
	u_char	un_density;		/* tape density or qic format */
	u_char	un_options;		/* driver options, bit options */
	...
	u_char	un_eof;			/* eof = 0, not found */
					/*     = 1, eof hit */
					/*     = 2, eof pending */
	u_char	un_cipher;		/* Sun-2 cipher drive found (no QIC-24) */
};
