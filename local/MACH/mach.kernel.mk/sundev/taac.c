 ...
 * HISTORY
 * $Log:	taac.c,v $
 * Revision 2.2  92/02/04  21:17:47  mja
...
#ifndef lint
static char sccs_id[]="@(#)taac.c	1.1 90/10/29 Copyr 1989 Sun Micro";
...
#ifndef IDM_SUN3X_PEGASUS
#define	IDM_SUN3X_PEGASUS	0x41	/* Sun-3x/470/480 */
#endif
#ifndef IDM_SUN3X_HYDRA
...
taacclose(dev, flag)
{
	...
	if (sc->sc_kmp)
#if	MACH
	    kmem_free(kernel_map, (vm_offset_t) sc->sc_kmp, (long) 1);
#else	MACH
...
dev_t dev; caddr_t data;
{
	...
	case TIOCPROFON:
...
#if	MACH
		sc->sc_kmp = kmem_alloc_pageable(kernel_map,
						 round_page(1));
		if (sc->sc_kmp == 0)
			panic("taac: out of kernelmap for devices");
		sc->sc_datap = (unsigned *)sc->sc_kmp;
#else	MACH
		...
		sc->sc_offset &= 0x1fff;
#if	MACH
		pmap_map_page(sc->sc_datap, pageval, 1); 
#else	MACH
