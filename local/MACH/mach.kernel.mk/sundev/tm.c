 ...
 * HISTORY
 * $Log:	tm.c,v $
 * Revision 2.3  89/03/09  21:47:32  rpd
 * 	More cleanup.
 * 
 * Revision 2.2  89/02/25  19:56:37  gm0w
 * 	Changes for cleanup.
 * 
 */
#ifndef	lint
static	char sccsid[] = "@(#)tm.c 1.1 86/02/03 Copyr 1983 Sun Micro";
#endif
	...
	int ctlr;
{
	...
	if (pokec(&reg->tmdev_reset, NOTZERO))
	...
	register struct buf *bp;
{
	...
	while (sc->sc_bufcnt >= MAXMTBUF)
		sleep((caddr_t)&sc->sc_bufcnt, PRIBIO);
	sc->sc_bufcnt++;
