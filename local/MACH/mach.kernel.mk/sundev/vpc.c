 ...
 * HISTORY
 * $Log:	vpc.c,v $
 * Revision 2.4  89/07/11  17:40:43  jjc
 * 	Changed to include machine dependent header files from machine/ not sun3/ because the Sun 4 uses this device driver too.
 * 	[89/07/06  16:56:48  jjc]
 * 
 * Revision 2.3  89/03/09  21:48:07  rpd
 * 	More cleanup.
 * 
 * Revision 2.2  89/02/25  19:57:19  gm0w
 * 	Changes for cleanup.
 * 
 */
#ifndef	lint
static	char sccsid[] = "@(#)vpc.c 1.1 86/02/03 Copyr 1985 Sun Micro";
#endif
...
/*
 * NNVPC is the real number of channels on the VPCs, there are NCHANNELS minor
 * devices per board, one minor device for each channel
 */
#define NCHANNELS	2
...
struct mb_driver vpcdriver = {
	...
	sizeof (struct vpcdevice), "vpc", vpcdinfo, 0, 0, 0,
};
	...
	caddr_t reg;
{
	 ...
	 * channel, allign the space an even boundary
	 */
	odd_addr_words = (char *)rmalloc(iopbmap, (long)((NNVPC * 2) + 1));
