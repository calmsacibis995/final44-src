 ...
 * HISTORY
 * $Log:	xd.c,v $
 * Revision 2.2  92/02/04  21:17:54  mja
...
#ifndef lint
static	char sccsid[] = "@(#)xd.c 1.1 90/10/29 SMI";
#endif
...
#define new_kmem_alloc(size, wait)	sun_kmem_alloc((size))
#define kmem_alloc	sun_kmem_alloc
#define faultcode_t	int
#define REL4 1
#else	MACH
...
#if	MACH
	bp = un->un_rtab;
	return (physio(xdstrategy, bp, dev, B_READ, minphys, uio));
#else	MACH
...
#if	MACH
	bp = un->un_rtab;
	return (physio(xdstrategy, bp, dev, B_WRITE, minphys, uio));
#else	MACH
	return (physio(xdstrategy, (struct buf *)NULL,
	...
	switch (cmd) {
		...
		case DKIOCSCMD:
			...
			if (com->dkc_buflen != 0) {
...
#if	MACH
				/*
				 * MACH will allow the buffer to be used
				 * as-is since it is un->un_rtab.
				 */
#else	MACH
				...
				if (buscheck(bp->b_un.b_addr, npf) < 0) {
