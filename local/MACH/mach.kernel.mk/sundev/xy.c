 ...
 * HISTORY
 * $Log:	xy.c,v $
 * Revision 2.8  90/08/17  10:42:21  mrt
 ...
 * 	Changed to include machine dependent header files from machine/ 
 *	not sun3/ because the Sun 4 uses this device driver too.
...
#if	MTXINU 
#if	SUNOS_COM
#define INPART(x)	(((x) >= pp->p_offset)			\
			 && ((x) < (pp->p_offset + pp->p_size)))
#define ONTRACK(x)	(!((x) % lp->d_nsectors))
	...
	caddr_t data;
{
...
#if	SUNOS_COM
	register struct dk_geom *sg;
	register struct dk_map  *sm;
	struct dk_info *inf;
	struct dk_conf *conf;
	struct dk_type *typ;
	...
	switch (cmd) {
		 ...
		 */
		case DKIOCGGEOM:
			sg = (struct dk_geom *)data;
			bzero((caddr_t)sg, sizeof(struct dk_geom));
			sg->dkg_ncyl = lp->d_ncylinders;
			sg->dkg_acyl = lp->d_acylinders;
			sg->dkg_bcyl = 0; /* don't know */
			sg->dkg_nhead = lp->d_ntracks;
			sg->dkg_nsect = lp->d_nsectors;
			sg->dkg_intrlv = lp->d_interleave;
			sg->dkg_apc = lp->d_sparespercyl;
			sg->dkg_rpm = lp->d_rpm;
			/* don't have physical cylinders, so we guess */
			sg->dkg_pcyl =  lp->d_ncylinders + lp->d_acylinders;
			return (0);
		...
		case DKIOCSGEOM:
			sg = (struct dk_geom *)data;
			lp->d_ncylinders = sg->dkg_ncyl;
			lp->d_acylinders = sg->dkg_acyl;
			lp->d_ntracks = sg->dkg_nhead;
			lp->d_nsectors = sg->dkg_nsect;
			lp->d_interleave = sg->dkg_intrlv;
			lp->d_sparespercyl = sg->dkg_apc;
			lp->d_rpm = sg->dkg_rpm;
			return (0);
		 ...
		 */
		case DKIOCGPART:
			sm = (struct dk_map *)data;
			sm->dkl_cylno =(lp->d_partitions[LPART(dev)].p_offset)/
				   (lp->d_secpercyl);
			sm->dkl_nblk = lp->d_partitions[LPART(dev)].p_size;
			return (0);
		...
		case DKIOCSPART:
			sm = (struct dk_map *)data;
			lp->d_partitions[LPART(dev)].p_offset=sm->dkl_cylno *
				   (lp->d_secpercyl);
			lp->d_partitions[LPART(dev)].p_size = sm->dkl_nblk;
		...
		case DKIOCGAPART:
			for (i = 0; i < NDKMAP; i++){
			    sm = (struct dk_map *)data[i];
			    sm->dkl_cylno = (lp->d_partitions[i].p_offset) /
				   (lp->d_secpercyl);
			    sm->dkl_nblk = lp->d_partitions[i].p_size;
			}
			return (0);
		...
		case DKIOCSAPART:
			...
			for (i = 0; i < NDKMAP; i++) {
			    sm = (struct dk_map *)data[i];
			    lp->d_partitions[i].p_offset = sm->dkl_cylno *
				   lp->d_secpercyl;
			    lp->d_partitions[i].p_size = sm->dkl_nblk;
			}
			(void) splx(s);
			return (0);
