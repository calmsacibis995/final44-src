 ...
 * HISTORY
 * $Log:	if_ie.c,v $
 * Revision 2.11  90/02/20  19:12:50  mrt
 * 	Removed extranous third argument from set_if_addr call.
 * 	[90/02/20            mrt]
 * 
 * Revision 2.10  89/10/11  14:51:54  dlb
 * 	New vm_map_pageable calling sequence.
 * 	[89/10/07            dlb]
 * 
 * Revision 2.9  89/07/17  16:48:57  rvb
 * 	Fwd declare iestartout() for gcc.
 * 	[89/07/17            rvb]
 * 
 * Revision 2.8  89/07/11  18:19:26  jjc
 * 	Modified SunOS 4.0 version for 4.3 under "#if MACH"
 * 		1) Don't take the address of en_addr because it's
 * 		   an array in 4.3 not a structure.
 * 		2) In ieioctl(), commented out multicast support
 * 		   and code for getting/setting size of buffer memory.
 * 		3) Commented out code for loaning out buffers to higher
 * 		   protocol layers.
 * 		4) In iegetmem(), commented out use of SunOS specific
 * 		   field to network interface structure, "if_memmap".
 * 	Mach-ified under "#if MACH"
 * 		1) Changed to use pme instead of pte.
 * 		2) Use Mach VM routines to allocate, free, and map
 * 		   virtual memory.
 * 	[88/12/12            jjc]
 * 
 */
#ifndef lint
static	char sccsid[] = "@(#)if_ie.c 1.91 88/03/08 Copyr 1987 Sun Micro";
#endif
...
#if	MACH
#include <sunif/ether_addr.h>	/* for struct ether_addr */
#endif	MACH
...
#if	MACH
	if ((getpgmap(reg) & PGT_MASK) == PGT_OBIO) {
#else	MACH
	mmu_getkpte(reg, &pte);
	if (pte.pg_type == OBIO) {
#endif	MACH
...
#if	MACH
	/*
	 *	In 4.3, the Ethernet address is just an array whereas
	 *	it is a structure in SunOS.
	 */
	...
	caddr_t data;
{
...
#if	MACH
	/*
	 *	Sun uses these, but Berkeley doesn't.
	 */
#else	MACH
	/*
	 ...
	 *	Although it's possible to use this ioctl to make the
	 *	driver allocate more memory, the driver won't use it.
	 */
	case SIOCSIFMEM:
		if (es->es_type != IE_OB) {
			error = EINVAL;
		...
		if ((mem = ifr->ifr_metric) < IE_MEMMIN)
			mem = IE_MEMMIN;
		es->es_memsize = mem;
		ieinit(unit);
		break;
	...
	register struct ie_softc	*es;
{
...
#if	MACH
	bcopy((caddr_t)es->es_enaddr, (caddr_t)iad->ieia_addr,
		sizeof(struct ether_addr));
#else	MACH
	*(struct ether_addr *)iad->ieia_addr = es->es_enaddr;
	...
	if (es->es_nmcaddr > 0) {
		...
		imc->iemc_count = sizeof (struct ether_addr) * es->es_nmcaddr;
		bcopy((caddr_t)es->es_mcaddr, (caddr_t)imc->iemc_addr,
			(u_int)imc->iemc_count);
...
#if	MACH
	/*
	 *	Sorry, we can't steal SUN's "funny" mbufs and give them
	 *	to other folks...
	 */
...
cantloan:
	 ...
	 */
	do_protocol(header, m, &es->es_ac, length, &es->es_if);
}
	...
	int	unit;
{
	...
	while (es->es_if.if_snd.ifq_head && es->es_tfd_free &&
		 ...
		 */
#if	MACH
		bcopy((caddr_t)ec->ether_dhost, (caddr_t)tfd->ietfd_dhost,
			sizeof(struct ether_addr));
#else	MACH
		*(struct ether_addr *)tfd->ietfd_dhost = ec->ether_dhost;
		 ...
		 */
#if	MACH
		bcopy((caddr_t)es->es_enaddr, (caddr_t)ec->ether_shost,
			sizeof(struct ether_addr));
#else	MACH
	...
	register struct ie_softc *es;
{
	...
	case IE_OB:
...
#if	MACH
			/*
			 *	Need to allocate virtual memory where chip
			 *	can DVMA
			 */
			va = (caddr_t)SYSBASE;	/* start here */
			vm_map_find(kernel_map, VM_OBJECT_NULL, 0, &va,
					round_page(es->es_memsize), TRUE);
			vm_map_pageable(kernel_map, va,
					va + round_page(es->es_memsize), 
		 ...
		 */
		{
			...
			mmu_getkpte(vaddr, &pte);
			paddr = pte.pg_pfnum | MAKE_PGT(pte.pg_type);
			mapin(&pte, (u_int)btop(IESCPADDR + es->es_memspace),
			    paddr, 1, PG_V | PG_KW);
			/*
			...
			es->es_scp = (struct iescp *)rmget(es->es_rmap,
			    ...
			    (u_long)(vaddr + (IESCPADDR & PGOFSET)));
			if (es->es_scp == NULL) {
				identify(&es->es_if);
				panic("no SCP memory");
			}
	...
	case IE_MB:
		...
		if (firsttime) {
			 ...
			 */
#if	MACH
			es->es_memspace = (caddr_t)kmem_alloc_pageable(
				    kernel_map, round_page(es->es_memsize));
			if (es->es_memspace == 0){
				identify(&es->es_if);
...
#if	MACH
		es->es_paddr = getpgmap((caddr_t)mie) & PG_PFNUM;
		es->es_paddr &= ~(0x100000/NBPG -1);
		es->es_paddr |= btop(paddr);
		pmap_map_page(vaddr, es->es_paddr, es->es_memsize);
#else	MACH
	...
	case IE_TE:
...
#if	MACH
			es->es_memspace = (caddr_t)kmem_alloc_pageable(
				    kernel_map, round_page(IE_TE_MEMSIZE));
			if (es->es_memspace == 0){
				identify(&es->es_if);
				printf("no kernelmap for ie memory\n");
				panic("iegetmem");
		 ...
		 * the address previously allocated and stored in memspace.
		 * The physical address is derived from the address set up
		 * by config.  Config is given the address of the csr; 
		...
		vaddr = es->es_memspace;
#if	MACH
		es->es_paddr = (getpgmap((caddr_t)tie) & PG_PFNUM) + 1;
		pmap_map_page(vaddr, es->es_paddr, IE_TE_MEMSIZE);
#else	MACH
		a = btokmx(vaddr);
		...
		mapin(&Usrptmap[a], (u_int)btop(vaddr), (u_int)es->es_paddr,
		    (int)btoc(IE_TE_MEMSIZE), PG_V | PG_KW);
#endif	MACH
	...
	register struct ie_softc	*es;
{
	 ...
	 */
	ie_tbdalloc(es, ie_tbds);
	ie_tfdalloc(es, ie_tfds);
	...
	register struct ie_softc *es;
{
	if (es->es_type == IE_OB) {
		...
		if (es->es_base) {
#if	MACH
			kmem_free(kernel_map, es->es_base, (u_int)es->es_memsize);
#else	MACH
...
#if	MACH
			/*
			 *	Don't bother to print out warning.
			 *	Since Mach kernel starts below address space of
			 *	chip, mbufs may be out of the chip's range.
			 */
#else	MACH
			identify(&es->es_if);
