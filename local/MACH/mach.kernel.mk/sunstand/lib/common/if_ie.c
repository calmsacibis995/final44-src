 ...
 * HISTORY
 * $Log:	if_ie.c,v $
 * Revision 2.2  89/07/12  00:19:29  jjc
 * 	Picked up from SunOS 4.0.
 * 	[89/07/03            jjc]
 * 
 */
#ifndef lint
static        char sccsid[] = "@(#)if_ie.c 1.11 88/02/08 Copyr 1985 Sun Micro";
#endif
	...
	struct saioreq *sip;
{
	...
	} else if (idprom(IDFORM_1, &id) == IDFORM_1 &&
	    id.id_machine == IDM_SUN3_E) {
		/* 3E ethernet */
		es = (struct ie_softc *)
	...
	for (j = 0; j < 10; j++) {
		...
		case IE_OB:
			...
			*obie = obie_reset;	/* Reset chip & interface */
			...
			savesegno = getsegmap((int)SCP_LOC >> PMGRPSHIFT);
			setsegmap(SCP_LOC, 
				getsegmap((int)(addr + BYTESPERSEG - 1)
								 >> PMGRPSHIFT));
#endif
