 ...
 * HISTORY
 * $Log:	sc.c,v $
 * Revision 2.2  89/07/12  00:20:15  jjc
 * 	Picked up from SunOS 4.0.
 * 	[89/07/03            jjc]
 * 
 */
/*
 * @(#)sc.c 1.8 88/02/08 Copyright (c) 1985 by Sun Microsystems, Inc.
 */
	...
	register struct saioreq *sip;
{
	...
	har->dma_addrh = ADDR_HI(SCSI_DMA_ADDR(sip->si_ma));
	har->dma_addrl = ADDR_LO(SCSI_DMA_ADDR(sip->si_ma));
	har->dma_count = ~sip->si_cc;
	cp = (char *) cdb;
	for (i = 0; i < SC_GROUP_0_SIZE; i++) {
	...
	if (har->icr & ICR_ODD_LENGTH) {
		if ((cdb->scc_cmd == SC_REQUEST_SENSE) || 
		    (cdb->scc_cmd == SC_READ)) {
	...
	return (sip->si_cc - (u_short)~har->dma_count);
}
