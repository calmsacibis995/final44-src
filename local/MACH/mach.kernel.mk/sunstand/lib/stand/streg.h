 ...
 * HISTORY
 * $Log:	streg.h,v $
 * Revision 2.2  89/07/12  00:23:31  jjc
 * 	Picked up from SunOS 4.0.
 * 	[89/07/03            jjc]
 * 
 */
/*	@(#)streg.h 1.2 88/02/08 SMI	*/
...
#define	OPEN		3
#define	CLOSING		4
#define OPEN_FAILED_LOADING	5
#define DETERMINE_CTLR	6
#define PRE_OPENING     7

/*
 * Operation codes - redefinitions or phony commands.
 */
...
#define IS_INVALID(dsi)	(dsi->un_ctype == ST_TYPE_INVALID ? 1 : 0)
...
/*
 * Mode sense info returned by Emulex MT02.
 */
struct st_emulex_mode_sense {
	/* byte 0 */
	u_char	reserv_0;	/* always 0x0c */
	u_char	media_type;	/* type of cartridge inserted */
	/* byte 2 */
	u_char	write_prot :1;	/* cartridge or tape is write protected if 1 */
	u_char	reserv_1   :2;
	u_char	buf_mode   :1;	/* buffered mode */
	u_char 	reserv_2   :2;
	u_char	speed      :2;	/* speed of tape drive */
	/* byte 3 */
	u_char	len;		/* descriptor length (always 8) */
	u_char	high_numblk;	/* number of blocks, high byte */
	u_char	mid_numblk;	/* number of blocks, middle byte */
	u_char	low_numblk;	/* number of blocks, low byte */
	u_char	reserv_3;
	u_char	high_blksize;	/* block size, high byte */
	u_char	mid_blksize;	/* block size, middle byte */
	u_char	low_blksize;	/* block size, low byte */
	/* byte 12 */
	u_char	reserv_4   :5;
	u_char  dea	   :1;	/* disable erase ahead */
	u_char  aui	   :1;	/* auto load inhibit */
...
#define	MODE_SENSE_WRITE_PROT(a) ((a)->write_prot)
