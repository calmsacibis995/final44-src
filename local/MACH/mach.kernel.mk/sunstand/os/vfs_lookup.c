 ...
 * HISTORY
 * $Log:	vfs_lookup.c,v $
 * Revision 2.2  89/07/12  01:15:51  jjc
 * 	Picked up from SunOS 4.0.
 * 	[89/07/03            jjc]
 * 
 */
#ifndef lint
static        char sccsid[] = "@(#)vfs_lookup.c 1.9 88/02/08 Copyr 1986 Sun Micro";
#endif
        ...
        struct vnode **compvpp;         /* ret for ptr to component vnode */
{
	...
	dprint(nfsdebug, 6,
        ...
        if (error)	{
#ifdef	NFSDEBUG
		dprint(nfsdebug, 0, "lookupname: pn_get error 0x%x\n", error);
#endif	NFSDEBUG
                return (error);
	}
        error = lookuppn(&lookpn, followlink, dirvpp, compvpp);
#ifdef	NFSDEBUG
	dprint(nfsdebug, 6, "lookupname dirvpp 0x%x compvpp 0x%x\n",
		dirvpp, compvpp);
#endif	NFSDEBUG
        pn_free(&lookpn);
        return (error);
