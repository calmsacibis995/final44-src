 ...
 * HISTORY
 * $Log:	win.c,v $
 * Revision 2.6  89/07/12  01:27:35  jjc
 * 	Modified for Sun 4.
 * 	[89/07/05  21:14:18  jjc]
 * 
 * Revision 2.5  89/03/09  22:00:07  rpd
 * 	More cleanup.
 * 
 * Revision 2.4  89/02/25  20:07:34  gm0w
 ...
 *
 * 27-Jan-88  Jonathan J. Chew (jjc) at Carngie-Mellon University
 *	Changed to use sun_kmem routines.
 *
 * 26-Oct-87  David Golub (dbg) at Carnegie-Mellon University
 *	Oops... select routines really do have to check whether
 *	the thread is waiting.  w_rsel could be non-zero from
 *	the last time...
 *
...
#include <machine/pmap.h>	/* for PG_PFNUM */
#else	MACH
#include <sys/vmmeter.h>	/* for vac_disable_kpage in non-VAC cases */
#include <sundev/kbd.h>	/* for LEFTFUNC */
	...
	dev_t	dev;
{
	...
	if (!w) {
#ifdef	MACH
		w = (struct window *)sun_kmem_alloc(WINALLOC_INC*sizeof (struct window));
#else	MACH
	...
	dev_t	dev;
{
	...
	if (!winsopen) {
		...
		for (wi = 0; wi < WINBUFS; wi++) {
			if (winbufs[wi] != NULL) {
#ifdef	MACH
				sun_kmem_free((caddr_t)winbufs[wi],
				    WINALLOC_INC*sizeof (struct window));
#else	MACH
...
#ifdef	MACH
		if (w->w_rsel &&
			((thread_t)(w->w_rsel))->wait_event == (int)&selwait)
#else	MACH
...
#ifdef	MACH
			w->w_rsel = (struct proc *) current_thread();
#else	MACH
			w->w_rsel = u.u_procp;
#endif	MACH
		break;
