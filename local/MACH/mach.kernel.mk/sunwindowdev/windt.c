 ...
 * HISTORY
 * $Log:	windt.c,v $
 * Revision 2.5  89/03/09  22:00:40  rpd
 * 	More cleanup.
 * 
 * Revision 2.4  89/02/25  20:07:59  gm0w
 * 	Added new copyright and fixed includes as part of kernel cleanup.
 * 	[89/02/12            jjc]
 * 
 * Revision 2.3  88/08/24  02:20:05  mwyoung
...
#ifdef	MACH
#define kmem_alloc	sun_kmem_alloc
	...
	int	fd;
{
	...
	dtop->shared_info = (Win_lock_block *) 
	    sun_kmem_alloc(sizeof(Win_lock_block));
#else	MACH
	dtop->shared_info = (Win_lock_block *) 
...
#ifdef	MACH
	sun_kmem_free((caddr_t) dtop->shared_info, sizeof(Win_lock_block));
#else	MACH
...
#ifdef	MACH
			if(tsleep((caddr_t)dtop->shared_info, LOCKPRI, 0) == TS_SIG) {
#else	MACH
