 ...
 * HISTORY
 * $Log:	winioctl.c,v $
 * Revision 2.7  89/03/09  22:00:56  rpd
 * 	More cleanup.
 * 
 * Revision 2.6  89/02/25  20:08:15  gm0w
 * 	Added new copyright and fixed includes as part of kernel cleanup.
 * 	[89/02/12            jjc]
 * 
 * Revision 2.5  88/12/08  15:37:27  mikeg
 * 	Modified to run Suntools under Mach.  The calls to wlok_init
 * 	have been removed since they just allocate virtual memory 
 * 	to later map in a user area and this allocation is not 
 * 	necessary under Mach.
 * 	[88/08/17  16:59:43  mikeg]
 * 
 * Revision 2.3  88/08/24  02:20:53  mwyoung
 * 	Corrected include file references.
 * 	[88/08/22            mwyoung]
 *
 * 15-Mar-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Changed call to sleep() with PCATCH argument to use tsleep()
 *	instead.
 *
 */
#ifndef	lint
...
#ifdef	MACH
			    if (tsleep((caddr_t)dtop->shared_info,
				LOCKPRI, 0) == TS_SIG) {
#else	MACH
