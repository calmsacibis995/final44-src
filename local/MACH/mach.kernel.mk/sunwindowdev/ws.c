 ...
 * HISTORY
 * $Log:	ws.c,v $
 * Revision 2.8  89/04/22  15:31:02  gm0w
 * 	Fixed up includes/conditionals.  Changed to use new inode macros
 * 	for manipulation of inodes.
 * 	[89/04/18            gm0w]
 * 
 * Revision 2.7  89/03/09  22:01:45  rpd
 * 	More cleanup.
 * 
 * Revision 2.6  89/02/25  20:09:01  gm0w
 * 	Added new copyright and fixed includes as part of kernel cleanup.
 * 	[89/02/12            jjc]
 * 
 * Revision 2.5  89/01/18  01:13:45  jsb
 * 	Vnode support: keep code that MACH otherwise undefines.
 * 	[89/01/17  11:00:40  jsb]
 * 
 * Revision 2.4  88/12/08  15:39:24  mikeg
 * 	Modified to run Suntools under Mach.  The calls to wlok_init
 * 	have been removed since they just allocate virtual memory to
 * 	map in a user area and this allocation is unnecessary in Mach.
 * 	[88/08/17  17:03:32  mikeg]
 * 
 * 15-Mar-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Merged with SunOS 3.4.
 *
 * 26-Jan-88  Jonathan J. Chew (jjc) at Carnegie-Mellon University
 *	Changed to use sun_kmem_{alloc,free}().
 *
...
#include <sys/errno.h>
#include <sys/file.h>		/* For FREAD f_data */
#if	MACH
#include <sys/inode.h>
#else	MACH
#include <sys/vnode.h>		/* For v_rdev */
#endif	MACH
...
#if	MACH
extern caddr_t		sun_kmem_alloc();
#define win_kmem_alloc	sun_kmem_alloc
#define kmem_free	sun_kmem_free
#endif	MACH
...
#if	MACH
	ws->ws_qdata = (caddr_t)sun_kmem_alloc((int)ws->ws_qbytes);
#else	MACH
	...
	register Workstation *ws;
{
	...
	if (ws->ws_qdata != NULL)
#if	MACH
		sun_kmem_free(ws->ws_qdata, (int)ws->ws_qbytes);
#else	MACH
	...
	int fd;
{
	...
	if ((err = kern_openfd(fd, &fp, FREAD)))
		goto SilentError;
#if	MACH
	dev = INODE_RDEV((struct inode *)fp->f_data);
#else	MACH
...
#if	MACH
	qdata = (caddr_t)sun_kmem_alloc(qbytes);
#else	MACH
