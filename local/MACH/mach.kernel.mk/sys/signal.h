 ...
 * HISTORY
 * $Log:	signal.h,v $
 * Revision 2.21  92/06/12  16:50:45  mja
 * 	Corrected SIG_IGN, SIG_DFL, and BADSIG prototypes
 * 	under _STDC_.
 * 	[92/06/09            neek]
 * 
 * Revision 2.20  91/04/03  13:15:47  mbj
 * 	Omron changes[sic]
 * 
 * Revision 2.19  91/01/23  08:12:53  berman
 * 	Changed non-ANSI declaration of signal() back to int.  Only the
 * 	ANSI version should be void for now.  Also changed the types
 * 	for SIG_DFL, SIG_IGN and friends to void under STDC conditionals.
 * 	Changed sigvec structure to have appropriate types for ANSI-C.
 * 	[91/01/14            berman]
 * 
 * Revision 2.18  90/10/18  12:55:30  mrt
 * 	Changed _STDC_ prototype for signal() from int to
 * 	extern void to aggree with ansi standard.
 * 	[90/10/17            mrt]
 * 
...
#define	    ILL_ALGN_FAULT	0x5	/* misaligned access exception */
#define     ILL_OPCODE_FAULT	0x6	/* unimplemented opcode exception */
#endif	luna88k
#ifdef	mc68000
...
#else	sparc
#ifdef	luna88k
#define     FPE_CHKINST_TRAP	0x7	/* bounds check violation exception */
#define     FPE_INTDIV_TRAP	0x8	/* illegal divide exception */
#define     FPE_INTOVF_TRAP	0x9	/* integer overflow exception */
#define     FPE_PRECISE_TRAP	0xB	/* floating point precise exception */
#define     FPE_IMPRECISE_TRAP	0xC	/* floating point imprecise exception */
...
struct	sigcontext {
	...
	int	sc_r2;			/* syscall argument */
	int	sc_r9;			/* syscall # */
	int	sc_epsr;		/* status */
	int	sc_ssbr;		/* scoreboard */
	int	sc_sxip;		/* xip */
	int	sc_snip;		/* nip */
	int	sc_sfip;		/* fip */
	int	sc_dmt0;		/* data access registers */
	int	sc_dmd0;
	int	sc_dma0;
	int	sc_dmt1;
	int	sc_dmd1;
	...
	int	sc_fpsr;
	int	sc_fpcr;
	int	sc_imask;		/* interrupt mask level */
	int	sc_mode;		/* interrupt mode */
	int	sc_vector;
#endif luna88k
#if	defined(mac2)
...
#else	defined(__STDC__) && !defined(KERNEL)
#define BADSIG		(int (*)())-1
