 ...
 * HISTORY
 * $Log:	user.h,v $
 * Revision 2.17  91/04/03  13:17:17  mbj
 * 	Omron changes[sic]
 * 
 * Revision 2.16  90/02/09  12:33:55  berman
...
#if	defined(luna88k) && !defined(NO_STEP_DEBUG)

	/*
	 * Support structure for single step debugging.  For more information
	 * see comments in task structure below: struct save_inst uu_next1. 
	 */ 
	struct save_inst
	 {
		int *addr;		/* address of traced instruction */
		int data;		/* save word for instruction contents */
	 };

#define	DEST_NULL	0		/* indicates no step breakpoint here */
#define	STEP_BREAK_INST	0xf000d000+504	/* this is a trap to vector 504 */

#endif	/* defined(luna88k) && !defined(NO_STEP_DEBUG) */
...
#define r_val2	u_rv.R_val2
	...
	uid_t	uu_VOPuid;		/* uid of cred passed to ufs_rdwr */
...
/* 1.1 - processes and protection */
	...
	/*
	 * The following two structures are used to hold an address of a 
	 * user instruction and the instruction itself when doing single-step
	 * debugging.  According to the MC88100 User's Manual, single
 	 * stepping on the 88K can be implemented by setting the error
	 * bit in the FIP.  This is true to some extent but gets very
  	 * unpredictable when you have real instruction access exceptions
 	 * and transfer of control (or worse, conditional transfer of
	 * control) instructions.  The other obvious alternative is to
	 * set a trap at the next instruction to be executed so we only
	 * ever execute one instruction.  This is only complicated by
	 * the fact that the next instruction to be executed is not in
	 * the next sequential memory address in the case of traps (such as
	 * system calls), jumps and branch instructions.
	 */

	struct save_inst uu_next1;	/* save instruction#1 for single step */
	struct save_inst uu_next2;	/* save instruction#2 for single step */
...
#define r_val2	u_rv.R_val2
	...
	uid_t	u_VOPuid;		/* uid of cred passed to ufs_rdwr */
...
/* 1.1 - processes and protection */
	 ...
	 * The following two structures are used to hold an address of a 
	 * user instruction and the instruction itself when doing single-step
	 * debugging.  Please see the comments above the declaration of the
	 * struct save_inst (in struct utask above) for more information.
	 */

	struct save_inst u_next1;	/* save instruction#1 for single step */
	struct save_inst u_next2;	/* save instruction#2 for single step */
#endif	/* defined(luna88k) && !defined(NO_STEP_DEBUG) */
...
#define u_VOPuid	uthread->uu_VOPuid
...
#if	defined(luna88k) && !defined(NO_STEP_DEBUG)
#define u_next1  	utask->uu_next1
#define u_next2  	utask->uu_next2
#endif	/* defined(luna88k) && !defined(NO_STEP_DEBUG) */
