 ...
 * HISTORY
 * $Log:	mem.h,v $
 * Revision 2.3  89/03/09  22:18:53  rpd
 * 	More cleanup.
 * 
 * Revision 2.2  89/02/25  20:25:45  gm0w
 * 	Changes for cleanup.
 * 
 * 14-May-86  Robert V. Baron (rvb) at Carnegie-Mellon University
 *	Add support for ma780 memory.
 *	And support for multiple bit ecc errors, only for ma780.
 *	(As hard as it is to believe, there is no support for
 *	"read data substitute" in 4.3.)
 *
 */ 
/*
...
#if	NMACH_MPM > 0
		/* NOTE:
			reg[4] only allows long word access;
			hence all the "mcrtmp" nonsense.  Further,
			we are making a first pass on processing
			"read substitute data" (SBIER  bit 13).
		 */
#define M780MA_INH(mcr)	\
	(((mcr)->mc_reg[4] = (M780_ICRD|M780_HIER|M780_ERLOG)), mtpr(SBIER, 3<<13))
#define M780MA_ENA(mcr)	\
	(((mcr)->mc_reg[4] = (M780_HIER|M780_ERLOG)), mtpr(SBIER, 7<<13))
#define M780MA_ERR(mcr)	\
	((mcrtmp = (mcr)->mc_reg[4]) & (M780_ERLOG))

#define M780MA_SYN(mcr)	((mcrtmp = (mcr)->mc_reg[4]) & 0xff)
#define M780MA_ADDR(mcr)	(((mcrtmp = (mcr)->mc_reg[4]) >> 8) & 0x7ffff)
#endif	NMACH_MPM
