 ...
 * HISTORY
 * $Log:	enet.c,v $
 * Revision 2.6  89/03/09  22:23:24  rpd
 * 	More cleanup.
 * 
 * Revision 2.5  89/02/26  10:50:37  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.4  88/08/24  03:03:29  mwyoung
 * 	Corrected include file references.
 * 	[88/08/22  23:28:41  mwyoung]
 *
 * 11-Jun-88  Michael Young (mwyoung) at Carnegie-Mellon University
 *	Another (hopefully more permanent) schain botch fix to
 *	enetioctl().
 *
 * 18-Jan-88  David Golub (dbg) at Carnegie-Mellon University
 *	Undo the schain botch fix (flip parity? (:-))
 *	Fix MACH copyright and include file names.
 *
 ...
 *	Replaced occurances of enQueue macro with enqueue_tail since we
 *	can't macro expand the enQueue macro *and* have an enQueue
 *	structure.
 *
 *  9-Sep-86  Michael Young (mwyoung) at Carnegie-Mellon University
 *	Eliminated uses of "bool".
 *
...
{									\
	enqueue_tail((queue_t)(q), (queue_entry_t)(elt));		\
	(q)->enQ_NumQueued++;						\
}
...
{									\
	...
	enqueue_tail((struct Queue *)&enFreeq, (struct Queue *)(p));		\
...
#ifdef	MACH
	/* Fix the schain botch problem by avoiding the cross-block goto */
#define BAD	return(EINVAL)
#endif	MACH
#if	DEBUG
...
#ifdef	MACH
				d->enOD_SelProc = (struct proc *) current_thread();
#else	MACH
...
register struct enOpenDescriptor *d;
{
    ...
    enqueue_tail((struct Queue *)&(nxt->enOD_Link),(struct Queue *)&(d->enOD_Link));
