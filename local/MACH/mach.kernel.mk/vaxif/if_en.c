 ...
 * HISTORY
 * $Log:	if_en.c,v $
 * Revision 2.4  89/03/09  22:25:34  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/26  10:52:30  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  88/08/24  03:04:07  mwyoung
 * 	Corrected include file references.
 * 	[88/08/22  23:29:01  mwyoung]
 * 
 *
...
#define endataaddr(en, off, type)	((type)(((caddr_t)((en)+1)+(off))))
	...
	{
		/*
		 *  We need the local net header after all.  Luckily, it
		 *  is exactly the same size as the interface pointer
		 *  which if_rubaget() tacked onto the buffer and can just
...
#endif	SWABALL
		bcopy((caddr_t)en, (caddr_t)mtod(m, struct en_header *),
						sizeof(struct en_header));
		enetFilter(es->es_enetunit, m,
				(len + sizeof(struct en_header)) );
		goto setup;
	}
#else	NENET > 0
...
#ifdef	MACH
		dest = 0;
		if (!arpresolve(&es->es_ac, m, &((struct sockaddr_in *)dst)->sin_addr, &dest, &usetrailers))
	...
	caddr_t data;
{
	...
	case SIOCSIFADDR:
		...
		case AF_INET:
#ifdef	MACH
			if ((in_lnaof(IA_SIN(ifa)->sin_addr)&0xff) != es->es_host)
#else	MACH
		...
		switch (ifa->ifa_addr.sa_family) {
		case AF_INET:
			es->es_ac.ac_ipaddr = IA_SIN(ifa)->sin_addr;
