 ...
 * HISTORY
 * $Log:	if_ni.h,v $
 * Revision 2.4  89/03/09  22:27:30  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/26  10:54:39  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  88/08/10  10:40:39  rvb
...
#define NI_FREEQ_2 NI_NBUF /* All other rcv datagrams */
#ifdef	notdef
#define NI_FREEQ_3 NI_NBUF /* All other rcv datagrams */
#endif
#define NI_NUMBUF 12	/* # of buffer names/dgsnd */
#define NI_RBUF 3	/* # of buffer names/dgrec */
#define NI_NRECV NI_FREEQ_1+NI_FREEQ_2
#define NI_MAXPACKETSZ	1024
#define NI_IPL 0x17

#define NI_MAXITRY 100000 /* How many times to try an insqti */

/* Number of bytes allocated for each zone. */
#define ZONE512_SZ (9 + NI_FREEQ_2 / NI_RBUF + NI_FREEQ_1 / NI_NUMBUF) * 512
#ifdef	notdef
#define ZONE124_SZ (NI_FREEQ_1 / NI_NUMBUF) * 128  /* round to multiple of 8 */
#endif	notdef
#define ZONE52_SZ  (NI_FREEQ_2 / NI_RBUF) * (NI_DQHEAD + NI_DGRLEN + 12)
#define ZONE14_SZ  (NI_FREEQ_2 / NI_RBUF) * 14
...
struct ni_counters {
	...
	unsigned long	bytes_rec;		/* bytes recieved */
	unsigned long	bytes_snt;		/* bytes sent */
	unsigned long	frame_rec;		/* frames recieved */
	unsigned long	frame_snt;		/* frames sent */
	unsigned long	mbytes_rec;		/* multicast bytes rec. */
	unsigned long	mframe_rec;		/* multicast frames rec. */
	unsigned long	mbytes_snt;		/* multicast bytes sent */
	unsigned long	mframe_snt;		/* multicast frames sent */
	unsigned long	fs_def;			/* frames sent initially deferred */
	unsigned long	fs_sc;			/* "	  "    single collision */
	unsigned long	fs_mc;			/* "      "    multiple " */
 ...
 */
typedef struct _vppqb	{		/* Vaxport Port Queue Block	     */
    struct _vpbq cmdq0;			/* Command queue head priority 0     */
    struct _vpbq cmdq1;			/* Command queue head priority 1     */
    struct _vpbq cmdq2;			/* Command queue head priority 2     */
    struct _vpbq cmdq3;			/* Command queue head priority 3     */
    struct _vpbq rspq;			/* Response queue head		     */
#ifdef	notdefyet
