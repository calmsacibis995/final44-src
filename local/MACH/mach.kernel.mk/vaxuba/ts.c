 ...
 * HISTORY
 * $Log:	ts.c,v $
 * Revision 2.4  89/03/09  22:39:50  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/26  11:08:54  gm0w
 * 	Changes for cleanup.
 * 
 * 19-Apr-88  Mike Accetta (mja) at Carnegie-Mellon University
 *	CMUCS: Add include of "sys/proc.h" to support controlling
 *	tty fields which have been moved to this structure.
 *	[ V5.1(XF23) ]
 *
 *  6-Nov-87  Robert V. Baron (rvb) at Carnegie-Mellon University
 *	Support for VAX650 just like VAX630.
 *	Only reset to spl5 in intr if cpu == VAX_650 or VAX_630.
 *
 * 09-Mar-87  Mike Accetta (mja) at Carnegie-Mellon University
 *	CMUCS:  switched "success" status message to be logged
 *	with LOG_NOTICE rather than direct to the console so we won't
 *	see it unless we're looking for it.
 *	[ V5.1(F6) ]
 *
 * 08-Dec-86  Mike Accetta (mja) at Carnegie-Mellon University
 *	CMUCS: increased reset delay in tsprobe() from 100 to 1000
 *	to give the SI Q-bus interface the extra time it evidently
 *	needs.
 *	[ V5.1(F1) ]
 *
 */ 
...
#ifdef	MACH
	/*
	 *  SI Q-bus interface needs more time it seems.
	 */
	DELAY(1000);
#else	MACH
	DELAY(100);
