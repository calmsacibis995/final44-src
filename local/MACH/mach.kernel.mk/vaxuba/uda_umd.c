 ...
 * HISTORY
 * $Log:	uda_umd.c,v $
 * Revision 2.12  89/04/07  14:58:57  rvb
 * 	Don't want cmap.h here.  Its a complicated story ...
 * 	And it only gets worse since we need phys.c
 * 	[89/04/06            rvb]
 * 
 * Revision 2.11  89/04/05  13:05:31  rvb
 * 	Well, I guess it's time to abandon the CMUCS_DISK hack for the
 * 	ra82, since the last cleanup made it not work any more.
 * 	[89/04/03            rvb]
 * 
 * Revision 2.10  89/03/09  22:40:51  rpd
 * 	More cleanup.
 * 
 * Revision 2.9  89/03/05  00:42:01  rpd
 * 	Added ra80wrenv696 disktype for wrenv on DILOG 696 ESDI
 * 	controller.
 * 	[89/02/12            berman]
 * 
 * Revision 2.8  89/02/26  11:09:49  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.7  88/12/19  02:56:44  mwyoung
 * 	Remove lint.
 * 	[88/12/17            mwyoung]
 * 	
 * 	Make ra82_sizes conditional on CMUCS_DISK.
 * 	And for now Make the above condition fail, so we get
 * 	non CMUCS_DISK treatment for our one RA82 at CMU running MACH.
 * 	[88/12/06            rvb]
 * 
...
}, ra80wrenv696_sizes[8] = {
	16320,	0,		/* A=sectors 0 thru 16319 */
	33660,	16320,		/* B=sectors 16320 thru 49979 */
	719100,	0,		/* C=sectors 0 thru 719099 */
	194310,	49980,		/* D=sectors 49980 thru 244289 */
	140250,	244290,		/* E=sectors 244290 thru 384539 */
	334560,	49980,		/* F=sectors 49980 thru 384539 */
	334560,	384540,		/* G=sectors 384540 thru 719099 */
	669120,	49980,		/* H=sectors 49980 thru 719099 */
}, ra80dd_sizes[8] = {
	16080,	0,		/* A=sectors 0 thru 16079 */
	67000,	16080,		/* B=sectors 16080 thru 83079 */
...
#define RA80WRENV696TYPE		  (NTYPES+15)
	"ra80wrenv696",	ra80wrenv696_sizes,
#else	MACH
#if	GYRE
	/*
...
/*
 *  Temporarily redefine the mfpr() routine to avoid lack of TODR on 8800 and
 *  630.  It is undefined at the end of the driver before the emulation
 *  routine. 
 *
 *  N.B. This approach assumes that mfpr() is not a macro!
 */
#define mfpr(r)	udamfpr(r)

/*
 *  This is used to provide the base of the mapping register area to the KDB50.
 *  It was also used on the MicroVAX-1 although we aren't likely to ever get
 *  around to restoring support for it here.
 */
#define seq_mapbase	seq_xxx1[0]	/* mapping base for KDB50 */
...
#ifdef	MACH
	udamfpr(-1);			/* prime TODR emulation */
#endif	MACH
...
gotit:
	 ...
	 */
	type = udafixtype(sc, mp->mscp_guse.guse_drivetype,
			 mp->mscp_guse.guse_mediaid);
#else	MACH
	...
	if ((sc->sc_flags & SC_MAPPED) == 0) {
		...
		if (sc->kdb50) {
			...
			sc->sc_uda = (struct uda *)
				pmap_extract(pmap_kernel(), &uda[ctlr]);
#endif	MACH
		}
		else {
#endif	NBI > 0
	...
	struct mscp *mp;
{
	...
	type = udafixtype(&uda_softc[ui->ui_ctlr], 
			mp->mscp_onle.onle_drivetype,
			mp->mscp_onle.onle_mediaid);
#else	MACH
	...
	type = udaadjtype(&uda_softc[ui->ui_ctlr],
			type, mp->mscp_onle.onle_unitsize);
#endif	MACH
	if (type != ui->ui_type) {
...
#define phys(cast, addr)	((cast) ((int) addr & 0x7fffffff))
	...
	i = udaadjtype(&uda_softc[ui->ui_ctlr],
			i, ud->uda1_rsp.mscp_onle.onle_unitsize);
#endif	MACH
