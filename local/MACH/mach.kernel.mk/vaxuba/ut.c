 ...
 * HISTORY
 * $Log:	ut.c,v $
 * Revision 2.4  89/03/09  22:41:45  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/26  11:10:36  gm0w
 * 	Changes for cleanup.
 * 
 ...
 *	[V1(1)]
 *
 * 17-Aug-83  Mike Accetta (mja) at Carnegie-Mellon University
 *	CMUCS:  Fixed to uniformly use a timeout period of 5
 *	minutes for tape positioning commands instead of incorrectly
 *	underestimating a limit based on the repeat count (V3.07b).
 *
 * 22-Mar-83  Mike Accetta (mja) at Carnegie-Mellon University
 *	CMUCS:  fixed bug in file skip ioctl calls (V3.06g).
 *
 */
/*
...
#ifdef	MACH
		/*
		 *  Use a uniform 5 minute timeout for all operations (a
		 *  value based on the repitition count is unreliable).
		 */
		sc->sc_timo = 5*60;
#endif	MACH
