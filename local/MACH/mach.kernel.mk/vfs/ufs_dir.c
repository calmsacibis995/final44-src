 ...
 * HISTORY
 * $Log:	ufs_dir.c,v $
 * Revision 2.11  91/04/05  14:43:24  mbj
 * 	Pay attention to the UMODE_P_GID bit in dirmakeinode() for
 * 	compatibility.
 * 	[91/03/15            dlc]
 * 	Fixed dirrename() to uncache the inode if it is a text file and
 * 	the last reference has been removed. [dlc]
 ...
 * 	[90/02/10            berman]
 * 
 * Revision 2.10  91/02/04  10:25:32  berman
 * 	Fixed dirrename() to uncache the inode if it is a text file and the last
 * 	reference has been removed.
 * 	[91/02/04  07:59:28  dlc]
 * 
...
bad:
	...
	if (tip->i_nlink == 0)
#else	MACH_NBC
	if (tip->i_nlink == 0 && (tip->i_flag & ITEXT) == ITEXT)
#endif	MACH_NBC
