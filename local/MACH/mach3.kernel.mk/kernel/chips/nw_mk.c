 ...
 * HISTORY
 * $Log:	nw_mk.c,v $
 * Revision 2.5  94/12/22  17:11:10  dbg
 * 	Use tod_clock instead of sys_clock.
 * 	[94/12/22            dbg]
 * 
 * Revision 2.4  94/12/15  15:07:16  dbg
...
nw_result mk_endpoint_deallocate_internal(nw_ep ep, task_t task,
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      if (projected_buffer_deallocate(task->map,
				(vm_offset_t) pv->buf_start,
				(vm_offset_t) pv->buf_end) != KERN_SUCCESS) {
	rc = NW_INCONSISTENCY;
