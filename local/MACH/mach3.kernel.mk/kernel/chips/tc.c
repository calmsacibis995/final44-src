 ...
 * HISTORY
 * $Log:	tc.c,v $
 * Revision 2.2  94/12/15  15:09:10  dbg
 * 	Combined bus_device and bus_ctlr structures.
 * 	[94/07/20            dbg]
 * 
 * 	Merged Alpha and PMAX files to get a common TurboChannel
 * 	configuration module.  Moved to 'chips' directory.
 * 	Perhaps there should be a 'busses' directory?
 * 	[94/07/07            dbg]
 * 
...
#ifdef	mips
#define	PHYS_TO_KVIRT(addr)	(PHYS_TO_K1SEG(addr))
#define	KVIRT_TO_PHYS(addr)	(K1SEG_TO_PHYS(addr))
#endif
#ifdef	alpha
#define	PHYS_TO_KVIRT(addr)	(PHYS_TO_K0SEG(addr))
#define	KVIRT_TO_PHYS(addr)	(K0SEG_TO_PHYS(addr))
#endif

boolean_t tc_probe_tc(
	vm_offset_t	addr,
...
struct bus_driver tc_driver =
	{ tc_probe_tc, 0, tc_autoconf, 0, 0, "tc" };

#define	private	static
#define	public

public
void		(*tc_enable_interrupt)(int, boolean_t, int);
...
struct drivers_map {
	...
	{ "MC146818",   "mc"},		/* (*) MC-146818 time-of-year clock */
	{ "DS1287A ",   "mc"},		/* (*) DS1287A time-of-year clock
					       (same as MC146818) */
	{ "", 0}			/* list end */
};
	...
	boolean_t	complain)
{
	...
	switch (width) {
	    ...
	    default:
		if (complain)
			dprintf("Invalid ROM width (%#x) at %#x\n",
			       width, addr);
		return FALSE;
	}
	...
	if (addr->rom_stride.value != 4) {
		if (complain)
			dprintf("Invalid ROM stride (%#x) at %#x\n",
	...
	if ((addr->test_data[0] != 0x55) ||
		...
		if (complain)
			dprintf("Test pattern failed, option at %#x\n",
			       addr);
		return FALSE;
	}
...
tc_find_all_options(
	...
	void		(*enable_interrupt)(int, boolean_t, int))
{
	register int	i;
	vm_offset_t    addr;
	boolean_t       found;
	...
	for (i = slot_max; i >= slot_min; ) {
		addr = PHYS_TO_KVIRT(slot_phys_base[i]);
	...
	/*
	 *	Set up the interrupt-enable routine for
	 *	later use.
	 */
	tc_enable_interrupt = enable_interrupt;
}
...
public void
tc_autoconf(
	struct bus_device *dev)		/* not used */
{
	register int    i;
	boolean_t       found;
	register tc_option_t *sl;
	...
	for (i = TC_MAX_LOGICAL_SLOTS-1; i >= 0; i--) {
		...
		found = (*config) (sl->driver_name, sl->k1seg_address,
				   KVIRT_TO_PHYS(sl->k1seg_address),
				   i, "tc");
