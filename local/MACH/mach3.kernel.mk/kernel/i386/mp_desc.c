 ...
 * HISTORY
 * $Log:	mp_desc.c,v $
 * Revision 2.6  93/11/17  16:36:48  dbg
...
/*
 *	Routine to convert descriptors from "fake" format
 *	(one that the compiler and linker can handle) to
 *	"real" format (that the machine understands)
 */

extern void fix_desc(
	struct fake_descriptor *desc_array,
	int			count);
