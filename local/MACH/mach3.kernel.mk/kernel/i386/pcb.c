 ...
 * HISTORY
 * $Log:	pcb.c,v $
 * Revision 2.16  94/12/15  15:25:29  dbg
 * 	Clear FS and GS registers in switch_ktss: the new thread may not
 * 	have valid LDT entries for those segments.  Change
 * 	i386_THREAD_STATE to set the segment registers if the thread has
 * 	its own LDT.  This makes i386_REGS_SEGS_STATE obsolete.
 * 	[94/09/13            dbg]
 * 
	...
	register pcb_t	pcb)
{
    ...
    {
	/*
	 *	Clear the FS and GS segment registers, since they
	 *	may not be valid in the new task.
	 */
	asm volatile("xorw %%ax, %%ax; movw %%ax,%%fs; movw %%ax, %%gs" 
		     : /* no inputs */
		     : /* no outputs */
		     : "%%eax" /* changes EAX */ );
    }
	...
	natural_t		count)
{
	...
	switch (flavor) {
		...
		if (state->efl & EFL_VM) {
		    ...
		    if (pcb->ims.v86s.int_table) {
			/*
			 *	V8086 interrupt flag is virtualized.
			 */
			pcb->ims.v86s.flags =
		...
		if (pcb->ims.ldt != 0 || flavor == i386_REGS_SEGS_STATE) {
		    /*
		     *	The user can set the segment registers if
		     *	the thread has a local LDT or if flavor
		     *	i386_REGS_SEGS_STATE is selected.
		     *
		     *	The code and stack selectors must not be null,
		     *	and must have user protection levels.  The
		     *	other selectors will be cleared (by IRET) if
		     *	they are invalid.
		     *
		     *	Only the low 16 bits of the segment registers
		     *	are valid.
		     */
		    ...
		    saved_state->ds = state->ds & 0xffff;
		    saved_state->es = state->es & 0xffff;
		    saved_state->fs = state->fs & 0xffff;
		    saved_state->gs = state->gs & 0xffff;
		}
		else {
		    /*
		     *	The thread does not have an LDT, and
		     *	therefore is using flat mode.  Set the
		     *	segment registers for a flat 32-bit
		     *	address space.
		     */
		...
		pcb->ims.v86s.int_table = int_table;
		pcb->ims.v86s.int_count = int_count;
	...
	switch (flavor)  {
		...
		*count = 4;
		...
		if (saved_state->efl & EFL_VM) {
		    ...
		    if (pcb->ims.v86s.int_table) {
			/*
			 *	Interrupt flag is virtualized
			 */
			if ((pcb->ims.v86s.flags &
		...
		*count = i386_ISA_PORT_MAP_STATE_COUNT;
		...
		state = (struct i386_v86_assist_state *) tstate;
		state->int_table = pcb->ims.v86s.int_table;
		state->int_count = pcb->ims.v86s.int_count;
