 ...
 * HISTORY
 * $Log:	start.s,v $
 * Revision 2.19  94/12/15  15:25:57  dbg
 * 	Added routine to find processor type (386, 486, ...).
 * 	[94/01/04            dbg]
 * 
...
/*
 *	Find processor type: 386 or 486.
 *	Returns:	bit 0 -> 486
 */
	.globl	_processor_type
_processor_type:
	pushfl			/* get flags */
	popl	%eax
	movl	%eax,%ecx	/* save current flags */
	xorl	$(EFL_AC),%eax	/* change alignment-check bit */
	pushl	%eax
	popfl			/* and move to flags */
	pushfl			/* get flags again */
	popl	%eax
	xorl	%ecx,%eax	/* compare with old flags */
	setnz	%al		/* set %al == 1 if AC changed (486) */
	movzbl	%al,%eax	/* set %al == 0 if AC unchanged (386) */
	pushl	%ecx		/* restore old flags */
	popfl
	ret			/* return */
