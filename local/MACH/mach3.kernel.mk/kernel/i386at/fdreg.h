 ...
 * HISTORY
 * $Log:	fdreg.h,v $
 * Revision 2.8  94/12/15  15:39:36  dbg
 * 	Added pointer to controller structure to device structure.
 * 	[94/01/13            dbg]
 * 
 ...
 ******************************************************************************/
/* FDC register */
#define CTRLREG(ADDR)	(ADDR)		/* control register */
...
struct fdcmd {
	int	c_rbmtr;		/* motor & rcalibrate flag */
	int	c_intr;			/* intr flag */
	int	c_stsflag;		/* motor flag */
...
struct fddrtab {
	unsigned short	dr_ncyl;	/* cylinder count		 */
	unsigned short	dr_spc;		/* actual sectors/cylinder	 */
	daddr_t		p_nsec;		/* disk length (sector count)	 */
	unsigned char	dr_nsec;	/* sector per track		 */
	unsigned char	dr_type;	/* media type			 */
	unsigned char	dr_rwgpl;	/* Read / Write Gap length	 */
	unsigned char	dr_fgpl;	/* Format Gap length		 */
};

struct unit_info {
	struct unit_info *b_unitf;	/* next slave			*/
	struct ctrl_info *ctlr;		/* controller for this slave	*/
	struct buf 	 *av_forw;	/* head of I/O queue (b_forw)	*/
	int		 b_seekaddr;	/* cylinder address		*/
	unsigned short	 addr;		/* controller registers		*/
	unsigned char	 slave;		/* slave number on controller	*/
	char		 wakeme;	/* set if someone wants to be woken */
	struct fddrtab	 d_drtab;	/* floppy disk parameters	*/
