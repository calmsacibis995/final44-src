 ...
 * HISTORY
 * $Log:	hd.c,v $
 * Revision 2.24  94/12/15  15:39:59  dbg
 * 	Combine bus_device and bus_ctlr structures.
 * 	[94/07/20            dbg]
 * 
 * 	Renamed hddriver to hdcdriver to match standard IOCONF.
 * 	dev->phys_address now holds address of disk table pointer.
 * 	[94/06/16            dbg]
 * 
 * 	Use device/pc_disk.h instead of i386at/disk.h.
 * 	[94/02/21            dbg]
 * 
 * 	ANSI-fied.
 * 	[93/12/28            dbg]
...
vm_offset_t	hd_std[] = { 0 };
struct	bus_device *hd_dinfo[NHD*NDRIVES];
struct	bus_device *hd_minfo[NHD];
struct	bus_driver	hdcdriver = {
	hdprobe, hdslave, hdattach, 0, hd_std,
	"hd", hd_dinfo, "hdc", hd_minfo, 0 };
	...
	for (; i; i--) {
		if (inb(PORT_STATUS(addr))&STAT_READY) {
			...
			printf("%s%d: port = %x, spl = %d, pic = %d.\n",
				ctlr->name,
				ctlr->unit, ctlr->address,
				ctlr->priority, ctlr->irq);
#if	0
			/* may be necessary for two controllers */
	...
	struct bus_device *dev)
{
	...
	hdalive[unit] = 1;
	n = *(unsigned long *)phystokv(dev->phys_address);
	 ...
	 */
	fudge_bsd_label(lp, DTYPE_ESDI,
			dosparm[unit].ncyl, dosparm[unit].nheads,
			dosparm[unit].nsec, 2);
	...
	natural_t	*count)	/* OUT */
{
	...
	/* Mandatory flavors */
		...
		data[DEV_GET_SIZE_DEVICE_SIZE] = lp->d_partitions[part].p_size
						* lp->d_secsize;
	...
	/* Extra flavors */
		...
		*count = sizeof(struct disk_parms)/sizeof(int);
		...
		bcopy(bp1->b_un.b_addr, (char *)data, SECSIZE);
		brelse(bp1);
...
*/		break;
	...
	case V_WRABS:
		...
		bp1 = geteblk(SECSIZE);
		bcopy((char *)data, bp1->b_un.b_addr, SECSIZE);
...
void hdstrategy(
	struct buf *bp)
{
	unsigned int		unit = UNIT(bp->b_dev),
				ctrl = unit>>1;
	struct disklabel	*lp = &label[unit];
	struct partition	*part_p =
				    &lp->d_partitions[PARTITION(bp->b_dev)];
	spl_t			opri;
	...
	if (bp->b_flags & B_MD1) {
		...
		if (bp->b_blkno == part_p->p_size) {
			/* indicate (read) EOF by setting b_resid to
			   b_bcount on last block */
	...
	int ctrl)
{
	...
	if ((i = need_set_controller[ctrl]) != 0) {
		if (i&1) set_controller(ctrl<<1);
		if (i&2) set_controller((ctrl<<1)||1);
		need_set_controller[ctrl] = 0;
	}
	if ((bp = hdunit[hh[ctrl].curdrive^1].b_actf) != 0)
		hh[ctrl].curdrive^=1;
	else if ((bp = hdunit[hh[ctrl].curdrive].b_actf) == 0)
		return;
	...
	int ctrl)
{
	...
	if (bp->b_flags & B_READ) {
		...
		linw(PORT_DATA(addr),
		     (unsigned short *)hh[ctrl].rw_addr,
		     SECSIZE/2); 
	}
