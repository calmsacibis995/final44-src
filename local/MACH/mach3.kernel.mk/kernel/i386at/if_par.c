 ...
 * HISTORY
 * $Log:	if_par.c,v $
 * Revision 2.7  94/12/15  15:42:42  dbg
...
struct bus_device *lpdev = lprinfo[unit];
	...
	ivect[lpdev->irq] = oldvect;
	iunit[lpdev->irq] = oldunit;
}
...
struct bus_device *lpdev = lprinfo[unit];
	...
	if (ivect[lpdev->irq] != parintr) {
		oldvect = ivect[lpdev->irq];
		oldunit = iunit[lpdev->irq];
		ivect[lpdev->irq] = parintr;
		iunit[lpdev->irq] = unit;
	}
...
struct bus_device *dev;
{
	...
	printf("\n  par%d: at lpr%d, port = %x, spl = %d, pic = %d. ",
		unit, unit, dev->address, dev->priority, dev->irq);
