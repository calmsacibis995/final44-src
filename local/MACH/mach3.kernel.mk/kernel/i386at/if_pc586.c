 ...
 * HISTORY
 * $Log:	if_pc586.c,v $
 * Revision 2.24  94/12/15  15:42:52  dbg
 * 	Renamed pcdriver to pc586driver to match standard IOCONF.
 * 	[94/06/16            dbg]
 * 
 * 	ANSI-fied.
 * 	[93/12/28            dbg]
...
#define	SPLNET	spl6
#define CMD(x, y, unit) \
	*(u_short *)(pc_softc[unit].prom + OFFSET_ ## x) = (u_short) (y)
...
void pc586bldcu(
	int	unit);
boolean_t pc586reset(
	int	unit);
boolean_t pc586init(
	int	unit);
boolean_t pc586hwrst(
	int	unit);
boolean_t pc586diag(
	int	unit);
boolean_t pc586config(
	int	unit);
char *	pc586bldru(
	int	unit);
char *	ram_to_ptr(
...
u_short	ptr_to_ram(
	char	*k_va,
	int	unit);
boolean_t pc586requeue(
	int	unit,
	fd_t	*fd_p);
#ifndef	REQUEUE_DBG
boolean_t bad_rbd_chain(
	u_short	offset,
	int	unit);
#endif
void pc586start(
