 ...
 * HISTORY
 * $Log:	kd.c,v $
 * Revision 2.25  94/12/15  15:43:17  dbg
 * 	Pass extended keycode prefix '0xe0' to user if in keyevent mode.
 * 	[94/09/28            dbg]
 * 
 * 	Merge in PS2 hooks from IBM (from i386ps2/kd.c).
 * 	[94/06/20            dbg]
 * 
 * 	Added a standard device-probe interface.
 * 	[94/06/16            dbg]
 * 
...
extern void kd_enqsc();			/* enqueues a scancode */

struct tty	kd_tty;
extern int	rebootflag;

/*
 *	Quick move routines (in assembler):
 */
extern void kd_slmscd(
	unsigned char	*from,
	unsigned char	*to,
	unsigned int	count);		/* moves pairs of bytes */
...
extern void kd_slmwd(
	unsigned char	*ptr,
	unsigned int	count,
	unsigned short	value);		/* fills pairs of bytes */
...
static void charmvup(
	csrpos_t from,
	csrpos_t to,
	int	count);
static void charmvdown(
	csrpos_t from,
	csrpos_t to,
	int	count);
static void charclear(
	...
	int	count,
	char	chattr);
static void charsetcursor(
	csrpos_t newpos);

static void kd_noopreset(void);
void kdreboot(void);

boolean_t kd_kbd_magic(
	int		scancode);
...
void	(*kd_dput)(
		csrpos_t pos,
		char	ch,
		char	chattr)	= charput;	/* put attributed char */
void	(*kd_dmvup)(
		csrpos_t from,
		csrpos_t to,
		int	count)	= charmvup;	/* block move up */
void	(*kd_dmvdown)(
		csrpos_t from,
		csrpos_t to,
		int	count)	= charmvdown;	/* block move down */
void	(*kd_dclear)(
		csrpos_t to,
		int	count,
		char	chattr)	= charclear;	/* block clear */
void	(*kd_dsetcursor)(
		csrpos_t newpos) = charsetcursor;
						/* set cursor position
						   on displayed page */
...
void kd_right(void);
void kd_cr(void);
void kd_scrollup(void);
void kd_scrolldn(void);
void kd_cltobcur(void);
void kd_cltopcur(void);
void kd_cltoecur(void);
void kd_clfrbcur(void);
void kd_delln(
	int	number);
void kd_insln(
	int	number);
...
void kd_erase(
	int	number);
void kd_eraseln(void);
boolean_t kd_isupper(
	unsigned char	c);
boolean_t kd_islower(
	unsigned char	c);
...
unsigned char state2leds();
void kd_handle_ack(void);
void kd_resend(void);

void kd_setleds1(
	unsigned char val);
void kd_setleds2(void);
...
/*
 *	Standard device configuration data.
 */
...
vm_offset_t		kd_std[] = { 0x60, 0 };
struct bus_device *	kd_info[1];	/* only one keyboard per machine */
struct bus_driver	kddriver = {
	kdprobe, 0, kdattach, 0, kd_std, "kd", kd_info, 0, 0, 0 };

/*
 *	Probe.  Keyboard is always there, so return TRUE.
 */
boolean_t kdprobe(
	vm_offset_t	port,
	struct bus_device *dev)
...
io_return_t kdread(
	...
	io_req_t ior)
{
	kd_tty.t_state |= TS_CARR_ON;
	return char_read(&kd_tty, ior);
}
...
int kd_cmdreg_read(void)
{
	while (inb(K_STATUS) & K_IBUF_FUL);
	outb(K_CMD, KC_CMD_READ);

	while ((inb(K_STATUS) & K_OBUF_FUL) == 0);
	return inb(K_RDWR);
}

void kd_cmdreg_write(
	int	val)
{
	while (inb(K_STATUS) & K_IBUF_FUL);
 ...
 */
csrpos_t	xga_getpos(void);	/* forward */
