 ...
 * HISTORY
 * $Log:	model_dep.c,v $
 * Revision 2.13  94/12/15  15:44:49  dbg
 * 	Use write protection in kernel mode, and native FPU error
 * 	handling, if on a 486.
 * 	[94/01/04            dbg]
 * 
 * 	Fix dma > 16 meg for floppy.
 * 	[93/11/29            rvb]
 * 
...
void machine_startup(void)
{
	 ...
	 *	Check for CPU type.
	 */
	if (processor_type() & PROC_486) {
	    /*
	     *	Use kernel write protection and
	     *	native FPU error handling.
	     */
	    kernel_write_protect = TRUE;
	    set_cr0(get_cr0() | CR0_WP | CR0_NE);
	}
...
/*
 * Halt a cpu.  On single-CPU systems, this is equivalent to
 * halt_all_cpus.
 */
void halt_cpu(void)
{
...
#include <scsi.h>
	...
	if (!use_all_mem && last_addr - loadpt > 16 * 1024*1024) {
	    ...
	    printf("Limiting usable memory to 16 Meg to avoid DMA problems.\n");
