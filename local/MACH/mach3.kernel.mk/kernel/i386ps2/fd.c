 ...
 * Copyright (c) 1994,1993,1991 Carnegie Mellon University
 ...
 * HISTORY
 * $Log:	fd.c,v $
 * Revision 2.6  94/12/15  15:46:21  dbg
...
boolean_t fdslave(struct bus_device *, vm_offset_t);
void	  fdattach(struct bus_device *);

int (*fdcintrs[])() = { fdintr, 0};

struct bus_device	*fdinfo[NFD];
struct bus_device	*fdcinfo[NFDC];

struct bus_driver	fdcdriver = {
	/* probe slave    attach   	  dname  dinfo   mname  minfo */
	fdprobe, fdslave, fdattach, 0, 0, "fd", fdinfo, "fdc", fdcinfo };
...
boolean_t fdslave(
	...
	vm_offset_t	addr)
{
    if (iod->slave >= fdadapter.fd_numd)
