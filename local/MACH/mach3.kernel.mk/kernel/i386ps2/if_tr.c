 ...
 * HISTORY
 * $Log:	if_tr.c,v $
 * Revision 2.5  94/12/15  15:48:03  dbg
 * 	Use standard autoconfiguration structures.
 * 	[94/03/22            dbg]
 * 
...
#include <i386ps2/if_trreg.h>
#include <i386ps2/if_trvar.h>
#include <i386ps2/pos.h>

#ifdef MACH_TTD
#include <ttd/ttd_stub.h>
#endif

boolean_t trprobe(vm_offset_t, struct bus_device *);
void	  trattach(struct bus_device *);
unsigned short tr_swap(unsigned short);

vm_offset_t	trstd[] = { 0x00000a20, 0x00000a24, 0};

struct bus_device *trinfo[NTR];

unsigned long tokdebug = 0;		/* flag for debugfs */

/*
 * see <chips/busses.h> for declaration, eric
 */
struct bus_driver trdriver = {
	/* probe slave attach	    dname  dinfo mname minfo */
	trprobe, 0, trattach, 0, 0, "tr", trinfo, 0, 0, 0 };
...
void trxint(
	int	unit);
void trbcopy(
	struct tr_softc *trs,	/* pointer to softc struct for
					   this adapter */
 ...
 */
boolean_t trprobe(
	vm_offset_t	address,	/* pointer to device registers */
	struct bus_device *iod)		/* pointer to device structure */
{
	...
	if ((irq = tr_get_irq(address)) != 0)
		iod->irq = irq;		/* set the IRQ level */

	iod->priority = SPL4;		/* set to known spl level */

	addr = (struct trdevice *)address;
	...
	return TRUE;	/* indicate adapter should have interrupted */
}
...
void trattach(
	struct bus_device *iod)		/* pointer to device structure */
{
	register struct tr_softc *trs = &tr_softc[iod->unit];
	register struct ifnet *ifp = &trs->trs_if;
	register struct trdevice *addr =
		(struct trdevice *)((unsigned int)iod->address & 0xffffff);
	unsigned long i;
	int temp;
#if	MACH_TTD
	int	unit;
...
void trreset(
	int	unit)
{
	register struct bus_device *iod;  /* pointer to device structure */
	...
	if (unit < NTR && (iod = trinfo[unit]) != 0 && iod->alive != 0){
	...
	int	unit)
{
	...
	register int i;
	register struct bus_device *iod = trinfo[unit];
...
int trintr(int unit_num)
{
	...
	else if (MM_IN(&trs->aca->rw.isrp_h) &
...
void trbcopy(
	register struct tr_softc *trs,	/* pointer to softc struct for
					   this adapter */
	...
	register int unit)
{
  	...
  	timeout(trtimout, (void *) unit, hz*30);
...
void trtimout(
	int	unit)
{
	struct tr_softc *trs = &tr_softc[unit];
