 ...
 * HISTORY
 * $Log:	pmap.c,v $
 * Revision 2.20  94/12/15  15:49:43  dbg
 * 	If hardware implements write protection in kernel mode, use it.
 * 	[94/01/04            dbg]
 * 
...
boolean_t	kernel_write_protect = FALSE;
					/* does hardware implement write
					   protection in kernel mode?
					   For a multiprocessor, ALL cpus
					   must implement it */
extern char end;
/*
	...
	vm_prot_t	prot)
{
	...
	if (pv_h->pmap != PMAP_NULL) {
		 ...
		 * or if write-protecting a kernel mapping and
		 * hardware does not handle it.
		 */
		if (remove ||
		    (pmap == kernel_pmap && !kernel_write_protect))
		{
		    /*
	...
	vm_prot_t	prot)
{
	 ...
	 */
	if (map == kernel_pmap && !kernel_write_protect) {
	    pmap_remove(map, s, e);
	    return;
	}
...
if (pmap_debug) printf("pmap(%x, %x)\n", v, pa);
	...
	if ((pmap == kernel_pmap && !kernel_write_protect)
