 ...
 * HISTORY
 * $Log:	kmin.c,v $
 * Revision 2.11  94/12/16  11:01:23  dbg
 * 	Use chips/tc.h.
 * 	[94/07/08            dbg]
 * 
 * 	RT-Mach merge.  mc_intr calls clock_interrupt itself.
 * 	[94/06/22            dbg]
 * 
	...
	struct mips_saved_state *ss)
{
	...
	if (intr & KMIN_INTR_TIMEOUT)
		...
		mc_intr(st,
			spllevel | (KMIN_SR_IM4 << KMIN_SR_SHIFT));
...
#define	FIRE_ALARM
		...
		if ((intr & KMIN_INTR_PSWARN) && (user_warned < 3)) {
			*imaskp = 0;
			user_warned++;
			if (!ignore_power_supply_overheating)
				printf("Power supply overheating\n");
		}
	}
#endif
