 ...
 * HISTORY
 * $Log:	maxine.c,v $
 * Revision 2.13  94/12/16  11:02:34  dbg
 * 	Use chips/tc.h.
 * 	[94/07/08            dbg]
 * 
 * 	RT-Mach merge.  mc_intr now calls clock_interrupt itself.
 * 	[94/06/22            dbg]
 * 
...
{
	dtop_probe(PHYS_TO_K1SEG(XINE_SYS_DTOP+0x20000), /* why +0x20000 ? */
		   ui);
	if (rcline) {
		ui->unit = SCC_UNIT;
		scc_probe(addr, ui);
	...
	spl_t spllevel)
{
	...
	mc_intr(st,
		(XINE_SR_IM4 << XINE_SR_SHIFT) | XINE_INT_LEV7 | SR_IEc);
