 ...
 * HISTORY
 * $Log:	model_dep.c,v $
 * Revision 2.22  94/12/16  11:03:42  dbg
 * 	Use common TurboChannel configuration routines.  Pass parameters
 * 	to tc_find_all_options instead of setting global variables.
 * 	[94/07/07            dbg]
 * 
 * 	RT-Mach merge.  mc_intr calls clock_interrupt itself.  kn01 and
 * 	kn03 use unmodified mc_intr for clock interrupts.
 * 	[94/06/22            dbg]
 * 
 * 	Disable clock interrupts in *_model_dep.  On some models (e.g.
 * 	the KMIN), clearing the frame buffer with clock interrupts
 * 	enabled causes the machine to crash back into the PROM
 * 	monitor, even though clock interrupts are masked at the CPU. (!)
 * 	[94/02/12            dbg]
 * 
...
extern void	kn02_tcintr(), mc_intr(), stray_intr(), kn02_errintr();
...
void kn01_model_dep(void)
{
	...
	mc_clock_disable_interrupts(PHYS_TO_K1SEG(KN01_SYS_CLOCK));
 ...
 *	KN02
 */
vm_offset_t	kn02_slot_phys_base[TC_MAX_SLOTS] = {
...
void kn02_model_dep(void)
{
	...
	extern int      (*memory_sizing_routine)(),
			size_memory_trusting_prom();
	...
	mc_clock_disable_interrupts(PHYS_TO_K1SEG(KN02_SYS_CLOCK));
	...
	tc_find_all_options(
		...
		KN02_TC_MAX,
		kn02_slot_hand_fill,
		kn02_enable_interrupt);
}

/*
 *	KMIN (3min)
 */
vm_offset_t	kmin_slot_phys_base[TC_MAX_SLOTS] = {
	KMIN_PHYS_TC_0_START,	KMIN_PHYS_TC_1_START,
	KMIN_PHYS_TC_2_START,	KMIN_PHYS_TC_3_START,
	(vm_offset_t) -1,	(vm_offset_t) -1,
	(vm_offset_t) -1,	(vm_offset_t) -1
};
...
void kmin_model_dep(void)
{
	...
	extern void	kmin_tc0intr(), kmin_tc1intr(), kmin_tc2intr(),
			kmin_tc3intr(), kn02ba_haltintr();
	extern          (*identify_devices) (), config_3max_routine();
	extern void	kmin_slot_hand_fill();
	extern void	kmin_enable_interrupt(int, boolean_t, int);
	extern int	kn02_dma_ops[];
	extern int	asc_clock_speed_in_mhz[];
	...
	mc_clock_disable_interrupts(PHYS_TO_K1SEG(KMIN_SYS_CLOCK));
 ...
 *	Maxine
 */
vm_offset_t	xine_slot_phys_base[TC_MAX_SLOTS] = {
	XINE_PHYS_TC_0_START,	XINE_PHYS_TC_1_START,
	XINE_PHYS_TC_3_START,	(vm_offset_t) -1,
	(vm_offset_t) -1,	(vm_offset_t) -1,
	(vm_offset_t) -1,	(vm_offset_t) -1
};
...
void xine_model_dep(void)
{
	...
	extern int	*wrclr_buserror_address;
	extern int      (*memory_sizing_routine)(),size_memory_trusting_prom();
	extern void	xine_tc3intr(), kn02ba_haltintr(), kn02ba_errintr(),
			xine_mc_intr();
	extern          (*identify_devices) (), config_3max_routine();
	extern void	xine_slot_hand_fill();
	extern void	xine_enable_interrupt(int, boolean_t, int);
	extern int	kn02_dma_ops[];
	extern int	asc_clock_speed_in_mhz[];
...
#if 0
	/* disabling clock interrupts is NOT necessary on this model */
#else
	mc_clock_disable_interrupts(PHYS_TO_K1SEG(XINE_SYS_CLOCK));
#endif
 ...
 *	3Max+
 */
vm_offset_t	kn03_slot_phys_base[TC_MAX_SLOTS] = {
	KN03_PHYS_TC_0_START,	KN03_PHYS_TC_1_START,
	KN03_PHYS_TC_2_START,	KN03_PHYS_TC_3_START,
	(vm_offset_t) -1,	(vm_offset_t) -1,
	(vm_offset_t) -1,	(vm_offset_t) -1
};
...
void kn03_model_dep(void)
{
	...
	extern int	*wrclr_buserror_address;
	extern int      (*memory_sizing_routine)(),size_memory_trusting_prom();
	extern void	kn03_tcintr();
	extern void	kn03_errintr();
	extern void	kn03_haltintr();
	extern int	(*identify_devices) (), config_3max_routine();
	extern void	kn03_slot_hand_fill();
	extern void	kn03_enable_interrupt(int, boolean_t, int);
	extern int	kmin_dma_ops[];
	extern int	kn02_dma_ops[];
	extern int	asc_clock_speed_in_mhz[];
	...
	*((volatile unsigned *)(PHYS_TO_K1SEG(KN03_REG_IMSK))) = KN03_IM4;
	...
	mc_clock_disable_interrupts(PHYS_TO_K1SEG(KN03_SYS_CLOCK));
