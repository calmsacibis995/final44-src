 ...
 * HISTORY
 * $Log:	mips_init.c,v $
 * Revision 2.24  93/11/17  17:54:26  dbg
 * 	Removed call to mc_close.  Added ANSI function prototypes.
 * 	[93/06/10            dbg]
 * 
...
static void init_memory(vm_offset_t);	/* forward */

void mips_init(
	int argc,
	char *argv[])
{
	unsigned int first_page;	/* first free physical page */
	int i;
	 ...
	 *	their workplace.
	 */
	bcopy(TRAP_tlb_umiss,
	      (void *) VEC_TLB_UMISS,
	      TRAP_tlb_umiss_end-TRAP_tlb_umiss);
	bcopy(TRAP_exception,
	      (void *) VEC_EXCEPTION,
	      TRAP_exception_end-TRAP_exception);

	mips_box_model_dep();		/* cold adaptation */

	/*
	 * [ Clock is not delivering interrupts. ]
	 */
	...
	int	to)
{
	...
	for (start = mips_ptob(from), end = mips_ptob(to);
		...
		if (check_memory(PHYS_TO_K1SEG(start),1))
			...
			bzero((void *) PHYS_TO_K0SEG(start), MIPS_PGBYTES);
	return from;
}
	...
	int	to)
{
	...
	bzero((void *) PHYS_TO_K0SEG(mips_ptob(from)), mips_ptob(to-from));

	return size;
}

int (*memory_sizing_routine)(int from, int to) = size_memory_sweeping;

vm_size_t	(*mipsbox_steal_memory)(vm_offset_t) = 0;
