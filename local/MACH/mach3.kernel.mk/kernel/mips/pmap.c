 ...
 * HISTORY
 * $Log:	pmap.c,v $
 * Revision 2.31  93/11/17  17:55:20  dbg
 * 	ANSI-fied.
 * 	[93/09/29            dbg]
 * 
...
void pmap_release_page(vm_offset_t pa);
...
void get_more_pte_pages(void);		/* forward */

vm_offset_t
get_free_ptepage(void)
{
	vm_offset_t page;
...
vm_offset_t pmap_map(
	...
	vm_prot_t	prot)		/* Machine indep. protection.	*/
{
	pte_template    pte;
	pt_entry_t     *ppte;
	int             num_mips_pages;
	...
	for (; num_mips_pages > 0; num_mips_pages--,
		...
		*ppte = pte.pte;
		tlb_map_random(0, virt, pte.pte);	/* eager */
		if (pte.raw & PG_M)
...
void pmap_bootstrap(void)
{
	...
	bzero((void *) root_kptes, MIPS_PGBYTES);
	...
	pmap_t	map)
{
	...
	for ( ; ppte < eppte; ppte++)
		if (*ppte) {
			pte_page_gc( map, (pt_entry_t *)*ppte, 0);
			*ppte = 0;
			map->ptepages_count--;
		}
	...
	if (map->ovf_ptepages) {
		...
		put_free_ptepage((vm_offset_t) map->ovf_ptepages);
		map->ovf_ptepages = 0;
	}
	...
	if (*(long *)pte & PG_V) {
		...
		for ( end_pte = pte + mips_btop(PAGE_SIZE) ; pte < end_pte; pte++,
			  tmpl.raw += MIPS_PGBYTES, v += MIPS_PGBYTES) {
				*pte = tmpl.pte;
				tlb_map_random(map->pid, v, tmpl.pte);
		  }
		/*
