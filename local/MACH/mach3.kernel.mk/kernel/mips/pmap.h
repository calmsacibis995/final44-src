 ...
 * HISTORY
 * $Log:	pmap.h,v $
 * Revision 2.15  93/11/17  17:55:42  dbg
 * 	ANSI-fied.
 * 	[93/09/29            dbg]
 * 
...
typedef	int	tlbpid_t;	/* range 0..63; -1 for not_in_use */
 ...
 */
struct pmap {
	tlbpid_t	pid;		/* TLBPID when in use 		*/
...
#define	PMAP_MIN_PTEPAGES	5	/* Average no. of ptepages	*/
	...
	void		(*hacking)(	/* horrible things needed	*/
			    struct pmap *,
			    vm_offset_t);
...
/*
 *	Get physical address of page that is known to
 *	be resident
 */
extern vm_offset_t pmap_resident_extract(pmap_t, vm_offset_t);

/*
 *	PID assignment
 */

extern void	assign_tlbpid(pmap_t pmap);	/* assign PID to pmap */
extern void	destroy_tlbpid(tlbpid_t pid, boolean_t flush);
						/* destroy a PID */

/*
 *	TLB manipulation
 */
extern void	tlb_zero(int index);
extern void	tlb_map(int index, tlbpid_t pid, vm_offset_t vaddr,
			pt_entry_t pte);
extern void	tlb_map_random(tlbpid_t pid, vm_offset_t vaddr,
			pt_entry_t pte);
extern void	tlb_modify(tlbpid_t pid, vm_offset_t vaddr,
			boolean_t writable);
extern void	tlb_unmap(tlbpid_t pid, vm_offset_t vaddr);
extern void	tlb_flush(int from_index, int to_index);
extern void	tlb_flush_pid(tlbpid_t pid, int from_index, int to_index);
extern void	tlb_set_context(pmap_t pmap);
