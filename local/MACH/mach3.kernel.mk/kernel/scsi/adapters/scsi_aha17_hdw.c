 ...
 * Revision 2.3  94/12/16  11:30:57  dbg
 * 	Combine bus_device and bus_ctlr structures.
 * 	[94/07/20            dbg]
 * 
 * 	Changed eaha_driver to eahadriver for standard
 * 	autoconfiguration.  Added ANSI function prototypes.
 * 	[94/07/18            dbg]
 * 
...
vm_offset_t		eaha_std[] = { 0 };
struct	bus_device	*eaha_dinfo[NEAHA*8];
struct	bus_device	*eaha_minfo[NEAHA];
struct	bus_driver eahadriver = 
        { eaha_probe, scsi_slave, scsi_attach,
	  (int (*)()) eaha_go,
	  eaha_std, "rz",
	...
	struct bus_device *ui)
{
	...
	eaha->wd.reset = (void (*)(void *)) eaha_reset_scsibus ;
			...
			do {
				...
				while (temp_targ.done == SCSI_RET_IN_PROGRESS) {
					...
					scsi_print_sense_data(
					  &eaha_xsns[temp_targ.target_id].sns);
					printf("\n") ;
				    }
                                }
