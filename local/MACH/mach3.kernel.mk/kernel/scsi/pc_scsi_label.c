 ...
 * Revision 2.10  94/12/16  11:31:46  dbg
 * 	Include device/pc_disk.h instead of i386at/disk.h.
 * 	[94/02/21            dbg]
 * 
	...
	struct bios_partition_info *bpart)
{
	 ...
	 * to the partitions 0-3 in the label.  Hence we use the "upart" above, in
	 * the line below.
	 */
	...
	ior->io_error = 0;
	ior->io_op = IO_READ;
	ior->io_recnum = HDPDLOC;	/* that's where the vtoc is */
	scdisk_strategy(ior);
	iowait(ior);
	...
	if (n >= MAXPARTITIONS)
		n = MAXPARTITIONS;
	fudge_bsd_label(label, DTYPE_SCSI, evp->cyls, evp->tracks,
			evp->sectors, n);
#ifdef	you_care
