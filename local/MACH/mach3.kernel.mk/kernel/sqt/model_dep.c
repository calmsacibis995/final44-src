 ...
 * HISTORY
 * $Log:	model_dep.c,v $
 * Revision 2.8  93/11/17  18:46:24  dbg
 * 	Moved timemmap to machine-independent code.  Added
 ...
 */
char		*calloc(vm_size_t);
#define	csalloc(n,type)	(type *) calloc((n)*sizeof(type))
void		callocrnd(int);
void		calloc_end(void);
...
void map_slic_and_LEDs(
	vm_offset_t	*phys_addr_p);	/* forward */
void self_init(void);
void localinit(void);
void return_fw(boolean_t);
...
machine_startup(void)
{
	 ...
	 * Zero BSS.
	 */
	bzero((char *)&edata, (char *)&end - (char *)&edata);
...
void access_error(int errval_)
{
	u_char	errval = (u_char) errval_;
