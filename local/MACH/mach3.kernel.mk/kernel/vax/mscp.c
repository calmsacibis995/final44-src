 ...
 * Copyright (c) 1991,1990,1988,1987 Carnegie Mellon University
 * All Rights Reserved.
 * 
 ...
 * HISTORY
 * $Log:	mscp.c,v $
 * Revision 2.6  93/05/18  10:10:43  rvb
 * 	Lint
 * 
 * Revision 2.5  93/05/15  18:58:21  mrt
 * 	machparam.h -> machspl.h
 * 
 * Revision 2.4  91/05/14  17:44:43  mrt
 * 	Correcting copyright
 * 
 * Revision 2.3  91/05/08  13:19:50  dbg
 ...
 * 	[91/03/21            dbg]
 * 
 * Revision 2.2  91/01/08  16:41:53  rpd
 ...
 * 	[88/08/22            mwyoung]
 * 
 * Revision 2.1  89/08/03  16:23:26  rwd
 * Created.
 * 
 * 19-Oct-87  David Golub (dbg) at Carnegie-Mellon University
 *	Conditionalize entire file under NET_UDA since it won't compile
 *	otherwise.
 *
 * 18-Sep-87  Mike Accetta (mja) at Carnegie-Mellon University
 *	CS_GENERIC: change mscp_go() to OR in the buffer address bits
 *	in the MSCP command packet so that device drivers (in
 *	particular for the KDB50) can set extra control bits there in
 *	advance.
 *	[ V5.1(XF16) ]
 *
 */
...
again:
	...
	if (mi->mi_credits <= MSCP_MINCREDITS) {
		...
		mi->mi_wantcredits = 1;
#ifdef	MACH_KERNEL
		assert_wait((event_t)&mi->mi_wantcredits, FALSE);
		thread_block((void (*)()) 0);
#else	MACH_KERNEL
	...
	if (mri->mri_desc[i] & MSCP_OWN) {
		...
		mi->mi_wantcmd = 1;
#ifdef	MACH_KERNEL
		assert_wait((event_t) &mi->mi_wantcmd, FALSE);
		thread_block((void (*)()) 0);
#else	MACH_KERNEL
...
#endif
	/*
	 *  KDB50 wants to run with mapped mode bit which we assume the driver
	 *  has already set in the buffer address before calling us here.
	 *
	 *  This should be safe for the normal case as well since the buffer
	 *  address is zeroed by mscp_getcp() when the command packet is
	 *  allocated.
	 */
...
loop:
	if (mi->mi_rsp.mri_desc[nextrsp] & MSCP_OWN) {
		...
		if (mi->mi_wantcredits && mi->mi_credits > MSCP_MINCREDITS) {
			mi->mi_wantcredits = 0;
#ifdef	MACH_KERNEL
			thread_wakeup((event_t) &mi->mi_wantcredits);
#else	MACH_KERNEL
