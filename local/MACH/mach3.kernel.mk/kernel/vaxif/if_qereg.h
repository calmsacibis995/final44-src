 ...
 * HISTORY
 * $Log:	if_qereg.h,v $
 * Revision 2.3  91/05/13  06:08:11  af
 * 	Rid of the "CMU" conditional once and forall.
 * 	[91/05/12  16:37:10  af]
 * 
 * Revision 2.2  91/05/08  13:30:09  dbg
 * 	Added volatile declarations.
 * 	[91/03/26            dbg]
 * 
 * Revision 2.1  89/08/03  16:39:55  rwd
 * Created.
 * 
 * Revision 2.2  88/08/22  21:33:54  mja
...
#if	MACH
#define QE_PARITY       0x4000          /* Host memory parity error     */
#endif	MACH
#define QE_RCV_INT	0x8000		/* Receiver interrupt		*/
#if	MACH

/*
 * Vector address register bits (var)
 * (most of these only apply to the DELQA)
 */
#define QE_IDENT        0x0001          /* Identity bit: DEQNA or DELQA */
#define QE_IVEC         0x03fc          /* Interrupt vector address     */
#define QE_SELFTEST     0x1c00          /* Self-test status             */
#define QE_TEST_ROM     0x1c00          /* status: ROM CRC test failed  */
#define QE_TEST_RAM     0x1800          /* status: RAM test failed      */
#define QE_TEST_68000   0x1400          /* status: 68000 test failed    */
#define QE_TEST_QIC     0x1000          /* status: QIC test failed      */
#define QE_TEST_QNA2    0x0c00          /* status: QNA2 test failed     */
#define QE_TEST_SAROM   0x0800          /* status: SA ROM test failed   */
#define QE_TEST_LANCE   0x0400          /* status: LANCE test failed    */
#define QE_REQTEST      0x2000          /* Request self-test            */
#define QE_OPTSWITCH    0x4000          /* S4 option switch setting     */
#define QE_MODESEL      0x8000          /* Mode select                  */
#endif	MACH
 
/*
 * Transmit and receive ring descriptor ---------------------------
 *
 * The QNA uses the flag, status1 and the valid bit as a handshake/semaphore
 * mechanism. 
 * 
...
/*
 * Status word definitions (receive)
 *	word1
 */
...
/*
 * Status word definitions (transmit)
 *	word1
 */
