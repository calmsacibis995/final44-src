 ...
 * HISTORY
 * $Log:	rk.c,v $
 * Revision 2.5  92/02/23  22:45:52  elf
 * 	Added rkgetstat().
 * 	[92/02/22  18:56:06  af]
 * 
 * Revision 2.4  91/05/13  06:08:55  af
 * 	Rid of the "CMU" conditional once and forall.
 * 	[91/05/12  16:41:34  af]
 * 
 * Revision 2.3  91/05/08  13:33:32  dbg
 * 	Move br and cvec to memory.
 * 	[91/03/26            dbg]
 * 
 * Revision 2.2  90/08/27  22:15:38  dbg
 * 	Moved sys/dkbad.h to vax directory.
 * 	[90/08/13            dbg]
 * 
 * 	Adapted for MACH_KERNEL.
 * 	[88/12/05            dbg]
 * 
 * Revision 2.1  89/08/03  16:42:39  rwd
 * Created.
 * 
 * Revision 2.4  88/11/14  14:35:14  gm0w
 * 	Fixed more schain botch compiler errors.
 * 	[88/10/25            gm0w]
 * 
 * Revision 2.3  88/07/21  17:39:54  mwyoung
 * Fix the problematic schain botch in rkintr() by rewriting
 * to avoid goto.
 * 
 * 21-Jul-88  Michael Young (mwyoung) at Carnegie-Mellon University
 *	Fix the problematic schain botch in rkintr() by rewriting
 *	to avoid goto.
 *
 * 19-Nov-87  Mach Kernel (mk) at Carnegie-Mellon University
 *	Eliminate MACH_LOAD support.
 *
 * 31-Mar-86  David Golub (dbg) at Carnegie-Mellon University
 *	Added bogus definition before label 'hard' to get around compiler
 *	bug.
 *
 * 26-Jan-86  Avadis Tevanian (avie) at Carnegie-Mellon University
 *	Upgraded to 4.3.
 *
 *  6-Mar-85  Robert V Baron (rvb) at Carnegie-Mellon University
 *	use abstophys in rkdump. see autoconf.c
 *
 * 15-Sep-84  Robert V Baron (rvb) at Carnegie-Mellon University
 *	Dump as of much of memory to slice b as will fit if there is more
 *	memory than disk.
 */
 ...
 * Get status routine
 */
rkgetstat(dev, flavor, status, count)
	dev_t	dev;
	int	*status, *count;
{
	register struct uba_device *ui;
	register struct rkst *st;
	register int xunit, unit;
