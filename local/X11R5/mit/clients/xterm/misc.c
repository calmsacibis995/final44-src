	...
	*/
	...
	if(screen->logfile == NULL || *screen->logfile == 0) {
		...
		if(log_default == NULL)
			...
			mktemp(log_default);
		if((screen->logfile =
		    malloc((unsigned)strlen(log_default) + 1)) == NULL) {
			Bell();
			return;
		}
...
#else
		/* Open with user's real privileges */
		screen->logfd = ropen(screen->logfile,
...
/* Just like open but use the real user's privileges */
int
