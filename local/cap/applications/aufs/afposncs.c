/*
 *  Author: cck    Date: 88/04/16 23:00:51  
 *  Header: afposncs.c,v 1.6 88/04/16 23:00:51 cck Rel  
 *  Revision: 1.6  
*/
...
#define NCS_entry(name, suf, conj, creator, type, totable, fromtable) \
   { name, suf, sizeof(suf)-1, conj, creator, sizeof(creator)-1, \
     type, sizeof(type)-1, totable, fromtable }
...
#endif
  /* ASCII */
  NCS_entry("ascii", NULL, NCS_AND, (byte *)"unix", (byte *)"TEXT", 
	    to_ascii, from_ascii),
  {NULL}
};
...
register int cnt;
{
  ...
  if (tt == to_ascii || tt == from_ascii) {
    ...
    if (tt == to_ascii) fc = ENEWLINE, tc = INEWLINE;
    else fc = INEWLINE, tc = ENEWLINE;
    while (cnt--) {
      if (*bp == fc)
	*bp = tc;
      bp++;
...
ncs_table_dump()
{
  ...
  for (idx = 0, tt = ncs_transtables; idx < NCS_TRANSTABLELEN; tt++, idx++) {
    log("  %s: xlate on: suffix %s %s file type %s, creator %s",
	tt->ncs_trans_name,
