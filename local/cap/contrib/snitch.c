...
char **argv;
{
  ...
  while ((c = getopt(argc,argv,"n:t:f:d:D:l:S")) != EOF) {
    ...
    case 't':
      ...
      break;
    case 'f':
      strncpy(finderstring, optarg, MAXSNITCHSTRING);
      break;
    case 'l':
      strncpy(lwstring, optarg, MAXSNITCHSTRING);
      break;
    case 'd':
...
setup_snitch()
{
  import word nis_net;		/* our nis network number */
  import byte nis_node;		/* our nis node number */
  ...
  if (this_net != nis_net || nis_node != this_node)
    sprintf(real_system, "System: CAP: bridge %s atis: net %3d.%02d node %d",
	    inet_ntoa(bridge_addr), 
	    nkipnetnumber(nis_net), nkipsubnetnumber(nis_net), nis_node);
  else 
