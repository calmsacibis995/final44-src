/*
 *  Author: cck    Date: 88/05/19 13:16:48  
 *  Header: ash.c,v 1.14 88/05/19 13:16:48 cck Rel  
 *  Revision: 1.14  
*/
...
int srn;
{
  ...
  if (getline(line)==NULL)
    return;
  if (!dologin(srn, line, line[0] == '\0' ? NULL : getpass("Password:"))) {
  ...
  if (vpr->gspr_nvols == 1) {
    strcpy(line, vpr->gspr_volp[0].volp_name);
  } else {
    printf("What volume?\n");
    if (getline(line) == NULL)
      return;
...
byte *path;
{
  ...
  if (path == NULL)		/* NULL name means no file */
    return;
  if ((p = rindex(path, ':')) == NULL) /* any ':'s in name? */
...
byte *path;
{
  ...
  comp = eFPOpenFork(srn, volid, dirid, 0x1, path, type,
		     (word)FP_DFLEN|FP_RFLEN, &epar, &fd, &cr);
  if (comp < 0) {
    return(FALSE);
  }
  if (cr != noErr) {
...
byte *path;
{
  ...
  strcat(buf, path);
  {
    ...
    if ((tmpfd = open(buf,O_RDONLY)) >= 0) {
      ...
      strcpy(epar.fdp_finfo, "TEXTEDIT");
    }
  }
