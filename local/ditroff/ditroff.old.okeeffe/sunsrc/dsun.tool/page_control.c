...
page_control(fp,additional_papers)
FILE * fp;			/* Pointer into the ditroff file */
int additional_papers;		/* are there additional documents to process? */
{
	int done = 0;
	struct page * find_page();
	int last_page;			/* have we processed the last page
	...
	while (!done){
		...
		if (io_pending){
			last_page = handle_input(&curr_proc_pgno,curr_dis_pgno);
			io_pending = 0;
		} else {
			output = 0;
		...
		if (last_page && additional_papers) 
			done = 1;	/* go and get the next paper */
	}
}
