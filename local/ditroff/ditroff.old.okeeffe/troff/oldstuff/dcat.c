...
char	SccId [] = "%W%	(Berkeley)	%G%";	/* version number */
...
char	*fontdir= "/usr/lib/font/devcat";
extern char devname[];
...
fileinit()	/* read in font and code files, etc. */
{
	...
	/* and set params
	*/
	sprintf(temp, "%s/DESC.out", fontdir);
...
/*	if (s1 == NULL || *s1 == 0)
		sprintf(temp, "%s/%s.code", fontdir, fontbase[n]->namefont);
	else
...
char *s, *s1;
{
	...
	if (strcmp(s, fontbase[n]->namefont) == 0)
		return;
	if (s1 == NULL || s1[0] == '\0')
		sprintf(temp, "%s/%s.out", fontdir, s);
	else
...
int n;
{
	...
	font = n;
		...
		/* the graphics routines - not available for cat-codes */
drawline()
{
}
