...
char	fitab[NFITAB];	/* font table: position of char i;  0 if not there */
...
char *argv[];
{
	...
	while (fscanf(fin, "%s", cmd) != EOF) {
		if (cmd[0] == '#')	/* comment */
		...
		} else if (strcmp(cmd, "stipples") == 0) {
			fscanf(fin, "%hd", &dev.nstips);
			for (i = 0; i < dev.nstips; i++)
				fscanf(fin, "%s", sname[i]);
			...
			fprintf(stderr, "devconfig: unknown command %s\n", cmd);
	}
	if (argc > 0 && strcmp(argv[1], "DESC") == 0) {
		...
		for (i = 0; i < dev.nstips; i++) {
			totfont += strlen(sname[i]) + 1;
			write(fdout, sname[i], strlen(sname[i]) + 1);
		}
			...
			while (fgets(buf, 100, fin) != NULL) {
				...
				*/
				if (strlen(ch) == 1)	/* it's ascii */
					...
					if (i >= dev.nchtab)
						fprintf(stderr, "devconfig: font %s: %s not in charset\n", name, ch);
				}
			}
			nw++;
			font.nwfont = n = nw;
