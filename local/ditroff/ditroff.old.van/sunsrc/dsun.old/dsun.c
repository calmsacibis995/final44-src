/*	%M%	%I%	%E%
 *
 * Sun I driver for device independent troff
 *
 * Authors:	BWK(BELL)
 *		VCAT(berkley)
...
FILE *	term;		/* terminal file to ask user for input at each page */
int	termset = 0;	/* flag to see if terminal is open yet. */
struct sgttyb termb;	/* terminal control buffer for setting modes */
struct sgttyb inittermb;/* terminal control buffer for resetting modes */
...
{
	if (termset) {
	    ioctl(fileno(term), TIOCSETP, &inittermb);
	    putc ('\n', term);
	    close(term);
	}
	exit(status);
...
t_init()	/* initialize device */
{
	...
	if (!termset) {		/* do this only once */
	    term = fopen("/dev/tty", "r");	/* open display for reading */
	    if (term == NULL) error(FATAL, "can't access terminal");
	    ioctl(fileno(term), TIOCGETP, &inittermb);
	    termb = inittermb;
	    termb.sg_flags |= CBREAK;
	    termb.sg_flags &= ~ECHO;
	    ioctl(fileno(term), TIOCSETP, &termb);
	    termset = 1;
...
int t_page(n, fp)  /* `n' is the page number (if `-1', this is the last page) */
int n;
FILE *fp;
{
	...
	char character;			/* character read in from user */
...
int code;		/* character to print */
{
    ...
    if (dis->nbytes) {
	x = ((hpos * sunres) / outres) - dis->left;
	y = ((vpos * sunres) / outres) - dis->up;
