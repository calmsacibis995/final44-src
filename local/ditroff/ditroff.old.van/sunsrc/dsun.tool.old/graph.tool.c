 ...
 *		from current (h_pos, v_pos) to (h_pos + dh, v_pos + dv).
 *
 * Side Efct:	Resulting position is at end of line (h_pos + dh, v_pos + dv)
 *----------------------------------------------------------------------------*/
...
register int dv;
{
    if (output) HGtline (h_pos, v_pos, h_pos + dh, v_pos + dv);
 ...
 *
 * Results:	Draws a circle with leftmost point at current (h_pos, v_pos)
 *		with the given diameter d.  Does a thick line by calling
 *		RoundEnd many times with varying radii.
 *
 * Side Efct:	Resulting position is at (h_pos + diameter, v_pos)
 *----------------------------------------------------------------------------*/
...
int d;
{
	...
	while (delta_rad <= limit) {
	    RoundEnd (scale(h_pos)+radius, scale(v_pos), delta_rad, FALSE);
	    delta_rad++;
	}
    }
 ...
 *
 * Side Efct:	Resulting position is at (h_pos + hd, v_pos).
 *----------------------------------------------------------------------------*/
...
int vd;
{
    ...
    basex = scale(h_pos);		/* bases == coordinates of center */
    hmot(hd);				/* horizontal motion (h_pos should */
    if (!output) return;		/*   NOT be used after this) */
    basey = scale(v_pos);
					/* hd and vd are radii, not diameters */
    if ((hd=scale(hd>>1)) < 1) hd = 1;	/* neither diameter can be zero. */
    basex += hd;			/*   hd changed!! no hmot after this */
    if ((vd=scale(vd>>1)) < 1) vd = 1;
 ...
 *
 * Results:	Draws an arc starting at current (h_pos, v_pos).  Center is
 *		at (h_pos + cdh, v_pos + cdv) and the terminating point is
...
register int pdv;
{
    ...
    if (output) HGArc(h_pos + cdh, v_pos + cdv, h_pos, v_pos, (int) angle);
    hmot(cdh + pdh);
    vmot(cdv + pdv);
}
...
int pic;
{
	...
	if (len - (ptr - buf) < 15 && *(buf + len - 1) != '\n') {
		...
		error(TRUE, "unexpected end of input");
	    ptr = buf;
	}
    }
...
int npts;
{
    register long nseg;		/* effective resolution for each curve */
    ...
    long j;			/* inner curve segment traverser */
	...
	xp = x[1] - x[2];	/* "nseg" is the number of line segments */
	yp = y[1] - y[2];	/* to be drawn for each curve segment. */
				/* The shift is there to get better */
				/* performace by sacrificing resolution. */
	...
	for (j = 1; j < nseg; j++) {
	    ...
	    t3 = t1 + ffixd(0.5) - (w + w);
	    t2 = ffixd(2.0) - (t3 + t1);
	    xp = (fintr(fmult(t1, ffix(x[2])) + fmult(t2, ffix(x[1]))
	    		+ fmult(t3, ffix(x[0]))) + 1) / 2;
	    yp = (fintr(fmult(t1, ffix(y[2])) + fmult(t2, ffix(y[1]))
			+ fmult(t3, ffix(y[0]))) + 1) / 2;
