...
/* Compatability mode hacks starts here */
int	inRES = RES;		/* Resolution when prepared */
char myname[] = "ver";		/* name of this device */
char inname[] = "ver";		/* prepared for this device */
struct namtab {
	char in[3], to[3];
	} namtab[100];

#define	SCALE(x)	(int)(((double)RES * (double)(x))/(double)inRES)
...
#define  vmot(n)	vgoto(vpos + SCALE((n)))
...
#ifndef BERK
		case 'n':
			name = operand(&argc, &argv);
			break;

		case 'h':
			host = operand(&argc, &argv);
			break;
		case 'x':
		case 'y':
			dummy = operand(&argc, &argv);
			break;
...
/******
 * initialise the name mapping table
 */
initnamtab()
{
	register struct namtab *np;
	register char *p;
	char temp[BUFSIZ];
	FILE *fd;

	if(strcmp(inname, myname) == 0)
		return;
	
	sprintf( temp, "%s/devver/%sto%smap", fontdir, inname, myname);
	if((fd = fopen(temp, "r")) == 0 )
		error(FATAL, "Can't open name map table (%s)", temp);
	np = &namtab[0];

#ifdef DEBUGABLE
	if(dbg)
		fprintf(stderr,"Name mapping\n");
#endif DEBUGABLE
	while(fgets(temp, BUFSIZ, fd) != NULL){
		p = temp;
		if(*p == '#')	/*commentline*/
			continue;
		sscanf(p,"%2s %2s", &(np->in[0]), &(np->to[0]));
#ifdef DEBUGABLE
		if(dbg)
	...
	np->to[0] = np->to[0] = (char)0;	/* flag end of table */
	fclose(fd);
}

/******
 * map an incoming fontname to a name we know
 */
char *
namemap(s)
char *s;
...
#ifdef DEBUGABLE
		if(dbg)
			fprintf(stderr,"Namemap: found :%s:\n", np->to);
#endif DEBUGABLE
			return(np->to);
		}
#ifdef DEBUGABLE
		if(dbg)
			fprintf(stderr,"Namemap: trying :%s:%s:\n", s, np->to);
#endif DEBUGABLE
		np++;
	}
#ifdef DEBUGABLE
	if(dbg)
		fprintf(stderr,"Cannot map :%s:, defaults to :R:\n", s);
#endif DEBUGABLE
	return("R");	/* This can always be found */
}
...
#ifdef DEBUGABLE
	if(dbg)
		fprintf(stderr, "Loading %s at fpos %d ...\n", s, n);
#endif DEBUGABLE
#ifdef BERK
...
#ifdef DEBUGABLE
		if(dbg)
		    fprintf(stderr,"Swapping %s at fpos %d with %s at fpos %d\n",
			fontbase[n]->namefont, n, fontbase[fin]->namefont, fin);
#endif DEBUGABLE
...
#ifdef DEBUGABLE
	if(dbg)fprintf(stderr, "outband (PAGEEND) vpos %d maxv %d\n", vpos,maxv);
#endif
	if(maxv < vpos)			/* be sure about the end of the */
		maxv = vpos;		/* page (JNA) */
...
#ifdef DEBUGABLE
	if(dbg)fprintf(stderr, "outband outsize %d\n", outsize);
#endif
...
#ifdef DEBUGABLE
	if(dbg)fprintf(stderr, "outband vpos %d\n", vpos);
#endif
	vorigin += NLINES;
	pagelen += NLINES;
...
#ifdef DEBUGABLE
		if(dbg > 1)fprintf(stderr,"fontmap testing :%s:%s:\n",s,fontmap[i].fname);
#endif
		if (strcmp(s, fontmap[i].fname) == 0) {
			s = fontmap[i].ffile;
#ifdef DEBUGABLE
			if(dbg)
				fprintf(stderr, "fontmap found :%s: at mpos %d\n",fontmap[i].ffile, i);
#endif
			break;
		}
	}

	if(fontmap[i].ffile == (char *) 0)
		error(FATAL,"Cannot find bitmaps for font %s", buf);
	fontname[n] = s;
