...
#pragma ident	"@(#)dlpi.h	1.16	92/07/14 SMI"	/* SVr4.0 1.2	*/
...
typedef struct {
	...
	ulong	dl_xidtest_flg;		/* auto init. of test and xid */
} dl_bind_req_t;
...
typedef struct {
	...
	ulong	dl_level;		/* physical,SAP, or ALL multicast */
} dl_promiscon_req_t;
...
typedef struct {
	...
	ulong		dl_reason;	/* norm., abnorm., perm. or trans. */
...
typedef struct {
	...
	ulong	dl_reason;		/* flow control, link error, resync */
} dl_reset_ind_t;
...
typedef struct {
	...
	ulong	dl_src_addr_offset;	/* offset from beginning of block */
	ulong	dl_priority;		/* pri for data unit transmission */
