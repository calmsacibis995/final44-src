/*
 * $Id: if.h,v 1.46.2.1 1995/01/13 19:18:44 jch Exp $
 */
...
struct _if_link {
    ...
    char	ifl_name[IFL_NAMELEN+1];	/* name (duplicated and null terminated) */
    u_short	ifl_index;			/* Interface index */
    sockaddr_un *ifl_addr;			/* Link level address */
    sockaddr_un *ifl_handle;			/* How the kernel identifies this interface */
    if_addr_entry *ifl_addrent;			/* Physical address info */
    if_addr_entry *ifl_nameent;			/* Pointer to name entry */
    if_addr_entry *ifl_nameent_wild;		/* Pointer to name entry for wildcard */
...
#define	ifa_link		ifa_info.ifi_link
    ...
    u_int	ifa_vif;			/* virtual interface index */
...
extern rt_parms int_rtparms;			/* Used to install interface routes learned from the routing socket */
