/*
 * $Id: ospf_mib.c,v 1.36.2.8 1995/01/13 19:11:29 jch Exp $
 */
...
#undef	NDX_SIZE
	...
	return o_ipaddr(oi,
			v,
			sock2unix(sockbuild_in(0, lsdb->lsdb_area->area_id),
				  (int *) 0));
	...
	return o_string(oi, v, DB_RTR(lsdb), ntohs(LS_LEN(lsdb)));
    }
...
#undef	NDX_SIZE
	...
	return o_ipaddr(oi,
			v,
			sock2unix(sockbuild_in(0, host->host_if_addr),
				  (int *) 0));
...
#undef	NDX_SIZE
	...
	return o_ipaddr(oi,
			v,
			sock2unix(sockbuild_in(0, intf->trans_area->area_id),
				  (int *) 0));
