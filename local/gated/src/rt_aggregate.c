/*
 *  $Id: rt_aggregate.c,v 1.30.2.7 1995/04/02 03:28:11 jch Exp $
 */
			 ...
			 starting_depth, u_int)
{
	...
	if (!starting_depth && rth->rth_aggregate_depth) {
		...
		if (BIT_TEST(rtah->rtah_flags, RTAHF_CHANGED)) {
			...
			rt_aggregate_rtparms.rtp_preference = rtah->rtah_rta_forw->rta_preference;
			rt_aggregate_rtparms.rtp_preference2 = aggr_rt->rt_preference2;
#ifdef	PROTO_ASPATHS
		...
		if (delete) {
		    ...
		    RT_CHANGE(rta->rta_head->rtah_rta_rt, rta->rta_head, old_asp, new_asp, new);
		...
		if (add && dm) {
		    ...
		    RT_CHANGE(aggr_rt, rtah, old_asp, new_asp, new);
		}
	    }
	}
