/*
 *  rt_static.c,v 1.13 1994/02/25 22:05:51 jch Exp
 */
	...
	if (BIT_TEST(rtp->rtp_state, RTS_GATEWAY)) {
		...
		if (BIT_TEST(ifap->ifa_state, IFS_UP)
		  && if_policy_match(ifap, rtsp->rts_intfs)) {
			...
			/* destination address */
			rtp->rtp_router = sockishost(rtp->rtp_dest, rtp->rtp_dest_mask) 
			    ? ifap->ifa_addr
				: ifap->ifa_addr_local;
		    } else {
...
#define	COPY_INFO(rtsp, rtp) \
	...
	rtsp = rn->rnode_rts;
	if (!rtsp) {
	    rn->rnode_rts = (rt_static *) task_block_alloc(rt_static_block_index);
	    rtt->rt_table_routes++;
	    rtsp = rn->rnode_rts;
	    COPY_INFO(rtsp, rtp);
	    return FALSE;
	...
	if (!BIT_TEST(rtsp->rts_parms.rtp_state, RTS_DELETE)) {
	    if (err_msg) {
		(void) sprintf(err_msg, "duplicate static route to %A", rtp->rtp_dest);
	    }
	    if (intfs) {
		adv_free_list(intfs);
	    }
