/* 
 * $UCB$
 * $Id: krb.c,v 1.4 1993/12/23 16:42:22 dglo Exp dglo $
 * $Source: /a/guest/dglo/nuk/rkinitd/RCS/krb.c,v $
 * $Author: dglo $
 *
...
static char *rcsid = "$Id: krb.c,v 1.4 1993/12/23 16:42:22 dglo Exp dglo $";
...
#include "krb_paths.h"
#else /* !SWW */
#define _PATH_KRB_SRVTAB	KEYFILE
#endif /* SWW */
#include "sys_proto.h"
...
#ifdef __STDC__
static int validate_user(rkinit_info *info, char *errmsg)
#else
static int validate_user(info, errmsg)
  rkinit_info *info;
  char *errmsg;
#endif /* __STDC__ */
{
    struct passwd *pwnam;	/* For access_check and uid */
    char *aname = info->aname;
    char *inst = info->inst;
    char *realm = info->realm;
	 ...
	 */
    	if (strlen(info->tktfilename) == 0)
		sprintf(info->tktfilename, "%s%d", TKT_ROOT, pwnam->pw_uid);
	setenv("KRBTKFILE", info->tktfilename);

	if (access(info->tktfilename, R_OK|W_OK) < 0) {
		if (errno != ENOENT) {
			sprintf(errmsg, "No access to %s: %s\n",
				info->tktfilename, sys_errlist[errno]);
			goto tell_flop;
		}
	}
    } else {
...
#endif /* __STDC__ */
{
     ...
     */
    if ((status = validate_user(&info, errmsg)) != RKINIT_SUCCESS) {
    ...
    if (pwnam = getpwnam(info.username))
	(void) chown(info.tktfilename, pwnam->pw_uid, pwnam->pw_gid);
    return(RKINIT_SUCCESS);
}
