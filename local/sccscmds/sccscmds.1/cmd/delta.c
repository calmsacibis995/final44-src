...
static char Sccsid[] = "@(#)delta.c	4.13	3/3/91";
USXALLOC();
...
# ifdef LOGDELTA
	...
	if (Logf = fopen(_PATH_SCCSLOG, "a")) {
		(void)flock(fileno(Logf), LOCK_EX);	/* race XXX */
		(void)fseek(Logf, 0L, SEEK_END);	/* just in case */
		if (pkt->p_file[0] != '/' && (p = getcwd(NULL)))
			(void)fprintf(Logf, "%s/", p);
		(void)fprintf(Logf,
		    "%s:\n%s%s\n", pkt->p_file, str + 5, Comments);
		(void)fclose(Logf);			/* implicit unlock */
	}
	}
# endif
	if (ilist)
