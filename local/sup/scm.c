 ...
 **********************************************************************
 * HISTORY
 * 17-Sept-90 Mary Thompson (mrt) Carnegie Mellon
 *       Changed to use builtin port for supfiledbg as well as supfileserv
 *       if getservbyname can't find it..
 * $Log:	scm.c,v $
...
int *retry;
{
	...
	if ((sp = getservbyname(server,"tcp")) == 0) {
		if (strcmp(server, FILEPORT) == 0){
			port = htons((u_short)FILEPORTNUM);
			printf("getservbyname can't find %s, using port %d\n",
			       server,port);
		      }
