...
retry:
...
#ifdef KSHELL
	if((fd=fdopen(open(histname,O_RDWR|O_APPEND),"a+"))==NULL)
	{
...
#endif	/* KSHELL */
	if((hsize == 0 || login_sh) && flock(fileno(fd),LOCK_EX|LOCK_NB) == 0)
		lockex = 1;
	else {
		(void)flock(fileno(fd),LOCK_SH);
		if (hsize > 0 && (hsize=lseek(fileno(fd),0L,2)) == 0) {
			fclose(fd);
			goto retry;
		}
	}
	...
	if(*fname)
		unlink(fname);
	if(login_sh && his_start>1 && hsize > HISBIG && lockex)
	{
		FILE *fdo;
		if((fdo=fdopen(open(histname,0),"r+"))==NULL)
			return(0);
		/*
		 * we lose our lock on the histfile here,
		 * between the unlink and our recursive call
		 * from hist_trim
		 */
		unlink(histname);
		hist_trim(fdo,fp->fixind-maxlines);
	}
	else if(lockex && !trimming)
		(void)flock(fileno(fd),LOCK_SH);
	return(0);
}
...
register FILE *fdo;
{
	...
	while(c!=EOF);
	(void)ftruncate(fileno(fdo), 0L);
	fclose(fdo);
#ifdef totally_gross
	free((char*)fdo->_base);
#endif
	(void)flock(fileno(fc_fix->fixfd),LOCK_SH);
	free((char*)fp);
	trimming = 0;
}
