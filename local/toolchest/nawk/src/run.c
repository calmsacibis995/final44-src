...
Cell *call(a, n) Node **a;
{
	...
	if (args == NULL || oargs == NULL)
		ERROR "out of space for function arguments calling %s", s FATAL;
	...
	while (*s) {
		...
		for (t=fmt; (*t++ = *s) != '\0'; s++) {
			if (islower(*s) && *s != 'l')
				break;
			if (*s == '*')
				ERROR "in %s: %%*... conversion not supported.  try concatenation.", os FATAL;
		}
		*t = '\0';
		if (t >= fmt + sizeof(fmt))
		...
		case 5: isnum(x) ? sprintf(p, fmt, (int) getfval(x))
