...
DEPBLOCK nextdbl;		/* final value for chain */
{
	...
	char temp[100+MAXNAMLEN];
	char fullname[100+MAXNAMLEN];
	NAMEBLOCK q;
	DEPBLOCK thisdbl;
	OPENDIR od;
	int dirofl = 0;
	...
	for(od=firstod ; od!=0; od = od->nextopendir)
		...
		{
			dirf = od->dirfc;
			rewinddir(dirf); /* start over at the beginning  */ /* MKCV */
			break;
		}
	...
	else	while((entry = readdir(dirf)) != NULL) /* MKCV */
		{
			if( amatch(entry->d_name,filepat) ) /* MKCV */
			{
