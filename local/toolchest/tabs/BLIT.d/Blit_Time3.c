 ...
 */
/*
From uucp Mon Jul 30 10:51 EDT 1984
>From pkc Mon Jul 30 10:49 EDT 1984 remote from sfbai
To: eagle!blewett
Subject: Blit_Time2.c with a bell

From uucp Thu Jul 26 11:26 EDT 1984
>From pkc Thu Jul 26 11:20 EDT 1984 remote from sfbai
I have made some changes to Blit_Time2.c to make it work like an alarm 
clock with appointment messages.  The new "-r" option will cause it to ring 
the bell whenever there is a change in the appointment message orders.  
This includes new appointment message being displayed, or an 
intermediate one having been deleted, thereby moving up other messages. 
However, the last message, when it is deleted, will not ring the bell.  
If you think this little change is useful, I would be more than happy to
mail you the new changes.
							Peter Chen
							45414 sfbai!pkc
P.S. some other useful additons to Blit_Time maight be: displaying the
current working directory along with the date/time message line, allowing
the user to specify the number of times the bell will ring, and to turn off
the bell and/or appointment messages that no loger need to be displayed. 
 ...
 */
/* 
 * Blit_Time2 can work like an alarm clock with your appointment messages.
 * The "-r" option will cause it to ring the bell whenever there is a change in the 
 * appointment message orders. This includes new appointment message being
 * displayed, or an intermediate one having been deleted, thereby moving up other
 * messages. However, the last message, when it is deleted, will not ring the
 * bell. 
 *                                                   pkc
 */
...
char  **argv;
{
    ...
    {
	fprintf (stderr,
		"Usage: Time [-r] bin-directory-name delay-time &\n");
	exit (1);
    }
    for (i = 0; i < 10; i++)
        msg_save[i][0] = '\0';
...
/*    freopen ("/dev/null", "w", stderr);  */
	...
	switch (MsgCount)
	{
		...
		for (i = 0; i < text_cnt; i++)
		{
		    ...
		    if (rflag == 1 ) {
		        if (strcmp(msg_save[i], Msg_Text[i]) != 0) {
			    strcpy(msg_save[i], Msg_Text[i]);
			    fputc('\007', stderr);
			    rflag ++;
			}
		    }
		    else if (rflag == 2 && strcmp(msg_save[i], Msg_Text[i]) != 0)
			strcpy(msg_save[i], Msg_Text[i]);
...
time_t itime;
{
    ...
    if (pre_tcnt < text_cnt)
        for (i = pre_tcnt; i < text_cnt; i++)
            msg_save[i][0] = '\0';
    pre_tcnt = text_cnt;
