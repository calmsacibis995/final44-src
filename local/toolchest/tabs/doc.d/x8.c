 ...
 */
	...
	Def_Char_Action (CTRL(D), BREAK, mybrkchar, EOF_LIST);	/* Page Down */
	old_eof = nttybuf.c_cc[VEOF];
	nttybuf.c_cc[VEOF] = 0;		/* ^@ - NULL will act like EOF */
	Def_Char_Action (CTRL(@), BREAK, brkchar, EOF_LIST);
	Def_Char_Action (CTRL(U), BREAK, mybrkchar, EOF_LIST);	/* Page Up */
