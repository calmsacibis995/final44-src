...
for K_HOME.
.P
Get_List() then determines which field it should use for the next
call to Get_Field().
The appropriate post-action routine, 
reg_post(), is called and an index to the next field is returned.
...
.I brkchar
actions only have their normal effect when:
.BL
.LI
there is no post-action routine 
.LI
any post-action routines that can effect the
field return the same index that is passed to them.
.LE
The reg_post() routine takes advantage of this so that 
CTRL(D) and CTRL(U) will page and
K_HOME will not have its normal effect.
That is,
...
Some BREAK characters such as `?' do not cause a return from 
Get_Field and therefore no action routines are called.
These BREAK characters always return control to the same field.
...
control to be moved to another field.
