...
static	void inspect_ifp();
static	struct	walkarg {
	struct	interface w_ifs;
	int	w_index;	
	int	w_flags;
	int	w_noipaddr;
};
...
ifinit()
{
	...
	w.w_index = w.w_noipaddr = 0;
	if (cp = ifa_walk(inspect_ifp, (void *)&w)) {
		syslog(LOG_ERR, "ifinit: %m, %s", cp);
		return;
	}
	if (w.w_noipaddr)
		lookingforinterfaces = 1;
	if (externalinterfaces > 1 && supplier < 0)
		supplier = 1;
}

void
inspect_ifp(ifam, ifm, info, if_walkarg)
	struct	ifa_msghdr *ifam;
	struct	if_msghdr *ifm;
	register struct	rt_addrinfo *info;
	void	*if_walkarg;
{
	register struct walkarg *w = (struct walkarg *)if_walkarg;
...
#define ifs	w->w_ifs
	...
	if (ifm->ifm_index != w->w_index) {
		bzero(&ifs, sizeof(ifs));
		ifs.int_flags = ifm->ifm_flags | IFF_INTERFACE;
		if ((w->w_flags & IFF_UP) == 0 || w->w_noipaddr)
			lookforinterfaces = 1;
		w->w_noipaddr = 1;
		sdl->sdl_data[sdl->sdl_nlen] = 0;
		ifs.int_name = sdl->sdl_data;
	}
	if (ifam == 0 || ifaaddr == 0 || (ifs.int_flags & IFF_UP) == 0)
		return;
	...
	*ifp = ifs;
	...
	strcpy(ifp->int_name, ifs.int_name);
	/*
