...
/*
 * Remove a directory entry. Used for "undone" creates and not done
 * removes.  If this entry is at offset 0 on a page, then we reclaim
 * it by simply setting its inode number to 0.  If it is not the first
 * entry on the page, then we have to find the previous entry
 * and add space to it.
 */
int
remdir(struct dinode *dip, struct dirent *dp, struct slot *locp)
{
	struct dirent *xdp;
	struct bufarea *bp;
	int curoff;
	...
	bp = getdirblk(locp->block, dblksize(fs, dip, locp->block));

	if (locp->off == 0)
		dp->d_fileno = 0;
	else {
		pagep = (char *)dp - locp->off;
		xdp = (struct dirent *)pagep;
		for (curoff = 0; curoff < locp->off; curoff += DIRSIZ(0, xdp))
			xdp = (struct dirent *)(pagep + curoff);
		xdp->d_reclen += dp->d_reclen;
	}
	dirty(bp);
	return(0);
}
